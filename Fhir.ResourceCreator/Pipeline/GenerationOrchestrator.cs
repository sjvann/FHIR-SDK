    ...
    public static async Task RunRegistryPackagesAsync(GeneratorOptions options, CancellationToken ct = default)
    {
        ...
        foreach (var pkg in options.Packages)
        {
            ...
            foreach (var file in jsonFiles)
            {
                ...
                var choiceCode = ChoiceHelperGenerator.Generate(model, rootNs);
                if (!string.IsNullOrEmpty(choiceCode))
                {
                    var choicePath = Path.Combine(projDir, $"{typeName}.Choice.generated.cs");
            ...
            if (FhirLineLabels.TryParseFromResourcesProject(outputName, out var lineLabel))
            {
                var repoRoot = FindRepoRoot();
                FhirLineScaffoldEmitter.Emit(repoRoot, lineLabel, outputName, projDir);
            }
        }
    }
