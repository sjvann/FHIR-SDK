...
/// </remarks>
public class Duration : Element, IExtensibleTypeFramework
{
    /// <summary>
    /// 數值（隱含精度）
    ...
    /// 建立物件的深層複本
    /// </summary>
    /// <returns>Duration 的深層複本</returns>
    public override Base DeepCopy()
    {
        var copy = new Duration
        {
            Id = Id,
            Value = Value,
    ...
    /// <summary>
    /// 判斷與另一個 Duration 物件是否相等
    /// </summary>
    /// <param name="other">要比較的物件</param>
    /// <returns>如果兩個物件相等則為 true，否則為 false</returns>
    public override bool IsExactly(Base other)
    {
        if (other is not Duration otherDuration)
            return false;

        return base.IsExactly(other) &&
               Equals(Value, otherDuration.Value) &&
               Equals(Comparator, otherDuration.Comparator) &&
               Equals(Unit, otherDuration.Unit) &&
               Equals(System, otherDuration.System) &&
               Equals(Code, otherDuration.Code);
    }

    /// <summary>
    /// 驗證 Duration 是否符合 FHIR 規範
    /// </summary>
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        if (!string.IsNullOrEmpty(Comparator))
        {
            ...
            {
                yield return new ValidationResult($"Duration.comparator '{Comparator}' is not a valid comparator");
            }
        }
        ...
        {
            yield return new ValidationResult("Duration.system must be a valid absolute URI");
        }
