...
/// </remarks>
public class Age : Element, IExtensibleTypeFramework
{
    /// <summary>
    /// 數值（隱含精度）
    /// FHIR Path: Age.value
    ...
    {
        if (other is not Age otherAge)
            return false;

        return base.IsExactly(other) &&
               Equals(Value, otherAge.Value) &&
               Equals(Comparator, otherAge.Comparator) &&
               Equals(Unit, otherAge.Unit) &&
               Equals(System, otherAge.System) &&
               Equals(Code, otherAge.Code);
    }
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        if (!string.IsNullOrEmpty(Comparator))
        {
            ...
            {
                yield return new ValidationResult($"Age.comparator '{Comparator}' is not a valid comparator");
            }
        }
        ...
        {
            yield return new ValidationResult("Age.system must be a valid absolute URI");
        }
