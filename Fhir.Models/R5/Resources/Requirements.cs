...
/// <summary>
/// The Requirements resource is used to describe an actor - a human or an application that plays a role
/// in data exchange, and that may have obligations associated with the role the actor plays.
/// </summary>
public class Requirements : DomainResource
{
    ...
    /// <summary>
    /// An absolute URI that is used to identify this Requirements when it is referenced in a specification,
    /// model, design or an instance; also called its canonical identifier. This SHOULD be globally unique
    /// and SHOULD be a literal address at which an authoritative instance of this Requirements is (or will
    /// be) published. This URL can be the target of a canonical reference. It SHALL remain the same when
    /// the Requirements is stored on different servers.
    /// </summary>
    ...
    /// <summary>
    /// The identifier that is used to identify this version of the Requirements when it is referenced in a
    /// specification, model, design or instance. This is an arbitrary value managed by the Requirements
    ...
    /// <summary>
    /// A natural language name identifying the Requirements. This name should be usable as an identifier
    ...
    /// <summary>
    /// The date (and optionally time) when the Requirements was published. The date must change when the
    ...
    /// appropriate Requirements instances.
    /// </summary>
    ...
    /// <summary>
    /// A copyright statement relating to the Requirements and/or its contents. Copyright statements are
    /// generally legal restrictions on the use and publishing of the Requirements.
    /// </summary>
    ...
    /// </summary>
    [FhirElement("derivedFrom", Order = 36)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("reference", Order = 37)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("statement", Order = 39)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("statement")]
    public List<BackboneElement>? Statement { get; set; }
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate Requirements cardinality
        var requirementsCount = Requirements?.Count ?? 0;
        if (requirementsCount < 0)
        {
            yield return new ValidationResult("Element 'Requirements' cardinality must be 0..*", new[] { nameof(Requirements) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'Requirements.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Requirements.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Requirements.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'Requirements.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'Requirements.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        // Validate Contact cardinality
        var contactCount = Contact?.Count ?? 0;
        if (contactCount < 0)
        {
            yield return new ValidationResult("Element 'Requirements.contact' cardinality must be 0..*", new[] { nameof(Contact) });
        }
        // Validate UseContext cardinality
        var usecontextCount = UseContext?.Count ?? 0;
        if (usecontextCount < 0)
        {
            yield return new ValidationResult("Element 'Requirements.useContext' cardinality must be 0..*", new[] { nameof(UseContext) });
        }
        // Validate Jurisdiction cardinality
        var jurisdictionCount = Jurisdiction?.Count ?? 0;
        if (jurisdictionCount < 0)
        {
            yield return new ValidationResult("Element 'Requirements.jurisdiction' cardinality must be 0..*", new[] { nameof(Jurisdiction) });
        }
        // Validate DerivedFrom cardinality
        var derivedfromCount = DerivedFrom?.Count ?? 0;
        if (derivedfromCount < 0)
        {
            yield return new ValidationResult("Element 'Requirements.derivedFrom' cardinality must be 0..*", new[] { nameof(DerivedFrom) });
        }
        // Validate Reference cardinality
        var referenceCount = Reference?.Count ?? 0;
        if (referenceCount < 0)
        {
            yield return new ValidationResult("Element 'Requirements.reference' cardinality must be 0..*", new[] { nameof(Reference) });
        }
        // Validate Actor cardinality
        var actorCount = Actor?.Count ?? 0;
        if (actorCount < 0)
        {
            yield return new ValidationResult("Element 'Requirements.actor' cardinality must be 0..*", new[] { nameof(Actor) });
        }
        // Validate Statement cardinality
        var statementCount = Statement?.Count ?? 0;
        if (statementCount < 0)
        {
            yield return new ValidationResult("Element 'Requirements.statement' cardinality must be 0..*", new[] { nameof(Statement) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Requirements.statement.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Requirements.statement.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Key == null)
        {
            yield return new ValidationResult("Element 'Requirements.statement.key' cardinality must be 1..1", new[] { nameof(Key) });
        }
        // Validate Conformance cardinality
        var conformanceCount = Conformance?.Count ?? 0;
        if (conformanceCount < 0)
        {
            yield return new ValidationResult("Element 'Requirements.statement.conformance' cardinality must be 0..*", new[] { nameof(Conformance) });
        }
        if (Requirement == null)
        {
            yield return new ValidationResult("Element 'Requirements.statement.requirement' cardinality must be 1..1", new[] { nameof(Requirement) });
        }
        // Validate SatisfiedBy cardinality
        var satisfiedbyCount = SatisfiedBy?.Count ?? 0;
        if (satisfiedbyCount < 0)
        {
            yield return new ValidationResult("Element 'Requirements.statement.satisfiedBy' cardinality must be 0..*", new[] { nameof(SatisfiedBy) });
        }
        // Validate Reference cardinality
        var referenceCount = Reference?.Count ?? 0;
        if (referenceCount < 0)
        {
            yield return new ValidationResult("Element 'Requirements.statement.reference' cardinality must be 0..*", new[] { nameof(Reference) });
        }
        // Validate Source cardinality
        var sourceCount = Source?.Count ?? 0;
        if (sourceCount < 0)
        {
            yield return new ValidationResult("Element 'Requirements.statement.source' cardinality must be 0..*", new[] { nameof(Source) });
        }
        ...
        // Validate Conformance ValueSet binding
        if (Conformance != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/conformance-expectation|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("Name should be usable as an identifier for the module by machine processing applications such as code generation", new[] { nameof(Requirements) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Statement) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Conformance) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Conditionality) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(SatisfiedBy) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
