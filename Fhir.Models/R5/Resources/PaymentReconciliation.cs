...
public class PaymentReconciliation : DomainResource
{
    ...
    /// <summary>
    /// The workflow or activity which gave rise to or during which the payment ocurred such as a kiosk,
    /// deposit on account, periodic payment etc.
    /// </summary>
    [FhirElement("kind", Order = 21)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("issuerType", Order = 25)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("paymentIssuer", Order = 26)]
    [Cardinality(0, 1)]
    [JsonPropertyName("paymentIssuer")]
    public Reference<Organization> PaymentIssuer { get; set; }
    ...
    public Reference<Task> Request { get; set; }

    /// <summary>
    /// The practitioner who is responsible for the services rendered to the patient.
    /// </summary>
    [FhirElement("requestor", Order = 28)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("disposition", Order = 30)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("cardBrand", Order = 34)]
    [Cardinality(0, 1)]
    ...
    /// purposes.
    /// </summary>
    [FhirElement("accountNumber", Order = 35)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("expirationDate", Order = 36)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("processor", Order = 37)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("referenceNumber", Order = 38)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("authorization", Order = 39)]
    [Cardinality(0, 1)]
    [JsonPropertyName("authorization")]
    public FhirString? Authorization { get; set; }

    /// <summary>
    /// The amount offered by the issuer, typically applies to cash when the issuer provides an amount in
    /// bank note denominations equal to or excess of the amount actually being paid.
    /// </summary>
    [FhirElement("tenderedAmount", Order = 40)]
    [Cardinality(0, 1)]
    ...
    /// &apos;change&apos;.
    /// </summary>
    [FhirElement("returnedAmount", Order = 41)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("paymentIdentifier", Order = 43)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("allocation", Order = 44)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("allocation")]
    public List<BackboneElement>? Allocation { get; set; }

    /// <summary>
    /// A code for the form to be used for printing the content.
    /// </summary>
    [FhirElement("formCode", Order = 45)]
    [Cardinality(0, 1)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate PaymentReconciliation cardinality
        var paymentreconciliationCount = PaymentReconciliation?.Count ?? 0;
        if (paymentreconciliationCount < 0)
        {
            yield return new ValidationResult("Element 'PaymentReconciliation' cardinality must be 0..*", new[] { nameof(PaymentReconciliation) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'PaymentReconciliation.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'PaymentReconciliation.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'PaymentReconciliation.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'PaymentReconciliation.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Type == null)
        {
            yield return new ValidationResult("Element 'PaymentReconciliation.type' cardinality must be 1..1", new[] { nameof(Type) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'PaymentReconciliation.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        if (Created == null)
        {
            yield return new ValidationResult("Element 'PaymentReconciliation.created' cardinality must be 1..1", new[] { nameof(Created) });
        }
        if (Date == null)
        {
            yield return new ValidationResult("Element 'PaymentReconciliation.date' cardinality must be 1..1", new[] { nameof(Date) });
        }
        if (Amount == null)
        {
            yield return new ValidationResult("Element 'PaymentReconciliation.amount' cardinality must be 1..1", new[] { nameof(Amount) });
        }
        // Validate Allocation cardinality
        var allocationCount = Allocation?.Count ?? 0;
        if (allocationCount < 0)
        {
            yield return new ValidationResult("Element 'PaymentReconciliation.allocation' cardinality must be 0..*", new[] { nameof(Allocation) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'PaymentReconciliation.allocation.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'PaymentReconciliation.allocation.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate ProcessNote cardinality
        var processnoteCount = ProcessNote?.Count ?? 0;
        if (processnoteCount < 0)
        {
            yield return new ValidationResult("Element 'PaymentReconciliation.processNote' cardinality must be 0..*", new[] { nameof(ProcessNote) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'PaymentReconciliation.processNote.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'PaymentReconciliation.processNote.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        ...
        if (Type != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/payment-type
            // This requires a terminology service or local ValueSet cache
        }
        ...
        if (Kind != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/payment-kind
            // This requires a terminology service or local ValueSet cache
        }
        // Validate IssuerType ValueSet binding
        if (IssuerType != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/payment-issuertype
            // This requires a terminology service or local ValueSet cache
        }
        // Validate Outcome ValueSet binding
        if (Outcome != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/payment-outcome|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate Method ValueSet binding
        if (Method != null)
        {
            // TODO: Implement ValueSet validation for http://terminology.hl7.org/ValueSet/v2-0570
            // This requires a terminology service or local ValueSet cache
        }
        // Validate Type ValueSet binding
        if (Type != null)
        ...
        if (Type != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/note-type|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(PaymentReconciliation) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(PaymentReconciliation) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(PaymentReconciliation) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(PaymentReconciliation) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(PaymentReconciliation) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(IssuerType) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(PaymentIssuer) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(CardBrand) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(AccountNumber) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Processor) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(ReferenceNumber) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(TenderedAmount) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(ReturnedAmount) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(PaymentIdentifier) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Allocation) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(TargetItem) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Submitter) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
