...
/// <summary>
/// A record of an event relevant for purposes such as operations, privacy, security, maintenance, and
...
public class AuditEvent : DomainResource
{
    ...
    /// </summary>
    [FhirElement("severity", Order = 21)]
    [Cardinality(0, 1)]
    [JsonPropertyName("severity")]
    public FhirCode? Severity { get; set; }

    /// <summary>
    /// The time or period during which the activity occurred. (as Period)
    /// </summary>
    [FhirElement("occurredPeriod", Order = 22)]
    [Cardinality(0, 1)]
    ...
    /// <summary>
    /// The time or period during which the activity occurred. (as dateTime)
    /// </summary>
    [FhirElement("occurredDateTime", Order = 23)]
    [Cardinality(0, 1)]
    ...
    /// outcome.text.
    /// </summary>
    [FhirElement("outcome", Order = 25)]
    [Cardinality(0, 1)]
    [JsonPropertyName("outcome")]
    public BackboneElement Outcome { get; set; }

    /// <summary>
    /// The authorization (e.g., PurposeOfUse) that was used during the event being recorded.
    /// </summary>
    [FhirElement("authorization", Order = 26)]
    [Cardinality(0, int.MaxValue)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate AuditEvent cardinality
        var auditeventCount = AuditEvent?.Count ?? 0;
        if (auditeventCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent' cardinality must be 0..*", new[] { nameof(AuditEvent) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Category cardinality
        var categoryCount = Category?.Count ?? 0;
        if (categoryCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.category' cardinality must be 0..*", new[] { nameof(Category) });
        }
        if (Code == null)
        {
            yield return new ValidationResult("Element 'AuditEvent.code' cardinality must be 1..1", new[] { nameof(Code) });
        }
        if (Recorded == null)
        {
            yield return new ValidationResult("Element 'AuditEvent.recorded' cardinality must be 1..1", new[] { nameof(Recorded) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.outcome.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.outcome.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Code == null)
        {
            yield return new ValidationResult("Element 'AuditEvent.outcome.code' cardinality must be 1..1", new[] { nameof(Code) });
        }
        // Validate Detail cardinality
        var detailCount = Detail?.Count ?? 0;
        if (detailCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.outcome.detail' cardinality must be 0..*", new[] { nameof(Detail) });
        }
        // Validate Authorization cardinality
        var authorizationCount = Authorization?.Count ?? 0;
        if (authorizationCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.authorization' cardinality must be 0..*", new[] { nameof(Authorization) });
        }
        // Validate BasedOn cardinality
        var basedonCount = BasedOn?.Count ?? 0;
        if (basedonCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.basedOn' cardinality must be 0..*", new[] { nameof(BasedOn) });
        }
        // Validate Agent cardinality
        var agentCount = Agent?.Count ?? 0;
        if (agentCount < 1)
        {
            yield return new ValidationResult("Element 'AuditEvent.agent' cardinality must be 1..*", new[] { nameof(Agent) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.agent.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.agent.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Role cardinality
        var roleCount = Role?.Count ?? 0;
        if (roleCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.agent.role' cardinality must be 0..*", new[] { nameof(Role) });
        }
        if (Who == null)
        {
            yield return new ValidationResult("Element 'AuditEvent.agent.who' cardinality must be 1..1", new[] { nameof(Who) });
        }
        // Validate Policy cardinality
        var policyCount = Policy?.Count ?? 0;
        if (policyCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.agent.policy' cardinality must be 0..*", new[] { nameof(Policy) });
        }
        // Validate Authorization cardinality
        var authorizationCount = Authorization?.Count ?? 0;
        if (authorizationCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.agent.authorization' cardinality must be 0..*", new[] { nameof(Authorization) });
        }
        if (Source == null)
        {
            yield return new ValidationResult("Element 'AuditEvent.source' cardinality must be 1..1", new[] { nameof(Source) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.source.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.source.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Observer == null)
        {
            yield return new ValidationResult("Element 'AuditEvent.source.observer' cardinality must be 1..1", new[] { nameof(Observer) });
        }
        // Validate Type cardinality
        var typeCount = Type?.Count ?? 0;
        if (typeCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.source.type' cardinality must be 0..*", new[] { nameof(Type) });
        }
        // Validate Entity cardinality
        var entityCount = Entity?.Count ?? 0;
        if (entityCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.entity' cardinality must be 0..*", new[] { nameof(Entity) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.entity.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.entity.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate SecurityLabel cardinality
        var securitylabelCount = SecurityLabel?.Count ?? 0;
        if (securitylabelCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.entity.securityLabel' cardinality must be 0..*", new[] { nameof(SecurityLabel) });
        }
        // Validate Detail cardinality
        var detailCount = Detail?.Count ?? 0;
        if (detailCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.entity.detail' cardinality must be 0..*", new[] { nameof(Detail) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.entity.detail.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.entity.detail.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Type == null)
        {
            yield return new ValidationResult("Element 'AuditEvent.entity.detail.type' cardinality must be 1..1", new[] { nameof(Type) });
        }
        if (Value == null)
        {
            yield return new ValidationResult("Element 'AuditEvent.entity.detail.value[x]' cardinality must be 1..1", new[] { nameof(Value) });
        }
        // Validate Agent cardinality
        var agentCount = Agent?.Count ?? 0;
        if (agentCount < 0)
        {
            yield return new ValidationResult("Element 'AuditEvent.entity.agent' cardinality must be 0..*", new[] { nameof(Agent) });
        }
        ...
        // Validate Action ValueSet binding
        if (Action != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/audit-event-action|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate Severity ValueSet binding
        if (Severity != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/audit-event-severity|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(AuditEvent) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(AuditEvent) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(AuditEvent) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(AuditEvent) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(AuditEvent) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Observer) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Query) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
