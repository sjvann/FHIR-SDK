...
public class MessageDefinition : DomainResource
{
    ...
    /// <summary>
    /// A formal identifier that is used to identify this message definition when it is represented in other
    ...
    /// <summary>
    /// The identifier that is used to identify this version of the message definition when it is referenced
    /// in a specification, model, design or instance. This is an arbitrary value managed by the message
    ...
    /// <summary>
    /// The date (and optionally time) when the message definition was last significantly changed. The date
    ...
    /// <summary>
    /// A copyright statement relating to the message definition and/or its contents. Copyright statements
    /// are generally legal restrictions on the use and publishing of the message definition.
    /// </summary>
    ...
    /// </summary>
    [FhirElement("parent", Order = 38)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("parent")]
    public List<FhirCanonical>? Parent { get; set; }

    /// <summary>
    /// Event code or link to the EventDefinition. (as Coding)
    /// </summary>
    [FhirElement("eventCoding", Order = 39)]
    [Cardinality(1, 1)]
    [ChoiceType("event", "coding")]
    [Required]
    [JsonPropertyName("eventCoding")]
    public Coding EventCoding { get; set; }

    /// <summary>
    /// Event code or link to the EventDefinition. (as uri)
    /// </summary>
    [FhirElement("eventUri", Order = 40)]
    [Cardinality(1, 1)]
    [ChoiceType("event", "uri")]
    [Required]
    ...
    /// </summary>
    [FhirElement("focus", Order = 42)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("focus")]
    public List<BackboneElement>? Focus { get; set; }

    /// <summary>
    /// Declare at a message definition level whether a response is required or only upon error or success,
    /// or never.
    /// </summary>
    [FhirElement("responseRequired", Order = 43)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("allowedResponse", Order = 44)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// reference to a GraphDefinition that it controls what additional resources are to be added to the
    /// Bundle when building the message. The GraphDefinition can also specify profiles that apply to the
    /// various resources.
    /// </summary>
    [FhirElement("graph", Order = 45)]
    [Cardinality(0, 1)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Choice Type validation for event[x]
        var eventProperties = new[] { nameof(EventCoding), nameof(EventUri) };
        var eventSetCount = eventProperties.Count(prop => 
            GetType().GetProperty(prop)?.GetValue(this) != null);

        if (eventSetCount > 1)
        {
            yield return new ValidationResult(
                "Only one of EventCoding, EventUri may be specified",
                new[] { nameof(EventCoding), nameof(EventUri) });
        }
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate MessageDefinition cardinality
        var messagedefinitionCount = MessageDefinition?.Count ?? 0;
        if (messagedefinitionCount < 0)
        {
            yield return new ValidationResult("Element 'MessageDefinition' cardinality must be 0..*", new[] { nameof(MessageDefinition) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'MessageDefinition.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'MessageDefinition.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'MessageDefinition.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'MessageDefinition.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        // Validate Replaces cardinality
        var replacesCount = Replaces?.Count ?? 0;
        if (replacesCount < 0)
        {
            yield return new ValidationResult("Element 'MessageDefinition.replaces' cardinality must be 0..*", new[] { nameof(Replaces) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'MessageDefinition.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        if (Date == null)
        {
            yield return new ValidationResult("Element 'MessageDefinition.date' cardinality must be 1..1", new[] { nameof(Date) });
        }
        // Validate Contact cardinality
        var contactCount = Contact?.Count ?? 0;
        if (contactCount < 0)
        {
            yield return new ValidationResult("Element 'MessageDefinition.contact' cardinality must be 0..*", new[] { nameof(Contact) });
        }
        // Validate UseContext cardinality
        var usecontextCount = UseContext?.Count ?? 0;
        if (usecontextCount < 0)
        {
            yield return new ValidationResult("Element 'MessageDefinition.useContext' cardinality must be 0..*", new[] { nameof(UseContext) });
        }
        // Validate Jurisdiction cardinality
        var jurisdictionCount = Jurisdiction?.Count ?? 0;
        if (jurisdictionCount < 0)
        {
            yield return new ValidationResult("Element 'MessageDefinition.jurisdiction' cardinality must be 0..*", new[] { nameof(Jurisdiction) });
        }
        // Validate Parent cardinality
        var parentCount = Parent?.Count ?? 0;
        if (parentCount < 0)
        {
            yield return new ValidationResult("Element 'MessageDefinition.parent' cardinality must be 0..*", new[] { nameof(Parent) });
        }
        if (Event == null)
        {
            yield return new ValidationResult("Element 'MessageDefinition.event[x]' cardinality must be 1..1", new[] { nameof(Event) });
        }
        // Validate Focus cardinality
        var focusCount = Focus?.Count ?? 0;
        if (focusCount < 0)
        {
            yield return new ValidationResult("Element 'MessageDefinition.focus' cardinality must be 0..*", new[] { nameof(Focus) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'MessageDefinition.focus.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'MessageDefinition.focus.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Code == null)
        {
            yield return new ValidationResult("Element 'MessageDefinition.focus.code' cardinality must be 1..1", new[] { nameof(Code) });
        }
        if (Min == null)
        {
            yield return new ValidationResult("Element 'MessageDefinition.focus.min' cardinality must be 1..1", new[] { nameof(Min) });
        }
        // Validate AllowedResponse cardinality
        var allowedresponseCount = AllowedResponse?.Count ?? 0;
        if (allowedresponseCount < 0)
        {
            yield return new ValidationResult("Element 'MessageDefinition.allowedResponse' cardinality must be 0..*", new[] { nameof(AllowedResponse) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'MessageDefinition.allowedResponse.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'MessageDefinition.allowedResponse.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Message == null)
        {
            yield return new ValidationResult("Element 'MessageDefinition.allowedResponse.message' cardinality must be 1..1", new[] { nameof(Message) });
        }
        ...
        if (Category != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/message-significance-category|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate Code ValueSet binding
        if (Code != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/resource-types|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate ResponseRequired ValueSet binding
        if (ResponseRequired != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/messageheader-response-request|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("Name should be usable as an identifier for the module by machine processing applications such as code generation", new[] { nameof(MessageDefinition) });
        // }
        // Constraint: ele-1
        ...
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("max='*' or (max.toInteger() > 0)"))
        // {
        //     yield return new ValidationResult("Max must be postive int or *", new[] { nameof(Focus) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(ResponseRequired) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(AllowedResponse) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Message) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Situation) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Graph) });
        // }
    }
