...
/// </summary>
public class DeviceAssociation : DomainResource
{
    ...
    /// </summary>
    [FhirElement("bodyStructure", Order = 24)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("operation", Order = 26)]
    [Cardinality(0, int.MaxValue)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate DeviceAssociation cardinality
        var deviceassociationCount = DeviceAssociation?.Count ?? 0;
        if (deviceassociationCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceAssociation' cardinality must be 0..*", new[] { nameof(DeviceAssociation) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceAssociation.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceAssociation.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceAssociation.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceAssociation.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Device == null)
        {
            yield return new ValidationResult("Element 'DeviceAssociation.device' cardinality must be 1..1", new[] { nameof(Device) });
        }
        // Validate Category cardinality
        var categoryCount = Category?.Count ?? 0;
        if (categoryCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceAssociation.category' cardinality must be 0..*", new[] { nameof(Category) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'DeviceAssociation.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        // Validate StatusReason cardinality
        var statusreasonCount = StatusReason?.Count ?? 0;
        if (statusreasonCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceAssociation.statusReason' cardinality must be 0..*", new[] { nameof(StatusReason) });
        }
        // Validate Operation cardinality
        var operationCount = Operation?.Count ?? 0;
        if (operationCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceAssociation.operation' cardinality must be 0..*", new[] { nameof(Operation) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceAssociation.operation.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceAssociation.operation.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'DeviceAssociation.operation.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        // Validate Operator cardinality
        var operatorCount = Operator?.Count ?? 0;
        if (operatorCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceAssociation.operation.operator' cardinality must be 0..*", new[] { nameof(Operator) });
        }
        ...
        if (Status != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/deviceassociation-status|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate StatusReason ValueSet binding
        if (StatusReason != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/deviceassociation-status-reason|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(DeviceAssociation) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(DeviceAssociation) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(DeviceAssociation) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(DeviceAssociation) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(DeviceAssociation) });
        // }
        // Constraint: ele-1
