...
public class Medication : DomainResource
{
    ...
    /// available. Usage note: This could be a standard medication code such as a code from RxNorm, SNOMED
    /// CT, IDMP etc. It could also be a national or local formulary code, optionally with translations to
    /// other code systems.
    /// </summary>
    ...
    /// authority, to market a medicine in one or more jurisdictions. Typically abbreviated MAH.Note: The
    /// MAH may manufacture the product and may also contract the manufacturing of the product to one or
    /// more companies (organizations).
    /// </summary>
    [FhirElement("marketingAuthorizationHolder", Order = 21)]
    [Cardinality(0, 1)]
    [JsonPropertyName("marketingAuthorizationHolder")]
    public Reference<Organization> MarketingAuthorizationHolder { get; set; }

    /// <summary>
    /// Describes the form of the item. Powder; tablets; capsule.
    /// </summary>
    [FhirElement("doseForm", Order = 22)]
    [Cardinality(0, 1)]
    [JsonPropertyName("doseForm")]
    public CodeableConcept DoseForm { get; set; }

    /// <summary>
    /// When the specified product code does not infer a package size, this is the specific amount of drug
    /// in the product. For example, when specifying a product that has the same strength (For example,
    /// Insulin glargine 100 unit per mL solution for injection), this attribute provides additional
    /// clarification of the package amount (For example, 3 mL, 10mL, etc.).
    /// </summary>
    [FhirElement("totalVolume", Order = 23)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("definition", Order = 26)]
    [Cardinality(0, 1)]
    [JsonPropertyName("definition")]
    public Reference<MedicationKnowledge> Definition { get; set; }
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        {
            yield return new ValidationResult("Element 'Medication' cardinality must be 0..*", new[] { nameof(Medication) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'Medication.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Medication.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Medication.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'Medication.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        // Validate Ingredient cardinality
        var ingredientCount = Ingredient?.Count ?? 0;
        if (ingredientCount < 0)
        {
            yield return new ValidationResult("Element 'Medication.ingredient' cardinality must be 0..*", new[] { nameof(Ingredient) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Medication.ingredient.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Medication.ingredient.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Item == null)
        {
            yield return new ValidationResult("Element 'Medication.ingredient.item' cardinality must be 1..1", new[] { nameof(Item) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Medication.batch.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Medication.batch.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        ...
        if (Status != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/medication-status|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(Medication) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(Medication) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(Medication) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(Medication) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(Medication) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(MarketingAuthorizationHolder) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(TotalVolume) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(IsActive) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Batch) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
