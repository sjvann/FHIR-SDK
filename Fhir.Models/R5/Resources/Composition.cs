...
public class Composition : DomainResource
{
    ...
    /// <summary>
    /// An absolute URI that is used to identify this Composition when it is referenced in a specification,
    /// model, design or an instance; also called its canonical identifier. This SHOULD be globally unique
    /// and SHOULD be a literal address at which an authoritative instance of this Composition is (or will
    /// be) published. This URL can be the target of a canonical reference. It SHALL remain the same when
    /// the Composition is stored on different servers.
    /// </summary>
    ...
    /// <summary>
    /// The workflow/clinical status of this composition. The status is a marker for the clinical standing
    /// of the document.
    /// </summary>
    ...
    /// <summary>
    /// Specifies the particular kind of composition (e.g. History and Physical, Discharge Summary, Progress
    /// Note). This usually equates to the purpose of making the composition.
    /// </summary>
    ...
    /// <summary>
    /// A categorization for the type of the composition - helps for indexing and searching. This may be
    /// implied by or derived from the code specified in the Composition Type.
    /// </summary>
    ...
    /// herd of livestock, or a set of patients that share a common exposure).
    /// </summary>
    ...
    /// appropriate Composition instances.
    /// </summary>
    [FhirElement("useContext", Order = 27)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("useContext")]
    public List<UsageContext>? UseContext { get; set; }

    /// <summary>
    /// Identifies who is responsible for the information in the composition, not necessarily who typed it
    /// in.
    /// </summary>
    [FhirElement("author", Order = 28)]
    ...
    /// </summary>
    [FhirElement("custodian", Order = 33)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("relatesTo", Order = 34)]
    [Cardinality(0, int.MaxValue)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate Composition cardinality
        var compositionCount = Composition?.Count ?? 0;
        if (compositionCount < 0)
        {
            yield return new ValidationResult("Element 'Composition' cardinality must be 0..*", new[] { nameof(Composition) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'Composition.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Composition.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Composition.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'Composition.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'Composition.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        if (Type == null)
        {
            yield return new ValidationResult("Element 'Composition.type' cardinality must be 1..1", new[] { nameof(Type) });
        }
        // Validate Category cardinality
        var categoryCount = Category?.Count ?? 0;
        if (categoryCount < 0)
        {
            yield return new ValidationResult("Element 'Composition.category' cardinality must be 0..*", new[] { nameof(Category) });
        }
        // Validate Subject cardinality
        var subjectCount = Subject?.Count ?? 0;
        if (subjectCount < 0)
        {
            yield return new ValidationResult("Element 'Composition.subject' cardinality must be 0..*", new[] { nameof(Subject) });
        }
        if (Date == null)
        {
            yield return new ValidationResult("Element 'Composition.date' cardinality must be 1..1", new[] { nameof(Date) });
        }
        // Validate UseContext cardinality
        var usecontextCount = UseContext?.Count ?? 0;
        if (usecontextCount < 0)
        {
            yield return new ValidationResult("Element 'Composition.useContext' cardinality must be 0..*", new[] { nameof(UseContext) });
        }
        // Validate Author cardinality
        var authorCount = Author?.Count ?? 0;
        if (authorCount < 1)
        {
            yield return new ValidationResult("Element 'Composition.author' cardinality must be 1..*", new[] { nameof(Author) });
        }
        if (Title == null)
        {
            yield return new ValidationResult("Element 'Composition.title' cardinality must be 1..1", new[] { nameof(Title) });
        }
        // Validate Note cardinality
        var noteCount = Note?.Count ?? 0;
        if (noteCount < 0)
        {
            yield return new ValidationResult("Element 'Composition.note' cardinality must be 0..*", new[] { nameof(Note) });
        }
        // Validate Attester cardinality
        var attesterCount = Attester?.Count ?? 0;
        if (attesterCount < 0)
        {
            yield return new ValidationResult("Element 'Composition.attester' cardinality must be 0..*", new[] { nameof(Attester) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Composition.attester.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Composition.attester.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Mode == null)
        {
            yield return new ValidationResult("Element 'Composition.attester.mode' cardinality must be 1..1", new[] { nameof(Mode) });
        }
        // Validate RelatesTo cardinality
        var relatestoCount = RelatesTo?.Count ?? 0;
        if (relatestoCount < 0)
        {
            yield return new ValidationResult("Element 'Composition.relatesTo' cardinality must be 0..*", new[] { nameof(RelatesTo) });
        }
        // Validate Event cardinality
        var eventCount = Event?.Count ?? 0;
        if (eventCount < 0)
        {
            yield return new ValidationResult("Element 'Composition.event' cardinality must be 0..*", new[] { nameof(Event) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Composition.event.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Composition.event.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Detail cardinality
        var detailCount = Detail?.Count ?? 0;
        if (detailCount < 0)
        {
            yield return new ValidationResult("Element 'Composition.event.detail' cardinality must be 0..*", new[] { nameof(Detail) });
        }
        // Validate Section cardinality
        var sectionCount = Section?.Count ?? 0;
        if (sectionCount < 0)
        {
            yield return new ValidationResult("Element 'Composition.section' cardinality must be 0..*", new[] { nameof(Section) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Composition.section.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Composition.section.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Author cardinality
        var authorCount = Author?.Count ?? 0;
        if (authorCount < 0)
        {
            yield return new ValidationResult("Element 'Composition.section.author' cardinality must be 0..*", new[] { nameof(Author) });
        }
        // Validate Entry cardinality
        var entryCount = Entry?.Count ?? 0;
        if (entryCount < 0)
        {
            yield return new ValidationResult("Element 'Composition.section.entry' cardinality must be 0..*", new[] { nameof(Entry) });
        }
        // Validate Section cardinality
        var sectionCount = Section?.Count ?? 0;
        if (sectionCount < 0)
        {
            yield return new ValidationResult("Element 'Composition.section.section' cardinality must be 0..*", new[] { nameof(Section) });
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(Composition) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(Composition) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(Composition) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(Composition) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(Composition) });
        // }
        // Constraint: ele-1
        ...
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("text.exists() or entry.exists() or section.exists()"))
        // {
        //     yield return new ValidationResult("A section must contain at least one of text, entries, or sub-sections", new[] { nameof(Section) });
        // }
        // Constraint: cmp-2
        ...
        // {
        //     yield return new ValidationResult("A section can only have an emptyReason if it is empty", new[] { nameof(Section) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
