...
public class OperationOutcome : DomainResource
{
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate OperationOutcome cardinality
        var operationoutcomeCount = OperationOutcome?.Count ?? 0;
        if (operationoutcomeCount < 0)
        {
            yield return new ValidationResult("Element 'OperationOutcome' cardinality must be 0..*", new[] { nameof(OperationOutcome) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'OperationOutcome.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'OperationOutcome.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'OperationOutcome.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Issue cardinality
        var issueCount = Issue?.Count ?? 0;
        if (issueCount < 1)
        {
            yield return new ValidationResult("Element 'OperationOutcome.issue' cardinality must be 1..*", new[] { nameof(Issue) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'OperationOutcome.issue.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'OperationOutcome.issue.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Severity == null)
        {
            yield return new ValidationResult("Element 'OperationOutcome.issue.severity' cardinality must be 1..1", new[] { nameof(Severity) });
        }
        if (Code == null)
        {
            yield return new ValidationResult("Element 'OperationOutcome.issue.code' cardinality must be 1..1", new[] { nameof(Code) });
        }
        // Validate Location cardinality
        var locationCount = Location?.Count ?? 0;
        if (locationCount < 0)
        {
            yield return new ValidationResult("Element 'OperationOutcome.issue.location' cardinality must be 0..*", new[] { nameof(Location) });
        }
        // Validate Expression cardinality
        var expressionCount = Expression?.Count ?? 0;
        if (expressionCount < 0)
        {
            yield return new ValidationResult("Element 'OperationOutcome.issue.expression' cardinality must be 0..*", new[] { nameof(Expression) });
        }
        ...
        if (Severity != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/issue-severity|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate Code ValueSet binding
        if (Code != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/issue-type|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(OperationOutcome) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(OperationOutcome) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(OperationOutcome) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(OperationOutcome) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(OperationOutcome) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Diagnostics) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
