...
public class Measure : DomainResource
{
    ...
    /// and SHOULD be a literal address at which an authoritative instance of this measure is (or will be)
    ...
    /// specification, model, design or instance. This is an arbitrary value managed by the measure author
    ...
    /// specification, use the format Major.Minor.Revision (e.g. 1.0.0). For more information on versioning
    /// knowledge assets, refer to the Decision Support Service specification. Note that a version is
    ...
    /// <summary>
    /// A natural language name identifying the measure. This name should be usable as an identifier for the
    ...
    /// assumed, but the subject of the measure can be anything. (as CodeableConcept)
    /// </summary>
    ...
    /// <summary>
    /// The intended subjects for the measure. If this element is not provided, a Patient subject is
    /// assumed, but the subject of the measure can be anything. (as Reference)
    /// </summary>
    ...
    /// <summary>
    /// The population basis specifies the type of elements in the population. For a subject-based measure,
    /// this is boolean (because the subject and the population basis are the same, and the population
    /// criteria define yes/no values for each individual in the population). For measures that have a
    /// population basis that is different than the subject, this element specifies the type of the
    /// population basis. For example, an encounter-based measure has a subject of Patient and a population
    /// basis of Encounter, and the population criteria all return lists of Encounters.
    /// </summary>
    [FhirElement("basis", Order = 30)]
    [Cardinality(0, 1)]
    [JsonPropertyName("basis")]
    public FhirCode? Basis { get; set; }

    /// <summary>
    /// The date (and optionally time) when the measure was last significantly changed. The date must change
    ...
    /// <summary>
    /// Descriptive topics related to the content of the measure. Topics provide a high-level categorization
    /// grouping types of measures that can be useful for filtering and searching.
    /// </summary>
    ...
    /// <summary>
    /// Notices and disclaimers regarding the use of the measure or related to intellectual property (such
    /// as code systems) referenced by the measure.
    /// </summary>
    [FhirElement("disclaimer", Order = 51)]
    [Cardinality(0, 1)]
    ...
    /// continuous-variable, and cohort. The value set is extensible, allowing additional measure scoring
    /// types to be represented.
    /// </summary>
    [FhirElement("scoring", Order = 52)]
    [Cardinality(0, 1)]
    [JsonPropertyName("scoring")]
    public CodeableConcept Scoring { get; set; }

    /// <summary>
    /// Defines the expected units of measure for the measure score. This element SHOULD be specified as a
    /// UCUM unit.
    /// </summary>
    [FhirElement("scoringUnit", Order = 53)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("compositeScoring", Order = 54)]
    [Cardinality(0, 1)]
    ...
    /// how they may be accounted for when computing and reporting measure results.
    /// </summary>
    [FhirElement("riskAdjustment", Order = 56)]
    [Cardinality(0, 1)]
    [JsonPropertyName("riskAdjustment")]
    public FhirMarkdown? RiskAdjustment { get; set; }

    /// <summary>
    /// Describes how to combine the information calculated, based on logic in each of several populations,
    /// into one summarized result.
    /// </summary>
    [FhirElement("rateAggregation", Order = 57)]
    [Cardinality(0, 1)]
    [JsonPropertyName("rateAggregation")]
    public FhirMarkdown? RateAggregation { get; set; }

    /// <summary>
    /// Provides a succinct statement of the need for the measure. Usually includes statements pertaining to
    /// importance criterion: impact, gap in care, and evidence.
    /// </summary>
    [FhirElement("rationale", Order = 58)]
    [Cardinality(0, 1)]
    [JsonPropertyName("rationale")]
    public FhirMarkdown? Rationale { get; set; }

    /// <summary>
    /// Provides a summary of relevant clinical guidelines or other clinical recommendations supporting the
    /// measure.
    /// </summary>
    [FhirElement("clinicalRecommendationStatement", Order = 59)]
    [Cardinality(0, 1)]
    ...
    /// range).
    /// </summary>
    [FhirElement("improvementNotation", Order = 60)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("supplementalData", Order = 64)]
    [Cardinality(0, int.MaxValue)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate Measure cardinality
        var measureCount = Measure?.Count ?? 0;
        if (measureCount < 0)
        {
            yield return new ValidationResult("Element 'Measure' cardinality must be 0..*", new[] { nameof(Measure) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'Measure.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        // Validate Contact cardinality
        var contactCount = Contact?.Count ?? 0;
        if (contactCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.contact' cardinality must be 0..*", new[] { nameof(Contact) });
        }
        // Validate UseContext cardinality
        var usecontextCount = UseContext?.Count ?? 0;
        if (usecontextCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.useContext' cardinality must be 0..*", new[] { nameof(UseContext) });
        }
        // Validate Jurisdiction cardinality
        var jurisdictionCount = Jurisdiction?.Count ?? 0;
        if (jurisdictionCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.jurisdiction' cardinality must be 0..*", new[] { nameof(Jurisdiction) });
        }
        // Validate Topic cardinality
        var topicCount = Topic?.Count ?? 0;
        if (topicCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.topic' cardinality must be 0..*", new[] { nameof(Topic) });
        }
        // Validate Author cardinality
        var authorCount = Author?.Count ?? 0;
        if (authorCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.author' cardinality must be 0..*", new[] { nameof(Author) });
        }
        // Validate Editor cardinality
        var editorCount = Editor?.Count ?? 0;
        if (editorCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.editor' cardinality must be 0..*", new[] { nameof(Editor) });
        }
        // Validate Reviewer cardinality
        var reviewerCount = Reviewer?.Count ?? 0;
        if (reviewerCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.reviewer' cardinality must be 0..*", new[] { nameof(Reviewer) });
        }
        // Validate Endorser cardinality
        var endorserCount = Endorser?.Count ?? 0;
        if (endorserCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.endorser' cardinality must be 0..*", new[] { nameof(Endorser) });
        }
        // Validate RelatedArtifact cardinality
        var relatedartifactCount = RelatedArtifact?.Count ?? 0;
        if (relatedartifactCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.relatedArtifact' cardinality must be 0..*", new[] { nameof(RelatedArtifact) });
        }
        // Validate Library cardinality
        var libraryCount = Library?.Count ?? 0;
        if (libraryCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.library' cardinality must be 0..*", new[] { nameof(Library) });
        }
        // Validate Type cardinality
        var typeCount = Type?.Count ?? 0;
        if (typeCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.type' cardinality must be 0..*", new[] { nameof(Type) });
        }
        // Validate Term cardinality
        var termCount = Term?.Count ?? 0;
        if (termCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.term' cardinality must be 0..*", new[] { nameof(Term) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.term.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.term.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Group cardinality
        var groupCount = Group?.Count ?? 0;
        if (groupCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.group' cardinality must be 0..*", new[] { nameof(Group) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.group.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.group.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Type cardinality
        var typeCount = Type?.Count ?? 0;
        if (typeCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.group.type' cardinality must be 0..*", new[] { nameof(Type) });
        }
        // Validate Library cardinality
        var libraryCount = Library?.Count ?? 0;
        if (libraryCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.group.library' cardinality must be 0..*", new[] { nameof(Library) });
        }
        // Validate Population cardinality
        var populationCount = Population?.Count ?? 0;
        if (populationCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.group.population' cardinality must be 0..*", new[] { nameof(Population) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.group.population.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.group.population.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Stratifier cardinality
        var stratifierCount = Stratifier?.Count ?? 0;
        if (stratifierCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.group.stratifier' cardinality must be 0..*", new[] { nameof(Stratifier) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.group.stratifier.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.group.stratifier.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Component cardinality
        var componentCount = Component?.Count ?? 0;
        if (componentCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.group.stratifier.component' cardinality must be 0..*", new[] { nameof(Component) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.group.stratifier.component.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.group.stratifier.component.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate SupplementalData cardinality
        var supplementaldataCount = SupplementalData?.Count ?? 0;
        if (supplementaldataCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.supplementalData' cardinality must be 0..*", new[] { nameof(SupplementalData) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.supplementalData.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.supplementalData.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Usage cardinality
        var usageCount = Usage?.Count ?? 0;
        if (usageCount < 0)
        {
            yield return new ValidationResult("Element 'Measure.supplementalData.usage' cardinality must be 0..*", new[] { nameof(Usage) });
        }
        if (Criteria == null)
        {
            yield return new ValidationResult("Element 'Measure.supplementalData.criteria' cardinality must be 1..1", new[] { nameof(Criteria) });
        }
        ...
        // Validate Basis ValueSet binding
        if (Basis != null)
        {
        ...
        // Validate Scoring ValueSet binding
        if (Scoring != null)
        {
            // TODO: Implement ValueSet validation for http://terminology.hl7.org/ValueSet/measure-scoring
            // This requires a terminology service or local ValueSet cache
        }
        // Validate CompositeScoring ValueSet binding
        if (CompositeScoring != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/composite-measure-scoring
            // This requires a terminology service or local ValueSet cache
        }
        // Validate Type ValueSet binding
        if (Type != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/measure-type
            // This requires a terminology service or local ValueSet cache
        }
        // Validate ImprovementNotation ValueSet binding
        if (ImprovementNotation != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/measure-improvement-notation|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate Type ValueSet binding
        if (Type != null)
        ...
        if (Code != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/measure-population
            // This requires a terminology service or local ValueSet cache
        }
        // Validate AggregateMethod ValueSet binding
        if (AggregateMethod != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/measure-aggregate-method
            // This requires a terminology service or local ValueSet cache
        }
        // Validate Usage ValueSet binding
        if (Usage != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/measure-data-usage
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("Name should be usable as an identifier for the module by machine processing applications such as code generation", new[] { nameof(Measure) });
        // }
        // Constraint: mea-1
        // Expression: group.stratifier.all((code | description | criteria).exists() xor component.exists())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("group.stratifier.all((code | description | criteria).exists() xor component.exists())"))
        // {
        //     yield return new ValidationResult("Stratifier SHALL be either a single criteria or a set of criteria components", new[] { nameof(Measure) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Disclaimer) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Scoring) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(ScoringUnit) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(CompositeScoring) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(RiskAdjustment) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(RateAggregation) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Rationale) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(ClinicalRecommendationStatement) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(ImprovementNotation) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Guidance) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("$this.length() <= 255"))
        // {
        //     yield return new ValidationResult("Link ids should be 255 characters or less", new[] { nameof(LinkId) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Population) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Criteria) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(GroupDefinition) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(InputPopulationId) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(AggregateMethod) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Stratifier) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(SupplementalData) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
