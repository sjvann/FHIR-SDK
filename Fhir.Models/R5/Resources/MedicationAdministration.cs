...
/// practitioner. This event can also be used to record waste using a status of not-done and the
...
public class MedicationAdministration : DomainResource
{
    ...
    /// They are business identifiers assigned to this resource by the performer or other systems and remain
    ...
    /// </summary>
    [FhirElement("statusReason", Order = 22)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("statusReason")]
    public List<CodeableConcept>? StatusReason { get; set; }

    /// <summary>
    /// The type of medication administration (for example, drug classification like ATC, where meds would
    /// be administered, legal category of the medication).
    /// </summary>
    [FhirElement("category", Order = 23)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// the details of the medication or a simple attribute carrying a code that identifies the medication
    /// from a known list of medications.
    /// </summary>
    [FhirElement("medication", Order = 24)]
    [Cardinality(1, 1)]
    [Required]
    ...
    /// <summary>
    /// The visit, admission, or other contact between patient and health care provider during which the
    ...
    /// the medication. This attribute can be used to provide documentation of specific characteristics of
    /// the patient present at the time of administration. For example, if the dose says &quot;give
    /// &quot;x&quot; if the heartrate exceeds &quot;y&quot;&quot;, then the heart rate can be included
    /// using this attribute.
    /// </summary>
    [FhirElement("supportingInformation", Order = 27)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("supportingInformation")]
    public List<Reference<Resource>>? SupportingInformation { get; set; }

    /// <summary>
    /// A specific date/time or interval of time during which the administration took place (or did not take
    /// place). For many administrations, such as swallowing a tablet the use of dateTime is more
    /// appropriate. (as dateTime)
    /// </summary>
    [FhirElement("occurenceDateTime", Order = 28)]
    [Cardinality(1, 1)]
    [ChoiceType("occurence", "dateTime")]
    [Required]
    ...
    /// A specific date/time or interval of time during which the administration took place (or did not take
    /// place). For many administrations, such as swallowing a tablet the use of dateTime is more
    /// appropriate. (as Period)
    /// </summary>
    [FhirElement("occurencePeriod", Order = 29)]
    [Cardinality(1, 1)]
    [ChoiceType("occurence", "period")]
    [Required]
    ...
    /// A specific date/time or interval of time during which the administration took place (or did not take
    /// place). For many administrations, such as swallowing a tablet the use of dateTime is more
    /// appropriate. (as Timing)
    /// </summary>
    [FhirElement("occurenceTiming", Order = 30)]
    [Cardinality(1, 1)]
    [ChoiceType("occurence", "timing")]
    [Required]
    ...
    /// </summary>
    [FhirElement("isSubPotent", Order = 32)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("subPotentReason", Order = 33)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("performer", Order = 34)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("request", Order = 36)]
    [Cardinality(0, 1)]
    [JsonPropertyName("request")]
    public Reference<MedicationRequest> Request { get; set; }

    /// <summary>
    /// The device that is to be used for the administration of the medication (for example, PCA Pump).
    /// </summary>
    [FhirElement("device", Order = 37)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// verified.
    /// </summary>
    [FhirElement("eventHistory", Order = 40)]
    [Cardinality(0, int.MaxValue)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Choice Type validation for occurence[x]
        var occurenceProperties = new[] { nameof(OccurenceDateTime), nameof(OccurencePeriod), nameof(OccurenceTiming) };
        var occurenceSetCount = occurenceProperties.Count(prop => 
            GetType().GetProperty(prop)?.GetValue(this) != null);

        if (occurenceSetCount > 1)
        {
            yield return new ValidationResult(
                "Only one of OccurenceDateTime, OccurencePeriod, OccurenceTiming may be specified",
                new[] { nameof(OccurenceDateTime), nameof(OccurencePeriod), nameof(OccurenceTiming) });
        }
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate MedicationAdministration cardinality
        var medicationadministrationCount = MedicationAdministration?.Count ?? 0;
        if (medicationadministrationCount < 0)
        {
            yield return new ValidationResult("Element 'MedicationAdministration' cardinality must be 0..*", new[] { nameof(MedicationAdministration) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        // Validate BasedOn cardinality
        var basedonCount = BasedOn?.Count ?? 0;
        if (basedonCount < 0)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.basedOn' cardinality must be 0..*", new[] { nameof(BasedOn) });
        }
        // Validate PartOf cardinality
        var partofCount = PartOf?.Count ?? 0;
        if (partofCount < 0)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.partOf' cardinality must be 0..*", new[] { nameof(PartOf) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        // Validate StatusReason cardinality
        var statusreasonCount = StatusReason?.Count ?? 0;
        if (statusreasonCount < 0)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.statusReason' cardinality must be 0..*", new[] { nameof(StatusReason) });
        }
        // Validate Category cardinality
        var categoryCount = Category?.Count ?? 0;
        if (categoryCount < 0)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.category' cardinality must be 0..*", new[] { nameof(Category) });
        }
        if (Medication == null)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.medication' cardinality must be 1..1", new[] { nameof(Medication) });
        }
        if (Subject == null)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.subject' cardinality must be 1..1", new[] { nameof(Subject) });
        }
        ...
        {
            yield return new ValidationResult("Element 'MedicationAdministration.supportingInformation' cardinality must be 0..*", new[] { nameof(SupportingInformation) });
        }
        if (Occurence == null)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.occurence[x]' cardinality must be 1..1", new[] { nameof(Occurence) });
        }
        // Validate SubPotentReason cardinality
        var subpotentreasonCount = SubPotentReason?.Count ?? 0;
        if (subpotentreasonCount < 0)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.subPotentReason' cardinality must be 0..*", new[] { nameof(SubPotentReason) });
        }
        // Validate Performer cardinality
        var performerCount = Performer?.Count ?? 0;
        if (performerCount < 0)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.performer' cardinality must be 0..*", new[] { nameof(Performer) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.performer.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.performer.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Actor == null)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.performer.actor' cardinality must be 1..1", new[] { nameof(Actor) });
        }
        // Validate Reason cardinality
        var reasonCount = Reason?.Count ?? 0;
        if (reasonCount < 0)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.reason' cardinality must be 0..*", new[] { nameof(Reason) });
        }
        // Validate Device cardinality
        var deviceCount = Device?.Count ?? 0;
        if (deviceCount < 0)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.device' cardinality must be 0..*", new[] { nameof(Device) });
        }
        // Validate Note cardinality
        var noteCount = Note?.Count ?? 0;
        if (noteCount < 0)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.note' cardinality must be 0..*", new[] { nameof(Note) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.dosage.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.dosage.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate EventHistory cardinality
        var eventhistoryCount = EventHistory?.Count ?? 0;
        if (eventhistoryCount < 0)
        {
            yield return new ValidationResult("Element 'MedicationAdministration.eventHistory' cardinality must be 0..*", new[] { nameof(EventHistory) });
        }
        ...
        if (Status != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/medication-admin-status|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(MedicationAdministration) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(MedicationAdministration) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(MedicationAdministration) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(MedicationAdministration) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(MedicationAdministration) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Occurence) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("(dose.exists() or rate.exists() or text.exists())"))
        // {
        //     yield return new ValidationResult("If dosage attribute is present then SHALL have at least one of dosage.text or dosage.dose or dosage.rate[x]", new[] { nameof(Dosage) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
