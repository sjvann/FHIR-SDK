...
/// <summary>
/// Represents a request a device to be provided to a specific patient. The device may be an implantable
/// device to be subsequently implanted, or an external assistive device, such as a walker, to be
/// delivered and subsequently be used.
/// </summary>
public class DeviceRequest : DomainResource
{
    ...
    public List<Reference<DeviceRequest>>? Replaces { get; set; }
    ...
    /// </summary>
    [FhirElement("groupIdentifier", Order = 23)]
    [Cardinality(0, 1)]
    ...
    /// <summary>
    /// If true, indicates that the provider is asking for the patient to either stop using or to not start
    /// using the specified device or category of devices. For example, the patient has undergone surgery
    /// and the provider is indicating that the patient should not wear contact lenses.
    /// </summary>
    ...
    /// </summary>
    [FhirElement("parameter", Order = 30)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// expressions, for example. &quot;Every 8 hours&quot;; &quot;Three times a day&quot;; &quot;1/2 an
    /// hour before breakfast for 10 days from 23-Dec 2011:&quot;; &quot;15 Oct 2013, 17 Oct 2013 and 1 Nov
    /// 2013&quot;. (as dateTime)
    /// </summary>
    ...
    /// </summary>
    [FhirElement("authoredOn", Order = 36)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("requester", Order = 37)]
    [Cardinality(0, 1)]
    [JsonPropertyName("requester")]
    public Reference<Device> Requester { get; set; }

    /// <summary>
    /// The desired individual or entity to provide the device to the subject of the request (e.g., patient,
    /// location).
    /// </summary>
    ...
    /// </summary>
    [FhirElement("asNeededFor", Order = 41)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("insurance", Order = 42)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// site).
    /// </summary>
    [FhirElement("supportingInfo", Order = 43)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// provided in a class. These may include for example a comment, an instruction, or a note associated
    /// with the statement.
    /// </summary>
    ...
    /// </summary>
    [FhirElement("relevantHistory", Order = 45)]
    [Cardinality(0, int.MaxValue)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate DeviceRequest cardinality
        var devicerequestCount = DeviceRequest?.Count ?? 0;
        if (devicerequestCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceRequest' cardinality must be 0..*", new[] { nameof(DeviceRequest) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceRequest.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceRequest.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceRequest.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceRequest.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        ...
        {
            yield return new ValidationResult("Element 'DeviceRequest.instantiatesCanonical' cardinality must be 0..*", new[] { nameof(InstantiatesCanonical) });
        }
        // Validate InstantiatesUri cardinality
        var instantiatesuriCount = InstantiatesUri?.Count ?? 0;
        if (instantiatesuriCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceRequest.instantiatesUri' cardinality must be 0..*", new[] { nameof(InstantiatesUri) });
        }
        // Validate BasedOn cardinality
        var basedonCount = BasedOn?.Count ?? 0;
        if (basedonCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceRequest.basedOn' cardinality must be 0..*", new[] { nameof(BasedOn) });
        }
        // Validate Replaces cardinality
        var replacesCount = Replaces?.Count ?? 0;
        if (replacesCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceRequest.replaces' cardinality must be 0..*", new[] { nameof(Replaces) });
        }
        if (Intent == null)
        {
            yield return new ValidationResult("Element 'DeviceRequest.intent' cardinality must be 1..1", new[] { nameof(Intent) });
        }
        if (Code == null)
        {
            yield return new ValidationResult("Element 'DeviceRequest.code' cardinality must be 1..1", new[] { nameof(Code) });
        }
        // Validate Parameter cardinality
        var parameterCount = Parameter?.Count ?? 0;
        if (parameterCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceRequest.parameter' cardinality must be 0..*", new[] { nameof(Parameter) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceRequest.parameter.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceRequest.parameter.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Subject == null)
        {
            yield return new ValidationResult("Element 'DeviceRequest.subject' cardinality must be 1..1", new[] { nameof(Subject) });
        }
        // Validate Reason cardinality
        var reasonCount = Reason?.Count ?? 0;
        if (reasonCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceRequest.reason' cardinality must be 0..*", new[] { nameof(Reason) });
        }
        // Validate Insurance cardinality
        var insuranceCount = Insurance?.Count ?? 0;
        if (insuranceCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceRequest.insurance' cardinality must be 0..*", new[] { nameof(Insurance) });
        }
        // Validate SupportingInfo cardinality
        var supportinginfoCount = SupportingInfo?.Count ?? 0;
        if (supportinginfoCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceRequest.supportingInfo' cardinality must be 0..*", new[] { nameof(SupportingInfo) });
        }
        // Validate Note cardinality
        var noteCount = Note?.Count ?? 0;
        if (noteCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceRequest.note' cardinality must be 0..*", new[] { nameof(Note) });
        }
        // Validate RelevantHistory cardinality
        var relevanthistoryCount = RelevantHistory?.Count ?? 0;
        if (relevanthistoryCount < 0)
        {
            yield return new ValidationResult("Element 'DeviceRequest.relevantHistory' cardinality must be 0..*", new[] { nameof(RelevantHistory) });
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(DeviceRequest) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(DeviceRequest) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(DeviceRequest) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(DeviceRequest) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(DeviceRequest) });
        // }
        // Constraint: ele-1
