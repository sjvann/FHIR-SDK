...
public class Schedule : DomainResource
{
    ...
    /// </summary>
    [FhirElement("serviceCategory", Order = 20)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("serviceType", Order = 21)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// appointment.
    /// </summary>
    [FhirElement("specialty", Order = 22)]
    [Cardinality(0, int.MaxValue)]
    ...
    public List<Reference<Patient>> Actor { get; set; }

    /// <summary>
    /// The period of time that the slots that reference this Schedule resource cover (even if none exist).
    /// These cover the amount of time that an organization&apos;s planning horizon; the interval for which
    /// they are currently accepting appointments. This does not define a &quot;template&quot; for planning
    /// outside these dates.
    /// </summary>
    [FhirElement("planningHorizon", Order = 25)]
    [Cardinality(0, 1)]
    [JsonPropertyName("planningHorizon")]
    public Period PlanningHorizon { get; set; }

    /// <summary>
    /// Comments on the availability to describe any extended information. Such as custom constraints on the
    /// slots that may be associated.
    /// </summary>
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        if (scheduleCount < 0)
        {
            yield return new ValidationResult("Element 'Schedule' cardinality must be 0..*", new[] { nameof(Schedule) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'Schedule.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Schedule.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Schedule.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'Schedule.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        // Validate ServiceCategory cardinality
        var servicecategoryCount = ServiceCategory?.Count ?? 0;
        if (servicecategoryCount < 0)
        {
            yield return new ValidationResult("Element 'Schedule.serviceCategory' cardinality must be 0..*", new[] { nameof(ServiceCategory) });
        }
        // Validate ServiceType cardinality
        var servicetypeCount = ServiceType?.Count ?? 0;
        if (servicetypeCount < 0)
        {
            yield return new ValidationResult("Element 'Schedule.serviceType' cardinality must be 0..*", new[] { nameof(ServiceType) });
        }
        // Validate Specialty cardinality
        var specialtyCount = Specialty?.Count ?? 0;
        if (specialtyCount < 0)
        {
            yield return new ValidationResult("Element 'Schedule.specialty' cardinality must be 0..*", new[] { nameof(Specialty) });
        }
        // Validate Actor cardinality
        var actorCount = Actor?.Count ?? 0;
        if (actorCount < 1)
        {
            yield return new ValidationResult("Element 'Schedule.actor' cardinality must be 1..*", new[] { nameof(Actor) });
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(Schedule) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(Schedule) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(Schedule) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(Schedule) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(Schedule) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(PlanningHorizon) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
