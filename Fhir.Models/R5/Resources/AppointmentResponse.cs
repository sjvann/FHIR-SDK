...
public class AppointmentResponse : DomainResource
{
    ...
    /// business processes and/ or used to refer to it when a direct URL reference to the resource itself is
    /// not appropriate.
    /// </summary>
    ...
    /// </summary>
    [FhirElement("appointment", Order = 19)]
    [Cardinality(1, 1)]
    [Required]
    [JsonPropertyName("appointment")]
    public Reference<Appointment> Appointment { get; set; }
    ...
    /// </summary>
    [FhirElement("proposedNewTime", Order = 20)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("start", Order = 21)]
    [Cardinality(0, 1)]
    [JsonPropertyName("start")]
    public FhirInstant? Start { get; set; }

    /// <summary>
    /// This may be either the same as the appointment request to confirm the details of the appointment, or
    /// alternately a new time to request a re-negotiation of the end time.
    /// </summary>
    [FhirElement("end", Order = 22)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("participantType", Order = 23)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("actor", Order = 24)]
    [Cardinality(0, 1)]
    [JsonPropertyName("actor")]
    public Reference<Patient> Actor { get; set; }

    /// <summary>
    /// Participation status of the participant. When the status is declined or tentative if the start/end
    /// times are different to the appointment, then these times should be interpreted as a requested time
    /// change. When the status is accepted, the times can either be the time of the appointment (as a
    /// confirmation of the time) or can be empty.
    /// </summary>
    [FhirElement("participantStatus", Order = 25)]
    [Cardinality(1, 1)]
    [Required]
    ...
    /// </summary>
    [FhirElement("recurring", Order = 27)]
    [Cardinality(0, 1)]
    [JsonPropertyName("recurring")]
    public FhirBoolean? Recurring { get; set; }

    /// <summary>
    /// The original date within a recurring request. This could be used in place of the recurrenceId to be
    /// more direct (or where the template is provided through the simple list of dates in
    /// `Appointment.occurrenceDate`).
    /// </summary>
    [FhirElement("occurrenceDate", Order = 28)]
    [Cardinality(0, 1)]
    ...
    /// request.
    /// </summary>
    [FhirElement("recurrenceId", Order = 29)]
    [Cardinality(0, 1)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate AppointmentResponse cardinality
        var appointmentresponseCount = AppointmentResponse?.Count ?? 0;
        if (appointmentresponseCount < 0)
        {
            yield return new ValidationResult("Element 'AppointmentResponse' cardinality must be 0..*", new[] { nameof(AppointmentResponse) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'AppointmentResponse.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'AppointmentResponse.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'AppointmentResponse.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'AppointmentResponse.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Appointment == null)
        {
            yield return new ValidationResult("Element 'AppointmentResponse.appointment' cardinality must be 1..1", new[] { nameof(Appointment) });
        }
        // Validate ParticipantType cardinality
        var participanttypeCount = ParticipantType?.Count ?? 0;
        if (participanttypeCount < 0)
        {
            yield return new ValidationResult("Element 'AppointmentResponse.participantType' cardinality must be 0..*", new[] { nameof(ParticipantType) });
        }
        if (ParticipantStatus == null)
        {
            yield return new ValidationResult("Element 'AppointmentResponse.participantStatus' cardinality must be 1..1", new[] { nameof(ParticipantStatus) });
        }
        ...
        // Validate ParticipantType ValueSet binding
        if (ParticipantType != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/encounter-participant-type
            // This requires a terminology service or local ValueSet cache
        }
        // Validate ParticipantStatus ValueSet binding
        if (ParticipantStatus != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/appointmentresponse-status|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // if (!EvaluateFHIRPath("participantType.exists() or actor.exists()"))
        // {
        //     yield return new ValidationResult("Either the participantType or actor must be specified", new[] { nameof(AppointmentResponse) });
        // }
        // Constraint: dom-2
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(AppointmentResponse) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(AppointmentResponse) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(AppointmentResponse) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(AppointmentResponse) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(AppointmentResponse) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Appointment) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(ProposedNewTime) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(ParticipantType) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(ParticipantStatus) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Recurring) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
