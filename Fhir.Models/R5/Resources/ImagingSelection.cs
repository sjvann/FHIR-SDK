...
/// </summary>
public class ImagingSelection : DomainResource
{
    ...
    /// imaging selection is about and into whose or what record the imaging selection is placed.
    /// </summary>
    ...
    /// </summary>
    [FhirElement("performer", Order = 22)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("derivedFrom", Order = 27)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("derivedFrom")]
    public List<Reference<ImagingStudy>>? DerivedFrom { get; set; }
    ...
    /// </summary>
    [FhirElement("seriesUid", Order = 29)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("seriesNumber", Order = 30)]
    [Cardinality(0, 1)]
    [JsonPropertyName("seriesNumber")]
    public FhirUnsignedInt? SeriesNumber { get; set; }

    /// <summary>
    /// The Frame of Reference UID identifying the coordinate system that conveys spatial and/or temporal
    /// information for the selected images or frames.
    /// </summary>
    [FhirElement("frameOfReferenceUid", Order = 31)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("focus", Order = 33)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("focus")]
    public List<Reference<ImagingSelection>>? Focus { get; set; }

    /// <summary>
    /// Each imaging selection includes one or more selected DICOM SOP instances.
    /// </summary>
    [FhirElement("instance", Order = 34)]
    [Cardinality(0, int.MaxValue)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate ImagingSelection cardinality
        var imagingselectionCount = ImagingSelection?.Count ?? 0;
        if (imagingselectionCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection' cardinality must be 0..*", new[] { nameof(ImagingSelection) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'ImagingSelection.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        // Validate Performer cardinality
        var performerCount = Performer?.Count ?? 0;
        if (performerCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection.performer' cardinality must be 0..*", new[] { nameof(Performer) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection.performer.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection.performer.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate BasedOn cardinality
        var basedonCount = BasedOn?.Count ?? 0;
        if (basedonCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection.basedOn' cardinality must be 0..*", new[] { nameof(BasedOn) });
        }
        // Validate Category cardinality
        var categoryCount = Category?.Count ?? 0;
        if (categoryCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection.category' cardinality must be 0..*", new[] { nameof(Category) });
        }
        if (Code == null)
        {
            yield return new ValidationResult("Element 'ImagingSelection.code' cardinality must be 1..1", new[] { nameof(Code) });
        }
        // Validate DerivedFrom cardinality
        var derivedfromCount = DerivedFrom?.Count ?? 0;
        if (derivedfromCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection.derivedFrom' cardinality must be 0..*", new[] { nameof(DerivedFrom) });
        }
        // Validate Endpoint cardinality
        var endpointCount = Endpoint?.Count ?? 0;
        if (endpointCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection.endpoint' cardinality must be 0..*", new[] { nameof(Endpoint) });
        }
        // Validate Focus cardinality
        var focusCount = Focus?.Count ?? 0;
        if (focusCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection.focus' cardinality must be 0..*", new[] { nameof(Focus) });
        }
        // Validate Instance cardinality
        var instanceCount = Instance?.Count ?? 0;
        if (instanceCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection.instance' cardinality must be 0..*", new[] { nameof(Instance) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection.instance.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection.instance.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Uid == null)
        {
            yield return new ValidationResult("Element 'ImagingSelection.instance.uid' cardinality must be 1..1", new[] { nameof(Uid) });
        }
        // Validate Subset cardinality
        var subsetCount = Subset?.Count ?? 0;
        if (subsetCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection.instance.subset' cardinality must be 0..*", new[] { nameof(Subset) });
        }
        // Validate ImageRegion2D cardinality
        var imageregion2dCount = ImageRegion2D?.Count ?? 0;
        if (imageregion2dCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection.instance.imageRegion2D' cardinality must be 0..*", new[] { nameof(ImageRegion2D) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection.instance.imageRegion2D.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection.instance.imageRegion2D.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (RegionType == null)
        {
            yield return new ValidationResult("Element 'ImagingSelection.instance.imageRegion2D.regionType' cardinality must be 1..1", new[] { nameof(RegionType) });
        }
        // Validate Coordinate cardinality
        var coordinateCount = Coordinate?.Count ?? 0;
        if (coordinateCount < 1)
        {
            yield return new ValidationResult("Element 'ImagingSelection.instance.imageRegion2D.coordinate' cardinality must be 1..*", new[] { nameof(Coordinate) });
        }
        // Validate ImageRegion3D cardinality
        var imageregion3dCount = ImageRegion3D?.Count ?? 0;
        if (imageregion3dCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection.instance.imageRegion3D' cardinality must be 0..*", new[] { nameof(ImageRegion3D) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection.instance.imageRegion3D.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'ImagingSelection.instance.imageRegion3D.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (RegionType == null)
        {
            yield return new ValidationResult("Element 'ImagingSelection.instance.imageRegion3D.regionType' cardinality must be 1..1", new[] { nameof(RegionType) });
        }
        // Validate Coordinate cardinality
        var coordinateCount = Coordinate?.Count ?? 0;
        if (coordinateCount < 1)
        {
            yield return new ValidationResult("Element 'ImagingSelection.instance.imageRegion3D.coordinate' cardinality must be 1..*", new[] { nameof(Coordinate) });
        }
        ...
        if (Status != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/imagingselection-status|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate Function ValueSet binding
        if (Function != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/series-performer-function
            // This requires a terminology service or local ValueSet cache
        }
        // Validate SopClass ValueSet binding
        if (SopClass != null)
        {
            // TODO: Implement ValueSet validation for http://dicom.nema.org/medical/dicom/current/output/chtml/part04/sect_B.5.html#table_B.5-1
            // This requires a terminology service or local ValueSet cache
        }
        // Validate RegionType ValueSet binding
        if (RegionType != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/imagingselection-2dgraphictype|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate RegionType ValueSet binding
        if (RegionType != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/imagingselection-3dgraphictype|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(ImagingSelection) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(ImagingSelection) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(ImagingSelection) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(ImagingSelection) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(ImagingSelection) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(StudyUid) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(SeriesUid) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(SeriesNumber) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(FrameOfReferenceUid) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Uid) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(SopClass) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Subset) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(ImageRegion2D) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(RegionType) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Coordinate) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(ImageRegion3D) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
