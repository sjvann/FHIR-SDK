...
/// <summary>
/// A ResearchSubject is a participant or object which is the recipient of investigative activities in a
/// research study.
/// </summary>
public class ResearchSubject : DomainResource
{
    ...
    /// </summary>
    [FhirElement("progress", Order = 20)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("progress")]
    public List<BackboneElement>? Progress { get; set; }
    ...
    /// </summary>
    [FhirElement("study", Order = 22)]
    [Cardinality(1, 1)]
    [Required]
    [JsonPropertyName("study")]
    public Reference<ResearchStudy> Study { get; set; }
    ...
    /// </summary>
    [FhirElement("assignedComparisonGroup", Order = 24)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("actualComparisonGroup", Order = 25)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("consent", Order = 26)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("consent")]
    public List<Reference<Consent>>? Consent { get; set; }
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate ResearchSubject cardinality
        var researchsubjectCount = ResearchSubject?.Count ?? 0;
        if (researchsubjectCount < 0)
        {
            yield return new ValidationResult("Element 'ResearchSubject' cardinality must be 0..*", new[] { nameof(ResearchSubject) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'ResearchSubject.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'ResearchSubject.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'ResearchSubject.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'ResearchSubject.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'ResearchSubject.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        // Validate Progress cardinality
        var progressCount = Progress?.Count ?? 0;
        if (progressCount < 0)
        {
            yield return new ValidationResult("Element 'ResearchSubject.progress' cardinality must be 0..*", new[] { nameof(Progress) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'ResearchSubject.progress.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'ResearchSubject.progress.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Study == null)
        {
            yield return new ValidationResult("Element 'ResearchSubject.study' cardinality must be 1..1", new[] { nameof(Study) });
        }
        if (Subject == null)
        {
            yield return new ValidationResult("Element 'ResearchSubject.subject' cardinality must be 1..1", new[] { nameof(Subject) });
        }
        // Validate Consent cardinality
        var consentCount = Consent?.Count ?? 0;
        if (consentCount < 0)
        {
            yield return new ValidationResult("Element 'ResearchSubject.consent' cardinality must be 0..*", new[] { nameof(Consent) });
        }
        ...
        // Validate SubjectState ValueSet binding
        if (SubjectState != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/research-subject-state|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(ResearchSubject) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(ResearchSubject) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(ResearchSubject) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(ResearchSubject) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(ResearchSubject) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(SubjectState) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Milestone) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(EndDate) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(AssignedComparisonGroup) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(ActualComparisonGroup) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Consent) });
        // }
    }
