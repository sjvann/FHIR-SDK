...
public class QuestionnaireResponse : DomainResource
{
    ...
    /// <summary>
    /// Business identifiers assigned to this questionnaire response by the performer and/or other systems.
    ...
    /// <summary>
    /// A plan, proposal or order that is fulfilled in whole or in part by this questionnaire response. For
    /// example, a ServiceRequest seeking an intake assessment or a decision support recommendation to
    ...
    public List<Reference<Observation>>? PartOf { get; set; }

    /// <summary>
    /// The Questionnaire that defines and organizes the questions for which answers are being provided.
    /// </summary>
    [FhirElement("questionnaire", Order = 21)]
    [Cardinality(1, 1)]
    [Required]
    ...
    /// <summary>
    /// The Encounter during which this questionnaire response was created or to which the creation of this
    ...
    /// <summary>
    /// The date and/or time that this questionnaire response was last modified by the user - e.g. changing
    /// answers or revising status.
    /// </summary>
    [FhirElement("authored", Order = 25)]
    [Cardinality(0, 1)]
    [JsonPropertyName("authored")]
    public FhirDateTime? Authored { get; set; }

    /// <summary>
    /// The individual or device that received the answers to the questions in the QuestionnaireResponse and
    /// recorded them in the system.
    /// </summary>
    [FhirElement("author", Order = 26)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("source", Order = 27)]
    [Cardinality(0, 1)]
    [JsonPropertyName("source")]
    public Reference<Device> Source { get; set; }

    /// <summary>
    /// A group or question item from the original questionnaire for which answers are provided.
    /// </summary>
    [FhirElement("item", Order = 28)]
    [Cardinality(0, int.MaxValue)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate QuestionnaireResponse cardinality
        var questionnaireresponseCount = QuestionnaireResponse?.Count ?? 0;
        if (questionnaireresponseCount < 0)
        {
            yield return new ValidationResult("Element 'QuestionnaireResponse' cardinality must be 0..*", new[] { nameof(QuestionnaireResponse) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'QuestionnaireResponse.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'QuestionnaireResponse.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'QuestionnaireResponse.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'QuestionnaireResponse.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        // Validate BasedOn cardinality
        var basedonCount = BasedOn?.Count ?? 0;
        if (basedonCount < 0)
        {
            yield return new ValidationResult("Element 'QuestionnaireResponse.basedOn' cardinality must be 0..*", new[] { nameof(BasedOn) });
        }
        // Validate PartOf cardinality
        var partofCount = PartOf?.Count ?? 0;
        if (partofCount < 0)
        {
            yield return new ValidationResult("Element 'QuestionnaireResponse.partOf' cardinality must be 0..*", new[] { nameof(PartOf) });
        }
        if (Questionnaire == null)
        {
            yield return new ValidationResult("Element 'QuestionnaireResponse.questionnaire' cardinality must be 1..1", new[] { nameof(Questionnaire) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'QuestionnaireResponse.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        // Validate Item cardinality
        var itemCount = Item?.Count ?? 0;
        if (itemCount < 0)
        {
            yield return new ValidationResult("Element 'QuestionnaireResponse.item' cardinality must be 0..*", new[] { nameof(Item) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'QuestionnaireResponse.item.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'QuestionnaireResponse.item.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (LinkId == null)
        {
            yield return new ValidationResult("Element 'QuestionnaireResponse.item.linkId' cardinality must be 1..1", new[] { nameof(LinkId) });
        }
        // Validate Answer cardinality
        var answerCount = Answer?.Count ?? 0;
        if (answerCount < 0)
        {
            yield return new ValidationResult("Element 'QuestionnaireResponse.item.answer' cardinality must be 0..*", new[] { nameof(Answer) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'QuestionnaireResponse.item.answer.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'QuestionnaireResponse.item.answer.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Value == null)
        {
            yield return new ValidationResult("Element 'QuestionnaireResponse.item.answer.value[x]' cardinality must be 1..1", new[] { nameof(Value) });
        }
        // Validate Item cardinality
        var itemCount = Item?.Count ?? 0;
        if (itemCount < 0)
        {
            yield return new ValidationResult("Element 'QuestionnaireResponse.item.answer.item' cardinality must be 0..*", new[] { nameof(Item) });
        }
        // Validate Item cardinality
        var itemCount = Item?.Count ?? 0;
        if (itemCount < 0)
        {
            yield return new ValidationResult("Element 'QuestionnaireResponse.item.item' cardinality must be 0..*", new[] { nameof(Item) });
        }
        ...
        if (Status != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/questionnaire-answers-status|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(QuestionnaireResponse) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(QuestionnaireResponse) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(QuestionnaireResponse) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(QuestionnaireResponse) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(QuestionnaireResponse) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Authored) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("(answer.exists() and item.exists()).not()"))
        // {
        //     yield return new ValidationResult("Item cannot contain both item and answer", new[] { nameof(Item) });
        // }
        // Constraint: qrs-2
        // Expression: repeat(answer|item).select(item.where(answer.value.exists()).linkId.isDistinct()).allTrue()
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("repeat(answer|item).select(item.where(answer.value.exists()).linkId.isDistinct()).allTrue()"))
        // {
        //     yield return new ValidationResult("Repeated answers are combined in the answers array of a single item", new[] { nameof(Item) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
