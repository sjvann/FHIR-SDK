...
public class CompartmentDefinition : DomainResource
{
    ...
    /// reference. It SHALL remain the same when the compartment definition is stored on different servers.
    /// </summary>
    ...
    /// compartment definition author and is not expected to be globally unique. For example, it might be a
    ...
    /// </summary>
    [FhirElement("versionAlgorithmString", Order = 20)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("versionAlgorithmCoding", Order = 21)]
    [Cardinality(0, 1)]
    ...
    /// <summary>
    /// A natural language name identifying the compartment definition. This name should be usable as an
    ...
    /// </summary>
    [FhirElement("experimental", Order = 25)]
    [Cardinality(0, 1)]
    [JsonPropertyName("experimental")]
    public FhirBoolean? Experimental { get; set; }

    /// <summary>
    /// The date (and optionally time) when the compartment definition was last significantly changed. The
    ...
    /// </summary>
    [FhirElement("publisher", Order = 27)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("useContext", Order = 30)]
    [Cardinality(0, int.MaxValue)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate CompartmentDefinition cardinality
        var compartmentdefinitionCount = CompartmentDefinition?.Count ?? 0;
        if (compartmentdefinitionCount < 0)
        {
            yield return new ValidationResult("Element 'CompartmentDefinition' cardinality must be 0..*", new[] { nameof(CompartmentDefinition) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'CompartmentDefinition.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'CompartmentDefinition.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'CompartmentDefinition.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Url == null)
        {
            yield return new ValidationResult("Element 'CompartmentDefinition.url' cardinality must be 1..1", new[] { nameof(Url) });
        }
        if (Name == null)
        {
            yield return new ValidationResult("Element 'CompartmentDefinition.name' cardinality must be 1..1", new[] { nameof(Name) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'CompartmentDefinition.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        // Validate Contact cardinality
        var contactCount = Contact?.Count ?? 0;
        if (contactCount < 0)
        {
            yield return new ValidationResult("Element 'CompartmentDefinition.contact' cardinality must be 0..*", new[] { nameof(Contact) });
        }
        // Validate UseContext cardinality
        var usecontextCount = UseContext?.Count ?? 0;
        if (usecontextCount < 0)
        {
            yield return new ValidationResult("Element 'CompartmentDefinition.useContext' cardinality must be 0..*", new[] { nameof(UseContext) });
        }
        if (Code == null)
        {
            yield return new ValidationResult("Element 'CompartmentDefinition.code' cardinality must be 1..1", new[] { nameof(Code) });
        }
        if (Search == null)
        {
            yield return new ValidationResult("Element 'CompartmentDefinition.search' cardinality must be 1..1", new[] { nameof(Search) });
        }
        // Validate Resource cardinality
        var resourceCount = Resource?.Count ?? 0;
        if (resourceCount < 0)
        {
            yield return new ValidationResult("Element 'CompartmentDefinition.resource' cardinality must be 0..*", new[] { nameof(Resource) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'CompartmentDefinition.resource.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'CompartmentDefinition.resource.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Code == null)
        {
            yield return new ValidationResult("Element 'CompartmentDefinition.resource.code' cardinality must be 1..1", new[] { nameof(Code) });
        }
        // Validate Param cardinality
        var paramCount = Param?.Count ?? 0;
        if (paramCount < 0)
        {
            yield return new ValidationResult("Element 'CompartmentDefinition.resource.param' cardinality must be 0..*", new[] { nameof(Param) });
        }
        ...
        // {
        //     yield return new ValidationResult("Name should be usable as an identifier for the module by machine processing applications such as code generation", new[] { nameof(CompartmentDefinition) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Param) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(StartParam) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(EndParam) });
        // }
    }
