...
public class Slot : DomainResource
{
    ...
    /// </summary>
    [FhirElement("serviceCategory", Order = 19)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("serviceType", Order = 20)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// appointment.
    /// </summary>
    [FhirElement("specialty", Order = 21)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("appointmentType", Order = 22)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("schedule", Order = 23)]
    [Cardinality(1, 1)]
    [Required]
    [JsonPropertyName("schedule")]
    public Reference<Schedule> Schedule { get; set; }
    ...
    /// </summary>
    [FhirElement("overbooked", Order = 27)]
    [Cardinality(0, 1)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        if (slotCount < 0)
        {
            yield return new ValidationResult("Element 'Slot' cardinality must be 0..*", new[] { nameof(Slot) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'Slot.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Slot.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Slot.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'Slot.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        // Validate ServiceCategory cardinality
        var servicecategoryCount = ServiceCategory?.Count ?? 0;
        if (servicecategoryCount < 0)
        {
            yield return new ValidationResult("Element 'Slot.serviceCategory' cardinality must be 0..*", new[] { nameof(ServiceCategory) });
        }
        // Validate ServiceType cardinality
        var servicetypeCount = ServiceType?.Count ?? 0;
        if (servicetypeCount < 0)
        {
            yield return new ValidationResult("Element 'Slot.serviceType' cardinality must be 0..*", new[] { nameof(ServiceType) });
        }
        // Validate Specialty cardinality
        var specialtyCount = Specialty?.Count ?? 0;
        if (specialtyCount < 0)
        {
            yield return new ValidationResult("Element 'Slot.specialty' cardinality must be 0..*", new[] { nameof(Specialty) });
        }
        // Validate AppointmentType cardinality
        var appointmenttypeCount = AppointmentType?.Count ?? 0;
        if (appointmenttypeCount < 0)
        {
            yield return new ValidationResult("Element 'Slot.appointmentType' cardinality must be 0..*", new[] { nameof(AppointmentType) });
        }
        if (Schedule == null)
        {
            yield return new ValidationResult("Element 'Slot.schedule' cardinality must be 1..1", new[] { nameof(Schedule) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'Slot.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        if (Start == null)
        {
            yield return new ValidationResult("Element 'Slot.start' cardinality must be 1..1", new[] { nameof(Start) });
        }
        if (End == null)
        {
            yield return new ValidationResult("Element 'Slot.end' cardinality must be 1..1", new[] { nameof(End) });
        }
        ...
        if (Status != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/slotstatus|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(Slot) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(Slot) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(Slot) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(Slot) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(Slot) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Overbooked) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
