...
public class ValueSet : DomainResource
{
    ...
    /// <summary>
    /// An absolute URI that is used to identify this value set when it is referenced in a specification,
    /// model, design or an instance; also called its canonical identifier. This SHOULD be globally unique
    /// and SHOULD be a literal address at which an authoritative instance of this value set is (or will be)
    ...
    /// <summary>
    /// The identifier that is used to identify this version of the value set when it is referenced in a
    /// specification, model, design or instance. This is an arbitrary value managed by the value set author
    ...
    /// <summary>
    /// Indicates the mechanism used to compare versions to determine which ValueSet is more current. (as
    /// string)
    /// </summary>
    ...
    /// <summary>
    /// A natural language name identifying the value set. This name should be usable as an identifier for
    ...
    /// <summary>
    /// The date (and optionally time) when the value set metadata or content logical definition (.compose)
    ...
    /// textual description specifies the span of meanings for concepts to be included within the Value Set
    /// Expansion, and also may specify the intended use and limitations of the Value Set.
    /// </summary>
    ...
    /// </summary>
    [FhirElement("immutable", Order = 33)]
    [Cardinality(0, 1)]
    ...
    /// <summary>
    /// Descriptions related to the content of the ValueSet. Topics provide a high-level categorization as
    /// well as keywords for the ValueSet that can be useful for filtering and searching.
    /// </summary>
    [FhirElement("topic", Order = 40)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// some aspect of the ValueSet.
    /// </summary>
    [FhirElement("reviewer", Order = 43)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// the ValueSet for use in some setting.
    /// </summary>
    [FhirElement("endorser", Order = 44)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("relatedArtifact", Order = 45)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("compose", Order = 46)]
    [Cardinality(0, 1)]
    [JsonPropertyName("compose")]
    public BackboneElement Compose { get; set; }

    /// <summary>
    /// A value set can also be &quot;expanded&quot;, where the value set is turned into a simple collection
    /// of enumerated codes. This element holds the expansion, if it has been performed.
    /// </summary>
    [FhirElement("expansion", Order = 47)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("scope", Order = 48)]
    [Cardinality(0, 1)]
    [JsonPropertyName("scope")]
    public BackboneElement Scope { get; set; }
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate ValueSet cardinality
        var valuesetCount = ValueSet?.Count ?? 0;
        if (valuesetCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet' cardinality must be 0..*", new[] { nameof(ValueSet) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'ValueSet.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        // Validate Contact cardinality
        var contactCount = Contact?.Count ?? 0;
        if (contactCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.contact' cardinality must be 0..*", new[] { nameof(Contact) });
        }
        // Validate UseContext cardinality
        var usecontextCount = UseContext?.Count ?? 0;
        if (usecontextCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.useContext' cardinality must be 0..*", new[] { nameof(UseContext) });
        }
        // Validate Jurisdiction cardinality
        var jurisdictionCount = Jurisdiction?.Count ?? 0;
        if (jurisdictionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.jurisdiction' cardinality must be 0..*", new[] { nameof(Jurisdiction) });
        }
        // Validate Topic cardinality
        var topicCount = Topic?.Count ?? 0;
        if (topicCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.topic' cardinality must be 0..*", new[] { nameof(Topic) });
        }
        // Validate Author cardinality
        var authorCount = Author?.Count ?? 0;
        if (authorCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.author' cardinality must be 0..*", new[] { nameof(Author) });
        }
        // Validate Editor cardinality
        var editorCount = Editor?.Count ?? 0;
        if (editorCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.editor' cardinality must be 0..*", new[] { nameof(Editor) });
        }
        // Validate Reviewer cardinality
        var reviewerCount = Reviewer?.Count ?? 0;
        if (reviewerCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.reviewer' cardinality must be 0..*", new[] { nameof(Reviewer) });
        }
        // Validate Endorser cardinality
        var endorserCount = Endorser?.Count ?? 0;
        if (endorserCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.endorser' cardinality must be 0..*", new[] { nameof(Endorser) });
        }
        // Validate RelatedArtifact cardinality
        var relatedartifactCount = RelatedArtifact?.Count ?? 0;
        if (relatedartifactCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.relatedArtifact' cardinality must be 0..*", new[] { nameof(RelatedArtifact) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Include cardinality
        var includeCount = Include?.Count ?? 0;
        if (includeCount < 1)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.include' cardinality must be 1..*", new[] { nameof(Include) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.include.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.include.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Concept cardinality
        var conceptCount = Concept?.Count ?? 0;
        if (conceptCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.include.concept' cardinality must be 0..*", new[] { nameof(Concept) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.include.concept.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.include.concept.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Code == null)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.include.concept.code' cardinality must be 1..1", new[] { nameof(Code) });
        }
        // Validate Designation cardinality
        var designationCount = Designation?.Count ?? 0;
        if (designationCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.include.concept.designation' cardinality must be 0..*", new[] { nameof(Designation) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.include.concept.designation.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.include.concept.designation.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate AdditionalUse cardinality
        var additionaluseCount = AdditionalUse?.Count ?? 0;
        if (additionaluseCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.include.concept.designation.additionalUse' cardinality must be 0..*", new[] { nameof(AdditionalUse) });
        }
        if (Value == null)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.include.concept.designation.value' cardinality must be 1..1", new[] { nameof(Value) });
        }
        // Validate Filter cardinality
        var filterCount = Filter?.Count ?? 0;
        if (filterCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.include.filter' cardinality must be 0..*", new[] { nameof(Filter) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.include.filter.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.include.filter.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Property == null)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.include.filter.property' cardinality must be 1..1", new[] { nameof(Property) });
        }
        if (Op == null)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.include.filter.op' cardinality must be 1..1", new[] { nameof(Op) });
        }
        if (Value == null)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.include.filter.value' cardinality must be 1..1", new[] { nameof(Value) });
        }
        // Validate ValueSet cardinality
        var valuesetCount = ValueSet?.Count ?? 0;
        if (valuesetCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.include.valueSet' cardinality must be 0..*", new[] { nameof(ValueSet) });
        }
        // Validate Exclude cardinality
        var excludeCount = Exclude?.Count ?? 0;
        if (excludeCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.exclude' cardinality must be 0..*", new[] { nameof(Exclude) });
        }
        // Validate Property cardinality
        var propertyCount = Property?.Count ?? 0;
        if (propertyCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.compose.property' cardinality must be 0..*", new[] { nameof(Property) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Timestamp == null)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.timestamp' cardinality must be 1..1", new[] { nameof(Timestamp) });
        }
        // Validate Parameter cardinality
        var parameterCount = Parameter?.Count ?? 0;
        if (parameterCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.parameter' cardinality must be 0..*", new[] { nameof(Parameter) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.parameter.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.parameter.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Name == null)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.parameter.name' cardinality must be 1..1", new[] { nameof(Name) });
        }
        // Validate Property cardinality
        var propertyCount = Property?.Count ?? 0;
        if (propertyCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.property' cardinality must be 0..*", new[] { nameof(Property) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.property.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.property.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Code == null)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.property.code' cardinality must be 1..1", new[] { nameof(Code) });
        }
        // Validate Contains cardinality
        var containsCount = Contains?.Count ?? 0;
        if (containsCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.contains' cardinality must be 0..*", new[] { nameof(Contains) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.contains.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.contains.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Designation cardinality
        var designationCount = Designation?.Count ?? 0;
        if (designationCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.contains.designation' cardinality must be 0..*", new[] { nameof(Designation) });
        }
        // Validate Property cardinality
        var propertyCount = Property?.Count ?? 0;
        if (propertyCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.contains.property' cardinality must be 0..*", new[] { nameof(Property) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.contains.property.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.contains.property.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Code == null)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.contains.property.code' cardinality must be 1..1", new[] { nameof(Code) });
        }
        if (Value == null)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.contains.property.value[x]' cardinality must be 1..1", new[] { nameof(Value) });
        }
        // Validate SubProperty cardinality
        var subpropertyCount = SubProperty?.Count ?? 0;
        if (subpropertyCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.contains.property.subProperty' cardinality must be 0..*", new[] { nameof(SubProperty) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.contains.property.subProperty.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.contains.property.subProperty.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Code == null)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.contains.property.subProperty.code' cardinality must be 1..1", new[] { nameof(Code) });
        }
        if (Value == null)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.contains.property.subProperty.value[x]' cardinality must be 1..1", new[] { nameof(Value) });
        }
        // Validate Contains cardinality
        var containsCount = Contains?.Count ?? 0;
        if (containsCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.expansion.contains.contains' cardinality must be 0..*", new[] { nameof(Contains) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.scope.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'ValueSet.scope.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        ...
        // {
        //     yield return new ValidationResult("Name should be usable as an identifier for the module by machine processing applications such as code generation", new[] { nameof(ValueSet) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Immutable) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Compose) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("valueSet.exists() or system.exists()"))
        // {
        //     yield return new ValidationResult("A value set include/exclude SHALL have a value set or a system", new[] { nameof(Include) });
        // }
        // Constraint: vsd-2
        // Expression: (concept.exists() or filter.exists()) implies system.exists()
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("(concept.exists() or filter.exists()) implies system.exists()"))
        // {
        //     yield return new ValidationResult("A value set with concepts or filters SHALL include a system", new[] { nameof(Include) });
        // }
        // Constraint: vsd-3
        // Expression: concept.empty() or filter.empty()
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("concept.empty() or filter.empty()"))
        // {
        //     yield return new ValidationResult("Cannot have both concept and filter", new[] { nameof(Include) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Next) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Contains) });
        // }
        // Constraint: vsd-6
        // Expression: code.exists() or display.exists()
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("code.exists() or display.exists()"))
        // {
        //     yield return new ValidationResult("SHALL have a code or a display", new[] { nameof(Contains) });
        // }
        // Constraint: vsd-9
        // Expression: code.exists() or abstract = true
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("code.exists() or abstract = true"))
        // {
        //     yield return new ValidationResult("SHALL have a code if not abstract", new[] { nameof(Contains) });
        // }
        // Constraint: vsd-10
        ...
        // {
        //     yield return new ValidationResult("SHALL have a system if a code is present", new[] { nameof(Contains) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(SubProperty) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(InclusionCriteria) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(ExclusionCriteria) });
        // }
    }
