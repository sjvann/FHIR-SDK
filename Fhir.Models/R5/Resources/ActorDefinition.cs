...
/// <summary>
/// The ActorDefinition resource is used to describe an actor - a human or an application that plays a
/// role in data exchange, and that may have obligations associated with the role the actor plays.
/// </summary>
public class ActorDefinition : DomainResource
{
    ...
    /// globally unique and SHOULD be a literal address at which an authoritative instance of this actor
    ...
    /// <summary>
    /// A formal identifier that is used to identify this actor definition when it is represented in other
    ...
    /// <summary>
    /// The identifier that is used to identify this version of the actor definition when it is referenced
    /// in a specification, model, design or instance. This is an arbitrary value managed by the actor
    ...
    /// <summary>
    /// The date (and optionally time) when the actor definition was last significantly changed. The date
    /// must change when the business version changes and it must change if the status code changes. In
    /// addition, it should change when the substantive content of the actor definition changes.
    /// </summary>
    ...
    /// appropriate actor definition instances.
    /// </summary>
    ...
    /// <summary>
    /// A copyright statement relating to the actor definition and/or its contents. Copyright statements are
    /// generally legal restrictions on the use and publishing of the actor definition.
    /// </summary>
    ...
    /// </summary>
    [FhirElement("documentation", Order = 37)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("reference", Order = 38)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("capabilities", Order = 39)]
    [Cardinality(0, 1)]
    [JsonPropertyName("capabilities")]
    public FhirCanonical? Capabilities { get; set; }

    /// <summary>
    /// A url that identifies the definition of this actor in another IG (which IG must be listed in the
    /// dependencies). This actor inherits all the obligations etc. as defined in the other IG.
    /// </summary>
    [FhirElement("derivedFrom", Order = 40)]
    [Cardinality(0, int.MaxValue)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate ActorDefinition cardinality
        var actordefinitionCount = ActorDefinition?.Count ?? 0;
        if (actordefinitionCount < 0)
        {
            yield return new ValidationResult("Element 'ActorDefinition' cardinality must be 0..*", new[] { nameof(ActorDefinition) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'ActorDefinition.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'ActorDefinition.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'ActorDefinition.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'ActorDefinition.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'ActorDefinition.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        // Validate Contact cardinality
        var contactCount = Contact?.Count ?? 0;
        if (contactCount < 0)
        {
            yield return new ValidationResult("Element 'ActorDefinition.contact' cardinality must be 0..*", new[] { nameof(Contact) });
        }
        // Validate UseContext cardinality
        var usecontextCount = UseContext?.Count ?? 0;
        if (usecontextCount < 0)
        {
            yield return new ValidationResult("Element 'ActorDefinition.useContext' cardinality must be 0..*", new[] { nameof(UseContext) });
        }
        // Validate Jurisdiction cardinality
        var jurisdictionCount = Jurisdiction?.Count ?? 0;
        if (jurisdictionCount < 0)
        {
            yield return new ValidationResult("Element 'ActorDefinition.jurisdiction' cardinality must be 0..*", new[] { nameof(Jurisdiction) });
        }
        if (Type == null)
        {
            yield return new ValidationResult("Element 'ActorDefinition.type' cardinality must be 1..1", new[] { nameof(Type) });
        }
        // Validate Reference cardinality
        var referenceCount = Reference?.Count ?? 0;
        if (referenceCount < 0)
        {
            yield return new ValidationResult("Element 'ActorDefinition.reference' cardinality must be 0..*", new[] { nameof(Reference) });
        }
        // Validate DerivedFrom cardinality
        var derivedfromCount = DerivedFrom?.Count ?? 0;
        if (derivedfromCount < 0)
        {
            yield return new ValidationResult("Element 'ActorDefinition.derivedFrom' cardinality must be 0..*", new[] { nameof(DerivedFrom) });
        }
        ...
        if (Type != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/examplescenario-actor-type|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("Name should be usable as an identifier for the module by machine processing applications such as code generation", new[] { nameof(ActorDefinition) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Capabilities) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
