...
/// </summary>
public class TestPlan : DomainResource
{
    ...
    /// <summary>
    /// An absolute URI that is used to identify this test plan when it is referenced in a specification,
    /// model, design or an instance; also called its canonical identifier. This SHOULD be globally unique
    /// and SHOULD be a literal address at which an authoritative instance of this test plan is (or will be)
    /// published. This URL can be the target of a canonical reference. It SHALL remain the same when the
    /// test plan is stored on different servers.
    /// </summary>
    ...
    /// <summary>
    /// A formal identifier that is used to identify this test plan when it is represented in other formats,
    ...
    /// <summary>
    /// The identifier that is used to identify this version of the test plan when it is referenced in a
    /// specification, model, design or instance. This is an arbitrary value managed by the test plan author
    /// and is not expected to be globally unique. For example, it might be a timestamp (e.g. yyyymmdd) if a
    /// managed version is not available. There is also no expectation that versions can be placed in a
    ...
    /// <summary>
    /// A natural language name identifying the test plan. This name should be usable as an identifier for
    ...
    /// it should change when the substantive content of the test plan changes.
    /// </summary>
    ...
    /// appropriate test plan instances.
    /// </summary>
    ...
    /// declaration (e.g. (c) &apos;2015+ xyz organization&apos; should be sent in the copyrightLabel
    /// element.
    /// </summary>
    ...
    /// </summary>
    [FhirElement("testTools", Order = 38)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("testCase", Order = 41)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("testCase")]
    public List<BackboneElement>? TestCase { get; set; }
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate TestPlan cardinality
        var testplanCount = TestPlan?.Count ?? 0;
        if (testplanCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan' cardinality must be 0..*", new[] { nameof(TestPlan) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'TestPlan.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        // Validate Contact cardinality
        var contactCount = Contact?.Count ?? 0;
        if (contactCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.contact' cardinality must be 0..*", new[] { nameof(Contact) });
        }
        // Validate UseContext cardinality
        var usecontextCount = UseContext?.Count ?? 0;
        if (usecontextCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.useContext' cardinality must be 0..*", new[] { nameof(UseContext) });
        }
        // Validate Jurisdiction cardinality
        var jurisdictionCount = Jurisdiction?.Count ?? 0;
        if (jurisdictionCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.jurisdiction' cardinality must be 0..*", new[] { nameof(Jurisdiction) });
        }
        // Validate Category cardinality
        var categoryCount = Category?.Count ?? 0;
        if (categoryCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.category' cardinality must be 0..*", new[] { nameof(Category) });
        }
        // Validate Scope cardinality
        var scopeCount = Scope?.Count ?? 0;
        if (scopeCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.scope' cardinality must be 0..*", new[] { nameof(Scope) });
        }
        // Validate Dependency cardinality
        var dependencyCount = Dependency?.Count ?? 0;
        if (dependencyCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.dependency' cardinality must be 0..*", new[] { nameof(Dependency) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.dependency.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.dependency.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate TestCase cardinality
        var testcaseCount = TestCase?.Count ?? 0;
        if (testcaseCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.testCase' cardinality must be 0..*", new[] { nameof(TestCase) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.testCase.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.testCase.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Scope cardinality
        var scopeCount = Scope?.Count ?? 0;
        if (scopeCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.testCase.scope' cardinality must be 0..*", new[] { nameof(Scope) });
        }
        // Validate Dependency cardinality
        var dependencyCount = Dependency?.Count ?? 0;
        if (dependencyCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.testCase.dependency' cardinality must be 0..*", new[] { nameof(Dependency) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.testCase.dependency.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.testCase.dependency.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate TestRun cardinality
        var testrunCount = TestRun?.Count ?? 0;
        if (testrunCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.testCase.testRun' cardinality must be 0..*", new[] { nameof(TestRun) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.testCase.testRun.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.testCase.testRun.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.testCase.testRun.script.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.testCase.testRun.script.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate TestData cardinality
        var testdataCount = TestData?.Count ?? 0;
        if (testdataCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.testCase.testData' cardinality must be 0..*", new[] { nameof(TestData) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.testCase.testData.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.testCase.testData.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Type == null)
        {
            yield return new ValidationResult("Element 'TestPlan.testCase.testData.type' cardinality must be 1..1", new[] { nameof(Type) });
        }
        // Validate Assertion cardinality
        var assertionCount = Assertion?.Count ?? 0;
        if (assertionCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.testCase.assertion' cardinality must be 0..*", new[] { nameof(Assertion) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.testCase.assertion.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.testCase.assertion.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Type cardinality
        var typeCount = Type?.Count ?? 0;
        if (typeCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.testCase.assertion.type' cardinality must be 0..*", new[] { nameof(Type) });
        }
        // Validate Object cardinality
        var objectCount = Object?.Count ?? 0;
        if (objectCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.testCase.assertion.object' cardinality must be 0..*", new[] { nameof(Object) });
        }
        // Validate Result cardinality
        var resultCount = Result?.Count ?? 0;
        if (resultCount < 0)
        {
            yield return new ValidationResult("Element 'TestPlan.testCase.assertion.result' cardinality must be 0..*", new[] { nameof(Result) });
        }
        ...
        // {
        //     yield return new ValidationResult("Name should be usable as an identifier for the module by machine processing applications such as code generation", new[] { nameof(TestPlan) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(TestTools) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Dependency) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Predecessor) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(ExitCriteria) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(TestCase) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(TestRun) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Script) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(TestData) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Assertion) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Object) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
