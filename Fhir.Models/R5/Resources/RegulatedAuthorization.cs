...
/// <summary>
/// Regulatory approval, clearance or licencing related to a regulated product, treatment, facility or
/// activity that is cited in a guidance, regulation, rule or legislative act. An example is Market
...
public class RegulatedAuthorization : DomainResource
{
    ...
    /// </summary>
    [FhirElement("region", Order = 22)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("region")]
    public List<CodeableConcept>? Region { get; set; }

    /// <summary>
    /// The status that is authorised e.g. approved. Intermediate states and actions can be tracked with
    ...
    /// </summary>
    [FhirElement("statusDate", Order = 24)]
    [Cardinality(0, 1)]
    ...
    /// example, a Marketing Authorization includes the date of authorization and/or an expiration date.
    /// </summary>
    [FhirElement("validityPeriod", Order = 25)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("indication", Order = 26)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("intendedUse", Order = 27)]
    [Cardinality(0, 1)]
    [JsonPropertyName("intendedUse")]
    public CodeableConcept IntendedUse { get; set; }

    /// <summary>
    /// The legal or regulatory framework against which this authorization is granted, or other reasons for
    /// it.
    /// </summary>
    [FhirElement("basis", Order = 28)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// &apos;regulator&apos;).
    /// </summary>
    [FhirElement("holder", Order = 29)]
    [Cardinality(0, 1)]
    [JsonPropertyName("holder")]
    public Reference<Organization> Holder { get; set; }
    ...
    /// </summary>
    [FhirElement("regulator", Order = 30)]
    [Cardinality(0, 1)]
    [JsonPropertyName("regulator")]
    public Reference<Organization> Regulator { get; set; }

    /// <summary>
    /// Additional information or supporting documentation about the authorization.
    /// </summary>
    [FhirElement("attachedDocument", Order = 31)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("case", Order = 32)]
    [Cardinality(0, 1)]
    [JsonPropertyName("case")]
    public BackboneElement Case { get; set; }
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate RegulatedAuthorization cardinality
        var regulatedauthorizationCount = RegulatedAuthorization?.Count ?? 0;
        if (regulatedauthorizationCount < 0)
        {
            yield return new ValidationResult("Element 'RegulatedAuthorization' cardinality must be 0..*", new[] { nameof(RegulatedAuthorization) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'RegulatedAuthorization.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'RegulatedAuthorization.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'RegulatedAuthorization.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'RegulatedAuthorization.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        // Validate Subject cardinality
        var subjectCount = Subject?.Count ?? 0;
        if (subjectCount < 0)
        {
            yield return new ValidationResult("Element 'RegulatedAuthorization.subject' cardinality must be 0..*", new[] { nameof(Subject) });
        }
        // Validate Region cardinality
        var regionCount = Region?.Count ?? 0;
        if (regionCount < 0)
        {
            yield return new ValidationResult("Element 'RegulatedAuthorization.region' cardinality must be 0..*", new[] { nameof(Region) });
        }
        // Validate Indication cardinality
        var indicationCount = Indication?.Count ?? 0;
        if (indicationCount < 0)
        {
            yield return new ValidationResult("Element 'RegulatedAuthorization.indication' cardinality must be 0..*", new[] { nameof(Indication) });
        }
        // Validate Basis cardinality
        var basisCount = Basis?.Count ?? 0;
        if (basisCount < 0)
        {
            yield return new ValidationResult("Element 'RegulatedAuthorization.basis' cardinality must be 0..*", new[] { nameof(Basis) });
        }
        // Validate AttachedDocument cardinality
        var attacheddocumentCount = AttachedDocument?.Count ?? 0;
        if (attacheddocumentCount < 0)
        {
            yield return new ValidationResult("Element 'RegulatedAuthorization.attachedDocument' cardinality must be 0..*", new[] { nameof(AttachedDocument) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'RegulatedAuthorization.case.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'RegulatedAuthorization.case.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Application cardinality
        var applicationCount = Application?.Count ?? 0;
        if (applicationCount < 0)
        {
            yield return new ValidationResult("Element 'RegulatedAuthorization.case.application' cardinality must be 0..*", new[] { nameof(Application) });
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(RegulatedAuthorization) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(RegulatedAuthorization) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(RegulatedAuthorization) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(RegulatedAuthorization) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(RegulatedAuthorization) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Holder) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Regulator) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Case) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Application) });
        // }
    }
