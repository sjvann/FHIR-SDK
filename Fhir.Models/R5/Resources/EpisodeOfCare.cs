...
public class EpisodeOfCare : DomainResource
{
    ...
    /// <summary>
    /// The EpisodeOfCare may be known by different identifiers for different contexts of use, such as when
    /// an external agency is tracking the Episode for funding purposes.
    /// </summary>
    ...
    /// </summary>
    [FhirElement("statusHistory", Order = 20)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("statusHistory")]
    public List<BackboneElement>? StatusHistory { get; set; }

    /// <summary>
    /// A classification of the type of episode of care; e.g. specialist referral, disease management, type
    /// of funded care.
    /// </summary>
    ...
    /// </summary>
    [FhirElement("diagnosis", Order = 23)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("managingOrganization", Order = 25)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("referralRequest", Order = 27)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("referralRequest")]
    public List<Reference<ServiceRequest>>? ReferralRequest { get; set; }

    /// <summary>
    /// The practitioner that is the care manager/care coordinator for this patient.
    /// </summary>
    [FhirElement("careManager", Order = 28)]
    [Cardinality(0, 1)]
    [JsonPropertyName("careManager")]
    public Reference<Practitioner> CareManager { get; set; }

    /// <summary>
    /// The list of practitioners that may be facilitating this episode of care for specific purposes.
    /// </summary>
    [FhirElement("careTeam", Order = 29)]
    [Cardinality(0, int.MaxValue)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        {
            yield return new ValidationResult("Element 'EpisodeOfCare' cardinality must be 0..*", new[] { nameof(EpisodeOfCare) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        // Validate StatusHistory cardinality
        var statushistoryCount = StatusHistory?.Count ?? 0;
        if (statushistoryCount < 0)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.statusHistory' cardinality must be 0..*", new[] { nameof(StatusHistory) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.statusHistory.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.statusHistory.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.statusHistory.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        if (Period == null)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.statusHistory.period' cardinality must be 1..1", new[] { nameof(Period) });
        }
        // Validate Type cardinality
        var typeCount = Type?.Count ?? 0;
        if (typeCount < 0)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.type' cardinality must be 0..*", new[] { nameof(Type) });
        }
        // Validate Reason cardinality
        var reasonCount = Reason?.Count ?? 0;
        if (reasonCount < 0)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.reason' cardinality must be 0..*", new[] { nameof(Reason) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.reason.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.reason.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Value cardinality
        var valueCount = Value?.Count ?? 0;
        if (valueCount < 0)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.reason.value' cardinality must be 0..*", new[] { nameof(Value) });
        }
        // Validate Diagnosis cardinality
        var diagnosisCount = Diagnosis?.Count ?? 0;
        if (diagnosisCount < 0)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.diagnosis' cardinality must be 0..*", new[] { nameof(Diagnosis) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.diagnosis.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.diagnosis.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Condition cardinality
        var conditionCount = Condition?.Count ?? 0;
        if (conditionCount < 0)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.diagnosis.condition' cardinality must be 0..*", new[] { nameof(Condition) });
        }
        if (Patient == null)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.patient' cardinality must be 1..1", new[] { nameof(Patient) });
        }
        // Validate ReferralRequest cardinality
        var referralrequestCount = ReferralRequest?.Count ?? 0;
        if (referralrequestCount < 0)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.referralRequest' cardinality must be 0..*", new[] { nameof(ReferralRequest) });
        }
        // Validate CareTeam cardinality
        var careteamCount = CareTeam?.Count ?? 0;
        if (careteamCount < 0)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.careTeam' cardinality must be 0..*", new[] { nameof(CareTeam) });
        }
        // Validate Account cardinality
        var accountCount = Account?.Count ?? 0;
        if (accountCount < 0)
        {
            yield return new ValidationResult("Element 'EpisodeOfCare.account' cardinality must be 0..*", new[] { nameof(Account) });
        }
        ...
        if (Status != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/episode-of-care-status|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate Status ValueSet binding
        if (Status != null)
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(EpisodeOfCare) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(EpisodeOfCare) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(EpisodeOfCare) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(EpisodeOfCare) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(EpisodeOfCare) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(StatusHistory) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(ReferralRequest) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(CareManager) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
