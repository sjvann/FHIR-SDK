...
public class List : DomainResource
{
    ...
    /// <summary>
    /// How this list was prepared - whether it is a working list that is suitable for being maintained on
    /// an ongoing basis, or if it represents a snapshot of a list of items from another source, or whether
    /// it is a prepared list where items may be marked as added, modified or deleted.
    /// </summary>
    [FhirElement("mode", Order = 20)]
    [Cardinality(1, 1)]
    [Required]
    ...
    /// <summary>
    /// The common subject(s) (or patient(s)) of the resources that are in the list if there is one (or a
    /// set of subjects).
    /// </summary>
    [FhirElement("subject", Order = 23)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("subject")]
    public List<Reference<Resource>>? Subject { get; set; }
    ...
    /// </summary>
    [FhirElement("date", Order = 25)]
    [Cardinality(0, 1)]
    [JsonPropertyName("date")]
    public FhirDateTime? Date { get; set; }

    /// <summary>
    /// The entity responsible for deciding what the contents of the list were. Where the list was created
    /// by a human, this is the same as the author of the list.
    /// </summary>
    [FhirElement("source", Order = 26)]
    [Cardinality(0, 1)]
    [JsonPropertyName("source")]
    public Reference<Practitioner> Source { get; set; }

    /// <summary>
    /// What order applies to the items in the list.
    /// </summary>
    [FhirElement("orderedBy", Order = 27)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("emptyReason", Order = 30)]
    [Cardinality(0, 1)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        if (listCount < 0)
        {
            yield return new ValidationResult("Element 'List' cardinality must be 0..*", new[] { nameof(List) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'List.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'List.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'List.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'List.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'List.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        if (Mode == null)
        {
            yield return new ValidationResult("Element 'List.mode' cardinality must be 1..1", new[] { nameof(Mode) });
        }
        // Validate Subject cardinality
        var subjectCount = Subject?.Count ?? 0;
        if (subjectCount < 0)
        {
            yield return new ValidationResult("Element 'List.subject' cardinality must be 0..*", new[] { nameof(Subject) });
        }
        // Validate Note cardinality
        var noteCount = Note?.Count ?? 0;
        if (noteCount < 0)
        {
            yield return new ValidationResult("Element 'List.note' cardinality must be 0..*", new[] { nameof(Note) });
        }
        // Validate Entry cardinality
        var entryCount = Entry?.Count ?? 0;
        if (entryCount < 0)
        {
            yield return new ValidationResult("Element 'List.entry' cardinality must be 0..*", new[] { nameof(Entry) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'List.entry.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'List.entry.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Item == null)
        {
            yield return new ValidationResult("Element 'List.entry.item' cardinality must be 1..1", new[] { nameof(Item) });
        }
        ...
        if (Status != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/list-status|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate Mode ValueSet binding
        if (Mode != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/list-mode|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(List) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(List) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(List) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(List) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(List) });
        // }
        // Constraint: lst-1
        // Expression: emptyReason.empty() or entry.empty()
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("emptyReason.empty() or entry.empty()"))
        // {
        //     yield return new ValidationResult("A list can only have an emptyReason if it is empty", new[] { nameof(List) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(OrderedBy) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Flag) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Deleted) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(EmptyReason) });
        // }
    }
