...
public class GraphDefinition : DomainResource
{
    ...
    /// globally unique and SHOULD be a literal address at which an authoritative instance of this graph
    ...
    /// <summary>
    /// A formal identifier that is used to identify this GraphDefinition when it is represented in other
    ...
    /// <summary>
    /// The identifier that is used to identify this version of the graph definition when it is referenced
    /// in a specification, model, design or instance. This is an arbitrary value managed by the graph
    ...
    /// <summary>
    /// The date (and optionally time) when the graph definition was last significantly changed. The date
    /// must change when the business version changes and it must change if the status code changes. In
    /// addition, it should change when the substantive content of the graph definition changes.
    /// </summary>
    ...
    /// <summary>
    /// A copyright statement relating to the graph definition and/or its contents. Copyright statements are
    /// generally legal restrictions on the use and publishing of the graph definition.
    /// </summary>
    ...
    /// <summary>
    /// The Node at which instances of this graph start. If there is no nominated start, the graph can start
    /// at any of the nodes.
    /// </summary>
    [FhirElement("start", Order = 36)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("node", Order = 37)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("node")]
    public List<BackboneElement>? Node { get; set; }

    /// <summary>
    /// Links this graph makes rules about.
    /// </summary>
    [FhirElement("link", Order = 38)]
    [Cardinality(0, int.MaxValue)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate GraphDefinition cardinality
        var graphdefinitionCount = GraphDefinition?.Count ?? 0;
        if (graphdefinitionCount < 0)
        {
            yield return new ValidationResult("Element 'GraphDefinition' cardinality must be 0..*", new[] { nameof(GraphDefinition) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'GraphDefinition.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'GraphDefinition.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'GraphDefinition.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'GraphDefinition.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Name == null)
        {
            yield return new ValidationResult("Element 'GraphDefinition.name' cardinality must be 1..1", new[] { nameof(Name) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'GraphDefinition.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        // Validate Contact cardinality
        var contactCount = Contact?.Count ?? 0;
        if (contactCount < 0)
        {
            yield return new ValidationResult("Element 'GraphDefinition.contact' cardinality must be 0..*", new[] { nameof(Contact) });
        }
        // Validate UseContext cardinality
        var usecontextCount = UseContext?.Count ?? 0;
        if (usecontextCount < 0)
        {
            yield return new ValidationResult("Element 'GraphDefinition.useContext' cardinality must be 0..*", new[] { nameof(UseContext) });
        }
        // Validate Jurisdiction cardinality
        var jurisdictionCount = Jurisdiction?.Count ?? 0;
        if (jurisdictionCount < 0)
        {
            yield return new ValidationResult("Element 'GraphDefinition.jurisdiction' cardinality must be 0..*", new[] { nameof(Jurisdiction) });
        }
        // Validate Node cardinality
        var nodeCount = Node?.Count ?? 0;
        if (nodeCount < 0)
        {
            yield return new ValidationResult("Element 'GraphDefinition.node' cardinality must be 0..*", new[] { nameof(Node) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'GraphDefinition.node.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'GraphDefinition.node.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (NodeId == null)
        {
            yield return new ValidationResult("Element 'GraphDefinition.node.nodeId' cardinality must be 1..1", new[] { nameof(NodeId) });
        }
        if (Type == null)
        {
            yield return new ValidationResult("Element 'GraphDefinition.node.type' cardinality must be 1..1", new[] { nameof(Type) });
        }
        // Validate Link cardinality
        var linkCount = Link?.Count ?? 0;
        if (linkCount < 0)
        {
            yield return new ValidationResult("Element 'GraphDefinition.link' cardinality must be 0..*", new[] { nameof(Link) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'GraphDefinition.link.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'GraphDefinition.link.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (SourceId == null)
        {
            yield return new ValidationResult("Element 'GraphDefinition.link.sourceId' cardinality must be 1..1", new[] { nameof(SourceId) });
        }
        if (TargetId == null)
        {
            yield return new ValidationResult("Element 'GraphDefinition.link.targetId' cardinality must be 1..1", new[] { nameof(TargetId) });
        }
        // Validate Compartment cardinality
        var compartmentCount = Compartment?.Count ?? 0;
        if (compartmentCount < 0)
        {
            yield return new ValidationResult("Element 'GraphDefinition.link.compartment' cardinality must be 0..*", new[] { nameof(Compartment) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'GraphDefinition.link.compartment.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'GraphDefinition.link.compartment.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Use == null)
        {
            yield return new ValidationResult("Element 'GraphDefinition.link.compartment.use' cardinality must be 1..1", new[] { nameof(Use) });
        }
        if (Rule == null)
        {
            yield return new ValidationResult("Element 'GraphDefinition.link.compartment.rule' cardinality must be 1..1", new[] { nameof(Rule) });
        }
        if (Code == null)
        {
            yield return new ValidationResult("Element 'GraphDefinition.link.compartment.code' cardinality must be 1..1", new[] { nameof(Code) });
        }
        ...
        if (Use != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/graph-compartment-use|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate Rule ValueSet binding
        if (Rule != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/graph-compartment-rule|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate Code ValueSet binding
        if (Code != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/compartment-type|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("Name should be usable as an identifier for the module by machine processing applications such as code generation", new[] { nameof(GraphDefinition) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Node) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(NodeId) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Params) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Compartment) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Rule) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
