...
/// </summary>
public class EncounterHistory : DomainResource
{
    ...
    /// </summary>
    [FhirElement("encounter", Order = 18)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("actualPeriod", Order = 26)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("plannedStartDate", Order = 27)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("plannedEndDate", Order = 28)]
    [Cardinality(0, 1)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate EncounterHistory cardinality
        var encounterhistoryCount = EncounterHistory?.Count ?? 0;
        if (encounterhistoryCount < 0)
        {
            yield return new ValidationResult("Element 'EncounterHistory' cardinality must be 0..*", new[] { nameof(EncounterHistory) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'EncounterHistory.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'EncounterHistory.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'EncounterHistory.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'EncounterHistory.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'EncounterHistory.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        if (Class == null)
        {
            yield return new ValidationResult("Element 'EncounterHistory.class' cardinality must be 1..1", new[] { nameof(Class) });
        }
        // Validate Type cardinality
        var typeCount = Type?.Count ?? 0;
        if (typeCount < 0)
        {
            yield return new ValidationResult("Element 'EncounterHistory.type' cardinality must be 0..*", new[] { nameof(Type) });
        }
        // Validate ServiceType cardinality
        var servicetypeCount = ServiceType?.Count ?? 0;
        if (servicetypeCount < 0)
        {
            yield return new ValidationResult("Element 'EncounterHistory.serviceType' cardinality must be 0..*", new[] { nameof(ServiceType) });
        }
        // Validate Location cardinality
        var locationCount = Location?.Count ?? 0;
        if (locationCount < 0)
        {
            yield return new ValidationResult("Element 'EncounterHistory.location' cardinality must be 0..*", new[] { nameof(Location) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'EncounterHistory.location.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'EncounterHistory.location.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Location == null)
        {
            yield return new ValidationResult("Element 'EncounterHistory.location.location' cardinality must be 1..1", new[] { nameof(Location) });
        }
        ...
        // Validate Class ValueSet binding
        if (Class != null)
        {
            // TODO: Implement ValueSet validation for http://terminology.hl7.org/ValueSet/v3-ActEncounterCode
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(EncounterHistory) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(EncounterHistory) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(EncounterHistory) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(EncounterHistory) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(EncounterHistory) });
        // }
        // Constraint: ele-1
