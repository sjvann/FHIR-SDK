...
public class Endpoint : DomainResource
{
    ...
    /// </summary>
    [FhirElement("connectionType", Order = 20)]
    ...
    /// <summary>
    /// The description of the endpoint and what it is for (typically used as supplemental information in an
    /// endpoint directory describing its usage/purpose).
    /// </summary>
    [FhirElement("description", Order = 22)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("environmentType", Order = 23)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("managingOrganization", Order = 24)]
    [Cardinality(0, 1)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        if (endpointCount < 0)
        {
            yield return new ValidationResult("Element 'Endpoint' cardinality must be 0..*", new[] { nameof(Endpoint) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'Endpoint.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Endpoint.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Endpoint.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'Endpoint.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'Endpoint.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        // Validate ConnectionType cardinality
        var connectiontypeCount = ConnectionType?.Count ?? 0;
        if (connectiontypeCount < 1)
        {
            yield return new ValidationResult("Element 'Endpoint.connectionType' cardinality must be 1..*", new[] { nameof(ConnectionType) });
        }
        // Validate EnvironmentType cardinality
        var environmenttypeCount = EnvironmentType?.Count ?? 0;
        if (environmenttypeCount < 0)
        {
            yield return new ValidationResult("Element 'Endpoint.environmentType' cardinality must be 0..*", new[] { nameof(EnvironmentType) });
        }
        // Validate Contact cardinality
        var contactCount = Contact?.Count ?? 0;
        if (contactCount < 0)
        {
            yield return new ValidationResult("Element 'Endpoint.contact' cardinality must be 0..*", new[] { nameof(Contact) });
        }
        // Validate Payload cardinality
        var payloadCount = Payload?.Count ?? 0;
        if (payloadCount < 0)
        {
            yield return new ValidationResult("Element 'Endpoint.payload' cardinality must be 0..*", new[] { nameof(Payload) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Endpoint.payload.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Endpoint.payload.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Type cardinality
        var typeCount = Type?.Count ?? 0;
        if (typeCount < 0)
        {
            yield return new ValidationResult("Element 'Endpoint.payload.type' cardinality must be 0..*", new[] { nameof(Type) });
        }
        // Validate MimeType cardinality
        var mimetypeCount = MimeType?.Count ?? 0;
        if (mimetypeCount < 0)
        {
            yield return new ValidationResult("Element 'Endpoint.payload.mimeType' cardinality must be 0..*", new[] { nameof(MimeType) });
        }
        if (Address == null)
        {
            yield return new ValidationResult("Element 'Endpoint.address' cardinality must be 1..1", new[] { nameof(Address) });
        }
        // Validate Header cardinality
        var headerCount = Header?.Count ?? 0;
        if (headerCount < 0)
        {
            yield return new ValidationResult("Element 'Endpoint.header' cardinality must be 0..*", new[] { nameof(Header) });
        }
        ...
        if (Status != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/endpoint-status|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate EnvironmentType ValueSet binding
        if (EnvironmentType != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/endpoint-environment
            // This requires a terminology service or local ValueSet cache
        }
        // Validate MimeType ValueSet binding
        if (MimeType != null)
        {
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(Endpoint) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(Endpoint) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(Endpoint) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(Endpoint) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(Endpoint) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(ConnectionType) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(EnvironmentType) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(MimeType) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Header) });
        // }
    }
