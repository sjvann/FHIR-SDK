...
public class MessageHeader : DomainResource
{
    ...
    /// defined as part of the FHIR specification are defined by the implementation. Alternatively a
    /// canonical uri to the EventDefinition. (as Coding)
    /// </summary>
    [FhirElement("eventCoding", Order = 18)]
    [Cardinality(1, 1)]
    ...
    /// Code that identifies the event this message represents and connects it with its definition. Events
    /// defined as part of the FHIR specification are defined by the implementation. Alternatively a
    /// canonical uri to the EventDefinition. (as canonical)
    /// </summary>
    [FhirElement("eventcanonicalUnknown", Order = 19)]
    [Cardinality(1, 1)]
    [ChoiceType("eventcanonical", "unknown")]
    [Required]
    [JsonPropertyName("eventcanonicalUnknown")]
    public FhirCanonical EventCanonical { get; set; }

    /// <summary>
    /// The destination application which the message is intended for.
    /// </summary>
    [FhirElement("destination", Order = 20)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("sender", Order = 21)]
    [Cardinality(0, 1)]
    [JsonPropertyName("sender")]
    public Reference<Practitioner> Sender { get; set; }
    ...
    /// <summary>
    /// The person or organization that accepts overall responsibility for the contents of the message. The
    /// implication is that the message event happened under the policies of the responsible party.
    /// </summary>
    [FhirElement("responsible", Order = 24)]
    [Cardinality(0, 1)]
    [JsonPropertyName("responsible")]
    public Reference<Practitioner> Responsible { get; set; }

    /// <summary>
    /// Coded indication of the cause for the event - indicates a reason for the occurrence of the event
    /// that is a focus of this message.
    /// </summary>
    ...
    /// to be a Parameters resource.
    /// </summary>
    [FhirElement("focus", Order = 27)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("definition", Order = 28)]
    [Cardinality(0, 1)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        var eventProperties = new[] { nameof(EventCoding) };
        ...
        if (eventSetCount > 1)
        {
            yield return new ValidationResult(
                "Only one of EventCoding may be specified",
                new[] { nameof(EventCoding) });
        }

        // Choice Type validation for eventcanonical[x]
        var eventcanonicalProperties = new[] { nameof(EventCanonical) };
        var eventcanonicalSetCount = eventcanonicalProperties.Count(prop => 
            GetType().GetProperty(prop)?.GetValue(this) != null);

        if (eventcanonicalSetCount > 1)
        {
            yield return new ValidationResult(
                "Only one of EventCanonical may be specified",
                new[] { nameof(EventCanonical) });
        }
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate MessageHeader cardinality
        var messageheaderCount = MessageHeader?.Count ?? 0;
        if (messageheaderCount < 0)
        {
            yield return new ValidationResult("Element 'MessageHeader' cardinality must be 0..*", new[] { nameof(MessageHeader) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'MessageHeader.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'MessageHeader.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'MessageHeader.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Event == null)
        {
            yield return new ValidationResult("Element 'MessageHeader.event[x]' cardinality must be 1..1", new[] { nameof(Event) });
        }
        // Validate Destination cardinality
        var destinationCount = Destination?.Count ?? 0;
        if (destinationCount < 0)
        {
            yield return new ValidationResult("Element 'MessageHeader.destination' cardinality must be 0..*", new[] { nameof(Destination) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'MessageHeader.destination.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'MessageHeader.destination.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Source == null)
        {
            yield return new ValidationResult("Element 'MessageHeader.source' cardinality must be 1..1", new[] { nameof(Source) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'MessageHeader.source.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'MessageHeader.source.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'MessageHeader.response.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'MessageHeader.response.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Identifier == null)
        {
            yield return new ValidationResult("Element 'MessageHeader.response.identifier' cardinality must be 1..1", new[] { nameof(Identifier) });
        }
        if (Code == null)
        {
            yield return new ValidationResult("Element 'MessageHeader.response.code' cardinality must be 1..1", new[] { nameof(Code) });
        }
        // Validate Focus cardinality
        var focusCount = Focus?.Count ?? 0;
        if (focusCount < 0)
        {
            yield return new ValidationResult("Element 'MessageHeader.focus' cardinality must be 0..*", new[] { nameof(Focus) });
        }
        ...
        if (Code != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/response-code|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(MessageHeader) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(MessageHeader) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(MessageHeader) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(MessageHeader) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(MessageHeader) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Details) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
