...
public class Questionnaire : DomainResource
{
    ...
    /// questionnaire is (or will be) published. This URL can be the target of a canonical reference. It
    ...
    /// specification, model, design or instance. This is an arbitrary value managed by the questionnaire
    ...
    /// <summary>
    /// A natural language name identifying the questionnaire. This name should be usable as an identifier
    ...
    /// </summary>
    [FhirElement("subjectType", Order = 28)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("subjectType")]
    public List<FhirCode>? SubjectType { get; set; }

    /// <summary>
    /// The date (and optionally time) when the questionnaire was last significantly changed. The date must
    ...
    /// the questionnaire.
    /// </summary>
    [FhirElement("publisher", Order = 30)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("description", Order = 32)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("jurisdiction", Order = 34)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("purpose", Order = 35)]
    [Cardinality(0, 1)]
    [JsonPropertyName("purpose")]
    public FhirMarkdown? Purpose { get; set; }

    /// <summary>
    /// A copyright statement relating to the questionnaire and/or its contents. Copyright statements are
    /// generally legal restrictions on the use and publishing of the questionnaire.
    /// </summary>
    [FhirElement("copyright", Order = 36)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("copyrightLabel", Order = 37)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("approvalDate", Order = 38)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("lastReviewDate", Order = 39)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("effectivePeriod", Order = 40)]
    [Cardinality(0, 1)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        {
            yield return new ValidationResult("Element 'Questionnaire' cardinality must be 0..*", new[] { nameof(Questionnaire) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        // Validate DerivedFrom cardinality
        var derivedfromCount = DerivedFrom?.Count ?? 0;
        if (derivedfromCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.derivedFrom' cardinality must be 0..*", new[] { nameof(DerivedFrom) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'Questionnaire.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        // Validate SubjectType cardinality
        var subjecttypeCount = SubjectType?.Count ?? 0;
        if (subjecttypeCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.subjectType' cardinality must be 0..*", new[] { nameof(SubjectType) });
        }
        // Validate Contact cardinality
        var contactCount = Contact?.Count ?? 0;
        if (contactCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.contact' cardinality must be 0..*", new[] { nameof(Contact) });
        }
        // Validate UseContext cardinality
        var usecontextCount = UseContext?.Count ?? 0;
        if (usecontextCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.useContext' cardinality must be 0..*", new[] { nameof(UseContext) });
        }
        // Validate Jurisdiction cardinality
        var jurisdictionCount = Jurisdiction?.Count ?? 0;
        if (jurisdictionCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.jurisdiction' cardinality must be 0..*", new[] { nameof(Jurisdiction) });
        }
        // Validate Code cardinality
        var codeCount = Code?.Count ?? 0;
        if (codeCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.code' cardinality must be 0..*", new[] { nameof(Code) });
        }
        // Validate Item cardinality
        var itemCount = Item?.Count ?? 0;
        if (itemCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.item' cardinality must be 0..*", new[] { nameof(Item) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.item.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.item.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (LinkId == null)
        {
            yield return new ValidationResult("Element 'Questionnaire.item.linkId' cardinality must be 1..1", new[] { nameof(LinkId) });
        }
        // Validate Code cardinality
        var codeCount = Code?.Count ?? 0;
        if (codeCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.item.code' cardinality must be 0..*", new[] { nameof(Code) });
        }
        if (Type == null)
        {
            yield return new ValidationResult("Element 'Questionnaire.item.type' cardinality must be 1..1", new[] { nameof(Type) });
        }
        // Validate EnableWhen cardinality
        var enablewhenCount = EnableWhen?.Count ?? 0;
        if (enablewhenCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.item.enableWhen' cardinality must be 0..*", new[] { nameof(EnableWhen) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.item.enableWhen.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.item.enableWhen.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Question == null)
        {
            yield return new ValidationResult("Element 'Questionnaire.item.enableWhen.question' cardinality must be 1..1", new[] { nameof(Question) });
        }
        if (Operator == null)
        {
            yield return new ValidationResult("Element 'Questionnaire.item.enableWhen.operator' cardinality must be 1..1", new[] { nameof(Operator) });
        }
        if (Answer == null)
        {
            yield return new ValidationResult("Element 'Questionnaire.item.enableWhen.answer[x]' cardinality must be 1..1", new[] { nameof(Answer) });
        }
        // Validate AnswerOption cardinality
        var answeroptionCount = AnswerOption?.Count ?? 0;
        if (answeroptionCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.item.answerOption' cardinality must be 0..*", new[] { nameof(AnswerOption) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.item.answerOption.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.item.answerOption.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Value == null)
        {
            yield return new ValidationResult("Element 'Questionnaire.item.answerOption.value[x]' cardinality must be 1..1", new[] { nameof(Value) });
        }
        // Validate Initial cardinality
        var initialCount = Initial?.Count ?? 0;
        if (initialCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.item.initial' cardinality must be 0..*", new[] { nameof(Initial) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.item.initial.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.item.initial.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Value == null)
        {
            yield return new ValidationResult("Element 'Questionnaire.item.initial.value[x]' cardinality must be 1..1", new[] { nameof(Value) });
        }
        // Validate Item cardinality
        var itemCount = Item?.Count ?? 0;
        if (itemCount < 0)
        {
            yield return new ValidationResult("Element 'Questionnaire.item.item' cardinality must be 0..*", new[] { nameof(Item) });
        }
        ...
        // Validate SubjectType ValueSet binding
        if (SubjectType != null)
        {
        ...
        if (Type != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/item-type|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate Operator ValueSet binding
        if (Operator != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/questionnaire-enable-operator|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate EnableBehavior ValueSet binding
        if (EnableBehavior != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/questionnaire-enable-behavior|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate DisabledDisplay ValueSet binding
        if (DisabledDisplay != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/questionnaire-disabled-display|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate AnswerConstraint ValueSet binding
        if (AnswerConstraint != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/questionnaire-answer-constraint|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("Name should be usable as an identifier for the module by machine processing applications such as code generation", new[] { nameof(Questionnaire) });
        // }
        // Constraint: que-2
        // Expression: descendants().linkId.isDistinct()
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("descendants().linkId.isDistinct()"))
        // {
        //     yield return new ValidationResult("The link ids for groups and questions must be unique within the questionnaire", new[] { nameof(Questionnaire) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(SubjectType) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("(type='group' and %resource.status='complete') implies item.empty().not()"))
        // {
        //     yield return new ValidationResult("Group items must have nested items when Questionanire is complete", new[] { nameof(Item) });
        // }
        // Constraint: que-1b
        // Expression: type='group' implies item.empty().not()
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("type='group' implies item.empty().not()"))
        // {
        //     yield return new ValidationResult("Groups should have items", new[] { nameof(Item) });
        // }
        // Constraint: que-1c
        // Expression: type='display' implies item.empty()
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("type='display' implies item.empty()"))
        // {
        //     yield return new ValidationResult("Display items cannot have child items", new[] { nameof(Item) });
        // }
        // Constraint: que-3
        // Expression: type!='display' or code.empty()
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("type!='display' or code.empty()"))
        // {
        //     yield return new ValidationResult("Display items cannot have a "code" asserted", new[] { nameof(Item) });
        // }
        // Constraint: que-4
        // Expression: answerOption.empty() or answerValueSet.empty()
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("answerOption.empty() or answerValueSet.empty()"))
        // {
        //     yield return new ValidationResult("A question cannot have both answerOption and answerValueSet", new[] { nameof(Item) });
        // }
        // Constraint: que-5
        ...
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("(type='coding' or type = 'decimal' or type = 'integer' or type = 'date' or type = 'dateTime' or type = 'time' or type = 'string' or type = 'quantity') or (answerValueSet.empty() and answerOption.empty())"))
        // {
        //     yield return new ValidationResult("Only coding, decimal, integer, date, dateTime, time, string or quantity  items can have answerOption or answerValueSet", new[] { nameof(Item) });
        // }
        // Constraint: que-6
        // Expression: type!='display' or (required.empty() and repeats.empty())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("type!='display' or (required.empty() and repeats.empty())"))
        // {
        //     yield return new ValidationResult("Required and repeat aren't permitted for display items", new[] { nameof(Item) });
        // }
        // Constraint: que-8
        // Expression: (type!='group' and type!='display') or initial.empty()
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("(type!='group' and type!='display') or initial.empty()"))
        // {
        //     yield return new ValidationResult("Initial values can't be specified for groups or display items", new[] { nameof(Item) });
        // }
        // Constraint: que-9
        // Expression: type!='display' or readOnly.empty()
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("type!='display' or readOnly.empty()"))
        // {
        //     yield return new ValidationResult("Read-only can't be specified for "display" items", new[] { nameof(Item) });
        // }
        // Constraint: que-10
        // Expression: (type in ('boolean' | 'decimal' | 'integer' | 'string' | 'text' | 'url')) or answerConstraint='optionOrString' or maxLength.empty()
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("(type in ('boolean' | 'decimal' | 'integer' | 'string' | 'text' | 'url')) or answerConstraint='optionOrString' or maxLength.empty()"))
        // {
        //     yield return new ValidationResult("Maximum length can only be declared for simple question types", new[] { nameof(Item) });
        // }
        // Constraint: que-11
        // Expression: answerOption.empty() or initial.empty()
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("answerOption.empty() or initial.empty()"))
        // {
        //     yield return new ValidationResult("If one or more answerOption is present, initial cannot be present.  Use answerOption.initialSelected instead", new[] { nameof(Item) });
        // }
        // Constraint: que-12
        // Expression: enableWhen.count() > 1 implies enableBehavior.exists()
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("enableWhen.count() > 1 implies enableBehavior.exists()"))
        // {
        //     yield return new ValidationResult("If there are more than one enableWhen, enableBehavior must be specified", new[] { nameof(Item) });
        // }
        // Constraint: que-13
        // Expression: repeats=true or initial.count() <= 1
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("repeats=true or initial.count() <= 1"))
        // {
        //     yield return new ValidationResult("Can only have multiple initial values for repeating items", new[] { nameof(Item) });
        // }
        // Constraint: que-14
        // Expression: answerConstraint.exists() implies answerOption.exists() or answerValueSet.exists()
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("answerConstraint.exists() implies answerOption.exists() or answerValueSet.exists()"))
        // {
        //     yield return new ValidationResult("Can only have answerConstraint if answerOption or answerValueSet are present.  (This is a warning because extensions may serve the same purpose)", new[] { nameof(Item) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(EnableWhen) });
        // }
        // Constraint: que-7
        // Expression: operator = 'exists' implies (answer is boolean)
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("operator = 'exists' implies (answer is boolean)"))
        // {
        //     yield return new ValidationResult("If the operator is 'exists', the value must be a boolean", new[] { nameof(EnableWhen) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Question) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(EnableBehavior) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(DisabledDisplay) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Repeats) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(ReadOnly) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(AnswerConstraint) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(AnswerValueSet) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(AnswerOption) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(InitialSelected) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Initial) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
