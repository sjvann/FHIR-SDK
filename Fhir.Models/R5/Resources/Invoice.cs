...
public class Invoice : DomainResource
{
    ...
    /// <summary>
    /// Identifier of this Invoice, often used for reference in correspondence about this invoice or for
    ...
    /// <summary>
    /// In case of Invoice cancellation a reason must be given (entered in error, superseded by corrected
    /// invoice etc.).
    /// </summary>
    [FhirElement("cancelledReason", Order = 20)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("recipient", Order = 23)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("creation", Order = 25)]
    [Cardinality(0, 1)]
    [JsonPropertyName("creation")]
    public FhirDateTime? Creation { get; set; }

    /// <summary>
    /// Date/time(s) range of services included in this invoice. (as date)
    /// </summary>
    [FhirElement("perioddateUnknown", Order = 26)]
    [Cardinality(0, 1)]
    [ChoiceType("perioddate", "unknown")]
    [JsonPropertyName("perioddateUnknown")]
    public FhirDate? PeriodDate { get; set; }

    /// <summary>
    /// Date/time(s) range of services included in this invoice. (as Period)
    /// </summary>
    [FhirElement("periodPeriod", Order = 27)]
    [Cardinality(0, 1)]
    [ChoiceType("period", "period")]
    ...
    /// </summary>
    [FhirElement("participant", Order = 28)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// surcharges/deductions that apply in certain conditions. The priceComponent element can be used to
    /// offer transparency to the recipient of the Invoice of how the total price was calculated.
    /// </summary>
    [FhirElement("totalPriceComponent", Order = 32)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("totalGross", Order = 34)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("paymentTerms", Order = 35)]
    [Cardinality(0, 1)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Choice Type validation for perioddate[x]
        var perioddateProperties = new[] { nameof(PeriodDate) };
        var perioddateSetCount = perioddateProperties.Count(prop => 
            GetType().GetProperty(prop)?.GetValue(this) != null);

        if (perioddateSetCount > 1)
        {
            yield return new ValidationResult(
                "Only one of PeriodDate may be specified",
                new[] { nameof(PeriodDate) });
        }

        // Choice Type validation for period[x]
        var periodProperties = new[] { nameof(PeriodPeriod) };
        ...
        {
            yield return new ValidationResult(
                "Only one of PeriodPeriod may be specified",
                new[] { nameof(PeriodPeriod) });
        }
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate Invoice cardinality
        var invoiceCount = Invoice?.Count ?? 0;
        if (invoiceCount < 0)
        {
            yield return new ValidationResult("Element 'Invoice' cardinality must be 0..*", new[] { nameof(Invoice) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'Invoice.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Invoice.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Invoice.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'Invoice.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'Invoice.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        // Validate Participant cardinality
        var participantCount = Participant?.Count ?? 0;
        if (participantCount < 0)
        {
            yield return new ValidationResult("Element 'Invoice.participant' cardinality must be 0..*", new[] { nameof(Participant) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Invoice.participant.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Invoice.participant.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Actor == null)
        {
            yield return new ValidationResult("Element 'Invoice.participant.actor' cardinality must be 1..1", new[] { nameof(Actor) });
        }
        // Validate LineItem cardinality
        var lineitemCount = LineItem?.Count ?? 0;
        if (lineitemCount < 0)
        {
            yield return new ValidationResult("Element 'Invoice.lineItem' cardinality must be 0..*", new[] { nameof(LineItem) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Invoice.lineItem.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Invoice.lineItem.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (ChargeItem == null)
        {
            yield return new ValidationResult("Element 'Invoice.lineItem.chargeItem[x]' cardinality must be 1..1", new[] { nameof(ChargeItem) });
        }
        // Validate PriceComponent cardinality
        var pricecomponentCount = PriceComponent?.Count ?? 0;
        if (pricecomponentCount < 0)
        {
            yield return new ValidationResult("Element 'Invoice.lineItem.priceComponent' cardinality must be 0..*", new[] { nameof(PriceComponent) });
        }
        // Validate TotalPriceComponent cardinality
        var totalpricecomponentCount = TotalPriceComponent?.Count ?? 0;
        if (totalpricecomponentCount < 0)
        {
            yield return new ValidationResult("Element 'Invoice.totalPriceComponent' cardinality must be 0..*", new[] { nameof(TotalPriceComponent) });
        }
        // Validate Note cardinality
        var noteCount = Note?.Count ?? 0;
        if (noteCount < 0)
        {
            yield return new ValidationResult("Element 'Invoice.note' cardinality must be 0..*", new[] { nameof(Note) });
        }
        ...
        if (Status != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/invoice-status|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(Invoice) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(Invoice) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(Invoice) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(Invoice) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(Invoice) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(CancelledReason) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(LineItem) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(TotalNet) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(TotalGross) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(PaymentTerms) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
