...
/// </summary>
public class RequestOrchestration : DomainResource
{
    ...
    /// <summary>
    /// A canonical URL referencing a FHIR-defined protocol, guideline, orderset or other definition that is
    ...
    /// <summary>
    /// A URL referencing an externally defined protocol, guideline, orderset or other definition that is
    /// adhered to in whole or in part by this request.
    /// </summary>
    ...
    public List<Reference<Resource>>? Replaces { get; set; }
    ...
    /// <summary>
    /// The current state of the request. For request orchestrations, the status reflects the status of all
    /// the requests in the orchestration.
    /// </summary>
    ...
    /// <summary>
    /// Indicates the level of authority/intentionality associated with the request and where the request
    ...
    /// </summary>
    [FhirElement("authoredOn", Order = 30)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("goal", Order = 33)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("goal")]
    public List<Reference<Goal>>? Goal { get; set; }

    /// <summary>
    /// Provides a mechanism to communicate additional information about the response.
    /// </summary>
    [FhirElement("note", Order = 34)]
    [Cardinality(0, int.MaxValue)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate RequestOrchestration cardinality
        var requestorchestrationCount = RequestOrchestration?.Count ?? 0;
        if (requestorchestrationCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration' cardinality must be 0..*", new[] { nameof(RequestOrchestration) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        ...
        {
            yield return new ValidationResult("Element 'RequestOrchestration.instantiatesCanonical' cardinality must be 0..*", new[] { nameof(InstantiatesCanonical) });
        }
        // Validate InstantiatesUri cardinality
        var instantiatesuriCount = InstantiatesUri?.Count ?? 0;
        if (instantiatesuriCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.instantiatesUri' cardinality must be 0..*", new[] { nameof(InstantiatesUri) });
        }
        // Validate BasedOn cardinality
        var basedonCount = BasedOn?.Count ?? 0;
        if (basedonCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.basedOn' cardinality must be 0..*", new[] { nameof(BasedOn) });
        }
        // Validate Replaces cardinality
        var replacesCount = Replaces?.Count ?? 0;
        if (replacesCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.replaces' cardinality must be 0..*", new[] { nameof(Replaces) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        if (Intent == null)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.intent' cardinality must be 1..1", new[] { nameof(Intent) });
        }
        // Validate Reason cardinality
        var reasonCount = Reason?.Count ?? 0;
        if (reasonCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.reason' cardinality must be 0..*", new[] { nameof(Reason) });
        }
        // Validate Goal cardinality
        var goalCount = Goal?.Count ?? 0;
        if (goalCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.goal' cardinality must be 0..*", new[] { nameof(Goal) });
        }
        // Validate Note cardinality
        var noteCount = Note?.Count ?? 0;
        if (noteCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.note' cardinality must be 0..*", new[] { nameof(Note) });
        }
        // Validate Action cardinality
        var actionCount = Action?.Count ?? 0;
        if (actionCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action' cardinality must be 0..*", new[] { nameof(Action) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Code cardinality
        var codeCount = Code?.Count ?? 0;
        if (codeCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.code' cardinality must be 0..*", new[] { nameof(Code) });
        }
        // Validate Documentation cardinality
        var documentationCount = Documentation?.Count ?? 0;
        if (documentationCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.documentation' cardinality must be 0..*", new[] { nameof(Documentation) });
        }
        // Validate Goal cardinality
        var goalCount = Goal?.Count ?? 0;
        if (goalCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.goal' cardinality must be 0..*", new[] { nameof(Goal) });
        }
        // Validate Condition cardinality
        var conditionCount = Condition?.Count ?? 0;
        if (conditionCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.condition' cardinality must be 0..*", new[] { nameof(Condition) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.condition.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.condition.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Kind == null)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.condition.kind' cardinality must be 1..1", new[] { nameof(Kind) });
        }
        // Validate Input cardinality
        var inputCount = Input?.Count ?? 0;
        if (inputCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.input' cardinality must be 0..*", new[] { nameof(Input) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.input.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.input.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Output cardinality
        var outputCount = Output?.Count ?? 0;
        if (outputCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.output' cardinality must be 0..*", new[] { nameof(Output) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.output.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.output.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate RelatedAction cardinality
        var relatedactionCount = RelatedAction?.Count ?? 0;
        if (relatedactionCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.relatedAction' cardinality must be 0..*", new[] { nameof(RelatedAction) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.relatedAction.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.relatedAction.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (TargetId == null)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.relatedAction.targetId' cardinality must be 1..1", new[] { nameof(TargetId) });
        }
        if (Relationship == null)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.relatedAction.relationship' cardinality must be 1..1", new[] { nameof(Relationship) });
        }
        // Validate Participant cardinality
        var participantCount = Participant?.Count ?? 0;
        if (participantCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.participant' cardinality must be 0..*", new[] { nameof(Participant) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.participant.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.participant.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate DynamicValue cardinality
        var dynamicvalueCount = DynamicValue?.Count ?? 0;
        if (dynamicvalueCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.dynamicValue' cardinality must be 0..*", new[] { nameof(DynamicValue) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.dynamicValue.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.dynamicValue.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Action cardinality
        var actionCount = Action?.Count ?? 0;
        if (actionCount < 0)
        {
            yield return new ValidationResult("Element 'RequestOrchestration.action.action' cardinality must be 0..*", new[] { nameof(Action) });
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(RequestOrchestration) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(RequestOrchestration) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(RequestOrchestration) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(RequestOrchestration) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(RequestOrchestration) });
        // }
        // Constraint: ele-1
        ...
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("resource.exists() != action.exists()"))
        // {
        //     yield return new ValidationResult("Must have resource or action but not both", new[] { nameof(Action) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
