...
public class ImmunizationEvaluation : DomainResource
{
    ...
    /// </summary>
    [FhirElement("authority", Order = 22)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("targetDisease", Order = 23)]
    [Cardinality(1, 1)]
    [Required]
    ...
    /// </summary>
    [FhirElement("immunizationEvent", Order = 24)]
    [Cardinality(1, 1)]
    [Required]
    [JsonPropertyName("immunizationEvent")]
    public Reference<Immunization> ImmunizationEvent { get; set; }

    /// <summary>
    /// Indicates if the dose is valid or not valid with respect to the published recommendations.
    /// </summary>
    [FhirElement("doseStatus", Order = 25)]
    [Cardinality(1, 1)]
    [Required]
    ...
    /// <summary>
    /// Provides an explanation as to why the vaccine administration event is valid or not relative to the
    /// published recommendations.
    /// </summary>
    [FhirElement("doseStatusReason", Order = 26)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("doseNumber", Order = 29)]
    [Cardinality(0, 1)]
    [JsonPropertyName("doseNumber")]
    public FhirString? DoseNumber { get; set; }

    /// <summary>
    /// The recommended number of doses to achieve immunity as determined by the outcome of the evaluation
    /// process.
    /// </summary>
    [FhirElement("seriesDoses", Order = 30)]
    [Cardinality(0, 1)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate ImmunizationEvaluation cardinality
        var immunizationevaluationCount = ImmunizationEvaluation?.Count ?? 0;
        if (immunizationevaluationCount < 0)
        {
            yield return new ValidationResult("Element 'ImmunizationEvaluation' cardinality must be 0..*", new[] { nameof(ImmunizationEvaluation) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'ImmunizationEvaluation.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'ImmunizationEvaluation.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'ImmunizationEvaluation.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'ImmunizationEvaluation.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'ImmunizationEvaluation.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        if (Patient == null)
        {
            yield return new ValidationResult("Element 'ImmunizationEvaluation.patient' cardinality must be 1..1", new[] { nameof(Patient) });
        }
        if (TargetDisease == null)
        {
            yield return new ValidationResult("Element 'ImmunizationEvaluation.targetDisease' cardinality must be 1..1", new[] { nameof(TargetDisease) });
        }
        if (ImmunizationEvent == null)
        {
            yield return new ValidationResult("Element 'ImmunizationEvaluation.immunizationEvent' cardinality must be 1..1", new[] { nameof(ImmunizationEvent) });
        }
        if (DoseStatus == null)
        {
            yield return new ValidationResult("Element 'ImmunizationEvaluation.doseStatus' cardinality must be 1..1", new[] { nameof(DoseStatus) });
        }
        // Validate DoseStatusReason cardinality
        var dosestatusreasonCount = DoseStatusReason?.Count ?? 0;
        if (dosestatusreasonCount < 0)
        {
            yield return new ValidationResult("Element 'ImmunizationEvaluation.doseStatusReason' cardinality must be 0..*", new[] { nameof(DoseStatusReason) });
        }
        ...
        if (Status != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/immunization-evaluation-status|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(ImmunizationEvaluation) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(ImmunizationEvaluation) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(ImmunizationEvaluation) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(ImmunizationEvaluation) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(ImmunizationEvaluation) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(ImmunizationEvent) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(DoseStatus) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(DoseStatusReason) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
