...
public class Substance : DomainResource
{
    ...
    /// <summary>
    /// When the substance is no longer valid to use. For some substances, a single arbitrary date is used
    /// for expiry.
    /// </summary>
    [FhirElement("expiry", Order = 24)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("ingredient", Order = 26)]
    [Cardinality(0, int.MaxValue)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate Substance cardinality
        var substanceCount = Substance?.Count ?? 0;
        if (substanceCount < 0)
        {
            yield return new ValidationResult("Element 'Substance' cardinality must be 0..*", new[] { nameof(Substance) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'Substance.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Substance.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Substance.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'Substance.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Instance == null)
        {
            yield return new ValidationResult("Element 'Substance.instance' cardinality must be 1..1", new[] { nameof(Instance) });
        }
        // Validate Category cardinality
        var categoryCount = Category?.Count ?? 0;
        if (categoryCount < 0)
        {
            yield return new ValidationResult("Element 'Substance.category' cardinality must be 0..*", new[] { nameof(Category) });
        }
        if (Code == null)
        {
            yield return new ValidationResult("Element 'Substance.code' cardinality must be 1..1", new[] { nameof(Code) });
        }
        // Validate Ingredient cardinality
        var ingredientCount = Ingredient?.Count ?? 0;
        if (ingredientCount < 0)
        {
            yield return new ValidationResult("Element 'Substance.ingredient' cardinality must be 0..*", new[] { nameof(Ingredient) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Substance.ingredient.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Substance.ingredient.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Substance == null)
        {
            yield return new ValidationResult("Element 'Substance.ingredient.substance[x]' cardinality must be 1..1", new[] { nameof(Substance) });
        }
        ...
        if (Status != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/substance-status|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate Category ValueSet binding
        if (Category != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/substance-category
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(Substance) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(Substance) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(Substance) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(Substance) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(Substance) });
        // }
        // Constraint: ele-1
