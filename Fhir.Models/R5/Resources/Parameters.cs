...
/// <summary>
/// This resource is used to pass information into and back from an operation (whether invoked directly
/// from REST or within a messaging environment). It is not persisted or allowed to be referenced by
/// other resources except as described in the definition of the Parameters resource.
/// </summary>
public class Parameters : Resource
{
    ...
    /// </summary>
    [FhirElement("parameter", Order = 14)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate Parameters cardinality
        var parametersCount = Parameters?.Count ?? 0;
        if (parametersCount < 0)
        {
            yield return new ValidationResult("Element 'Parameters' cardinality must be 0..*", new[] { nameof(Parameters) });
        }
        // Validate Parameter cardinality
        var parameterCount = Parameter?.Count ?? 0;
        if (parameterCount < 0)
        {
            yield return new ValidationResult("Element 'Parameters.parameter' cardinality must be 0..*", new[] { nameof(Parameter) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Parameters.parameter.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Parameters.parameter.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Name == null)
        {
            yield return new ValidationResult("Element 'Parameters.parameter.name' cardinality must be 1..1", new[] { nameof(Name) });
        }
        // Validate Part cardinality
        var partCount = Part?.Count ?? 0;
        if (partCount < 0)
        {
            yield return new ValidationResult("Element 'Parameters.parameter.part' cardinality must be 0..*", new[] { nameof(Part) });
        }
        ...
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("(part.exists() and value.empty() and resource.empty()) or (part.empty() and (value.exists() xor resource.exists()))"))
        // {
        //     yield return new ValidationResult("A parameter must have one and only one of (value, resource, part)", new[] { nameof(Parameter) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
