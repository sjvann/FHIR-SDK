...
public class Organization : DomainResource
{
    ...
    /// <summary>
    /// Identifier for the organization that is used to identify the organization across multiple disparate
    /// systems.
    /// </summary>
    ...
    /// </summary>
    [FhirElement("alias", Order = 22)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("alias")]
    public List<FhirString>? Alias { get; set; }

    /// <summary>
    /// Description of the organization, which helps provide additional general context on the organization
    /// to ensure that the correct organization is selected.
    /// </summary>
    [FhirElement("description", Order = 23)]
    [Cardinality(0, 1)]
    [JsonPropertyName("description")]
    public FhirMarkdown? Description { get; set; }

    /// <summary>
    /// The contact details of communication devices available relevant to the specific Organization. This
    /// can include addresses, phone numbers, fax numbers, mobile numbers, email addresses and web sites.
    /// </summary>
    [FhirElement("contact", Order = 24)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// and/or otherwise endorse the provision of care by the organization. For example, an approval to
    /// provide a type of services issued by a certifying body (such as the US Joint Commission) to an
    /// organization.
    /// </summary>
    [FhirElement("qualification", Order = 27)]
    [Cardinality(0, int.MaxValue)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        {
            yield return new ValidationResult("Element 'Organization' cardinality must be 0..*", new[] { nameof(Organization) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'Organization.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Organization.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Organization.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'Organization.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        // Validate Type cardinality
        var typeCount = Type?.Count ?? 0;
        if (typeCount < 0)
        {
            yield return new ValidationResult("Element 'Organization.type' cardinality must be 0..*", new[] { nameof(Type) });
        }
        // Validate Alias cardinality
        var aliasCount = Alias?.Count ?? 0;
        if (aliasCount < 0)
        {
            yield return new ValidationResult("Element 'Organization.alias' cardinality must be 0..*", new[] { nameof(Alias) });
        }
        // Validate Contact cardinality
        var contactCount = Contact?.Count ?? 0;
        if (contactCount < 0)
        {
            yield return new ValidationResult("Element 'Organization.contact' cardinality must be 0..*", new[] { nameof(Contact) });
        }
        // Validate Endpoint cardinality
        var endpointCount = Endpoint?.Count ?? 0;
        if (endpointCount < 0)
        {
            yield return new ValidationResult("Element 'Organization.endpoint' cardinality must be 0..*", new[] { nameof(Endpoint) });
        }
        // Validate Qualification cardinality
        var qualificationCount = Qualification?.Count ?? 0;
        if (qualificationCount < 0)
        {
            yield return new ValidationResult("Element 'Organization.qualification' cardinality must be 0..*", new[] { nameof(Qualification) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Organization.qualification.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Organization.qualification.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'Organization.qualification.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Code == null)
        {
            yield return new ValidationResult("Element 'Organization.qualification.code' cardinality must be 1..1", new[] { nameof(Code) });
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(Organization) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(Organization) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(Organization) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(Organization) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(Organization) });
        // }
        // Constraint: org-1
        // Expression: (identifier.count() + name.count()) > 0
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("(identifier.count() + name.count()) > 0"))
        // {
        //     yield return new ValidationResult("The organization SHALL at least have a name or an identifier, and possibly more than one", new[] { nameof(Organization) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Qualification) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
