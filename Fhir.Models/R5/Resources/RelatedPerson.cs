...
public class RelatedPerson : DomainResource
{
    ...
    /// </summary>
    [FhirElement("relationship", Order = 21)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("birthDate", Order = 25)]
    [Cardinality(0, 1)]
    ...
    /// <summary>
    /// The period of time during which this relationship is or was active. If there are no dates defined,
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate RelatedPerson cardinality
        var relatedpersonCount = RelatedPerson?.Count ?? 0;
        if (relatedpersonCount < 0)
        {
            yield return new ValidationResult("Element 'RelatedPerson' cardinality must be 0..*", new[] { nameof(RelatedPerson) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'RelatedPerson.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'RelatedPerson.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'RelatedPerson.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'RelatedPerson.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Patient == null)
        {
            yield return new ValidationResult("Element 'RelatedPerson.patient' cardinality must be 1..1", new[] { nameof(Patient) });
        }
        // Validate Relationship cardinality
        var relationshipCount = Relationship?.Count ?? 0;
        if (relationshipCount < 0)
        {
            yield return new ValidationResult("Element 'RelatedPerson.relationship' cardinality must be 0..*", new[] { nameof(Relationship) });
        }
        // Validate Name cardinality
        var nameCount = Name?.Count ?? 0;
        if (nameCount < 0)
        {
            yield return new ValidationResult("Element 'RelatedPerson.name' cardinality must be 0..*", new[] { nameof(Name) });
        }
        // Validate Telecom cardinality
        var telecomCount = Telecom?.Count ?? 0;
        if (telecomCount < 0)
        {
            yield return new ValidationResult("Element 'RelatedPerson.telecom' cardinality must be 0..*", new[] { nameof(Telecom) });
        }
        // Validate Address cardinality
        var addressCount = Address?.Count ?? 0;
        if (addressCount < 0)
        {
            yield return new ValidationResult("Element 'RelatedPerson.address' cardinality must be 0..*", new[] { nameof(Address) });
        }
        // Validate Photo cardinality
        var photoCount = Photo?.Count ?? 0;
        if (photoCount < 0)
        {
            yield return new ValidationResult("Element 'RelatedPerson.photo' cardinality must be 0..*", new[] { nameof(Photo) });
        }
        // Validate Communication cardinality
        var communicationCount = Communication?.Count ?? 0;
        if (communicationCount < 0)
        {
            yield return new ValidationResult("Element 'RelatedPerson.communication' cardinality must be 0..*", new[] { nameof(Communication) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'RelatedPerson.communication.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'RelatedPerson.communication.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Language == null)
        {
            yield return new ValidationResult("Element 'RelatedPerson.communication.language' cardinality must be 1..1", new[] { nameof(Language) });
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(RelatedPerson) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(RelatedPerson) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(RelatedPerson) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(RelatedPerson) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(RelatedPerson) });
        // }
        // Constraint: ele-1
