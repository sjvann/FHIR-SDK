...
public class Subscription : DomainResource
{
    ...
    /// </summary>
    [FhirElement("end", Order = 23)]
    [Cardinality(0, 1)]
    [JsonPropertyName("end")]
    public FhirInstant? End { get; set; }

    /// <summary>
    /// Entity with authorization to make subsequent revisions to the Subscription and also determines what
    /// data the subscription is authorized to disclose.
    /// </summary>
    [FhirElement("managingEntity", Order = 24)]
    [Cardinality(0, 1)]
    [JsonPropertyName("managingEntity")]
    public Reference<CareTeam> ManagingEntity { get; set; }

    /// <summary>
    /// A description of why this subscription is defined.
    /// </summary>
    [FhirElement("reason", Order = 25)]
    [Cardinality(0, 1)]
    [JsonPropertyName("reason")]
    public FhirString? Reason { get; set; }

    /// <summary>
    /// The filter properties to be applied to narrow the subscription topic stream. When multiple filters
    /// are applied, evaluates to true if all the conditions applicable to that resource are met; otherwise
    /// it returns false (i.e., logical AND).
    /// </summary>
    [FhirElement("filterBy", Order = 26)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("filterBy")]
    public List<BackboneElement>? FilterBy { get; set; }

    /// <summary>
    /// The type of channel to send notifications on.
    /// </summary>
    [FhirElement("channelType", Order = 27)]
    [Cardinality(1, 1)]
    [Required]
    ...
    /// </summary>
    [FhirElement("parameter", Order = 29)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("parameter")]
    public List<BackboneElement>? Parameter { get; set; }

    /// <summary>
    /// If present, a &apos;heartbeat&apos; notification (keep-alive) is sent via this channel with an
    /// interval period equal to this elements integer value in seconds. If not present, a heartbeat
    /// notification is not sent.
    /// </summary>
    [FhirElement("heartbeatPeriod", Order = 30)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("timeout", Order = 31)]
    [Cardinality(0, 1)]
    [JsonPropertyName("timeout")]
    public FhirUnsignedInt? Timeout { get; set; }

    /// <summary>
    /// The MIME type to send the payload in - e.g., `application/fhir+xml` or `application/fhir+json`. Note
    /// that: * clients may request notifications in a specific FHIR version by using the [FHIR Version
    /// Parameter](http.html#version-parameter) - e.g., `application/fhir+json; fhirVersion=4.0`. *
    /// additional MIME types can be allowed by channels - e.g., `text/plain` and `text/html` are defined by
    /// the Email channel.
    /// </summary>
    [FhirElement("contentType", Order = 32)]
    [Cardinality(0, 1)]
    [JsonPropertyName("contentType")]
    public FhirCode? ContentType { get; set; }

    /// <summary>
    /// How much of the resource content to deliver in the notification payload. The choices are an empty
    /// payload, only the resource id, or the full resource content.
    /// </summary>
    [FhirElement("content", Order = 33)]
    [Cardinality(0, 1)]
    [JsonPropertyName("content")]
    public FhirCode? Content { get; set; }

    /// <summary>
    /// If present, the maximum number of events that will be included in a notification bundle. Note that
    /// this is not a strict limit on the number of entries in a bundle, as dependent resources can be
    /// included.
    /// </summary>
    [FhirElement("maxCount", Order = 34)]
    [Cardinality(0, 1)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate Subscription cardinality
        var subscriptionCount = Subscription?.Count ?? 0;
        if (subscriptionCount < 0)
        {
            yield return new ValidationResult("Element 'Subscription' cardinality must be 0..*", new[] { nameof(Subscription) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'Subscription.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Subscription.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Subscription.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'Subscription.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'Subscription.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        if (Topic == null)
        {
            yield return new ValidationResult("Element 'Subscription.topic' cardinality must be 1..1", new[] { nameof(Topic) });
        }
        // Validate Contact cardinality
        var contactCount = Contact?.Count ?? 0;
        if (contactCount < 0)
        {
            yield return new ValidationResult("Element 'Subscription.contact' cardinality must be 0..*", new[] { nameof(Contact) });
        }
        // Validate FilterBy cardinality
        var filterbyCount = FilterBy?.Count ?? 0;
        if (filterbyCount < 0)
        {
            yield return new ValidationResult("Element 'Subscription.filterBy' cardinality must be 0..*", new[] { nameof(FilterBy) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Subscription.filterBy.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Subscription.filterBy.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (FilterParameter == null)
        {
            yield return new ValidationResult("Element 'Subscription.filterBy.filterParameter' cardinality must be 1..1", new[] { nameof(FilterParameter) });
        }
        if (Value == null)
        {
            yield return new ValidationResult("Element 'Subscription.filterBy.value' cardinality must be 1..1", new[] { nameof(Value) });
        }
        if (ChannelType == null)
        {
            yield return new ValidationResult("Element 'Subscription.channelType' cardinality must be 1..1", new[] { nameof(ChannelType) });
        }
        // Validate Parameter cardinality
        var parameterCount = Parameter?.Count ?? 0;
        if (parameterCount < 0)
        {
            yield return new ValidationResult("Element 'Subscription.parameter' cardinality must be 0..*", new[] { nameof(Parameter) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Subscription.parameter.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Subscription.parameter.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Name == null)
        {
            yield return new ValidationResult("Element 'Subscription.parameter.name' cardinality must be 1..1", new[] { nameof(Name) });
        }
        if (Value == null)
        {
            yield return new ValidationResult("Element 'Subscription.parameter.value' cardinality must be 1..1", new[] { nameof(Value) });
        }
        ...
        if (Status != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/subscription-status|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // Validate ChannelType ValueSet binding
        if (ChannelType != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/subscription-channel-type
            // This requires a terminology service or local ValueSet cache
        }
        ...
        if (Content != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/subscription-payload-content|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(Subscription) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(Subscription) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(Subscription) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(Subscription) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(Subscription) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(FilterBy) });
        // }
        // Constraint: scr-1
        // Expression: (comparator.exists() and modifier.exists()).not()
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("(comparator.exists() and modifier.exists()).not()"))
        // {
        //     yield return new ValidationResult("Subscription filters may only contain a modifier or a comparator", new[] { nameof(FilterBy) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(HeartbeatPeriod) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Timeout) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(MaxCount) });
        // }
    }
