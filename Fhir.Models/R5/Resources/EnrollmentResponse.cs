...
public class EnrollmentResponse : DomainResource
{
    ...
    public Reference<EnrollmentRequest> Request { get; set; }

    /// <summary>
    /// Processing status: error, complete.
    /// </summary>
    [FhirElement("outcome", Order = 21)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("disposition", Order = 22)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("organization", Order = 24)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("requestProvider", Order = 25)]
    [Cardinality(0, 1)]
    [JsonPropertyName("requestProvider")]
    public Reference<Practitioner> RequestProvider { get; set; }
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate EnrollmentResponse cardinality
        var enrollmentresponseCount = EnrollmentResponse?.Count ?? 0;
        if (enrollmentresponseCount < 0)
        {
            yield return new ValidationResult("Element 'EnrollmentResponse' cardinality must be 0..*", new[] { nameof(EnrollmentResponse) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'EnrollmentResponse.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'EnrollmentResponse.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'EnrollmentResponse.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'EnrollmentResponse.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        ...
        if (Outcome != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/enrollment-outcome|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(EnrollmentResponse) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(EnrollmentResponse) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(EnrollmentResponse) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(EnrollmentResponse) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(EnrollmentResponse) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(RequestProvider) });
        // }
    }
