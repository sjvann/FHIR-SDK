...
public class CareTeam : DomainResource
{
    ...
    /// such as care plan team, episode of care team, longitudinal care team.
    /// </summary>
    ...
    /// </summary>
    [FhirElement("participant", Order = 24)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("managingOrganization", Order = 26)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("managingOrganization")]
    public List<Reference<Organization>>? ManagingOrganization { get; set; }

    /// <summary>
    /// A central contact detail for the care team (that applies to all members).
    /// </summary>
    [FhirElement("telecom", Order = 27)]
    [Cardinality(0, int.MaxValue)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        if (careteamCount < 0)
        {
            yield return new ValidationResult("Element 'CareTeam' cardinality must be 0..*", new[] { nameof(CareTeam) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'CareTeam.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'CareTeam.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'CareTeam.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'CareTeam.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        // Validate Category cardinality
        var categoryCount = Category?.Count ?? 0;
        if (categoryCount < 0)
        {
            yield return new ValidationResult("Element 'CareTeam.category' cardinality must be 0..*", new[] { nameof(Category) });
        }
        // Validate Participant cardinality
        var participantCount = Participant?.Count ?? 0;
        if (participantCount < 0)
        {
            yield return new ValidationResult("Element 'CareTeam.participant' cardinality must be 0..*", new[] { nameof(Participant) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'CareTeam.participant.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'CareTeam.participant.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Reason cardinality
        var reasonCount = Reason?.Count ?? 0;
        if (reasonCount < 0)
        {
            yield return new ValidationResult("Element 'CareTeam.reason' cardinality must be 0..*", new[] { nameof(Reason) });
        }
        // Validate ManagingOrganization cardinality
        var managingorganizationCount = ManagingOrganization?.Count ?? 0;
        if (managingorganizationCount < 0)
        {
            yield return new ValidationResult("Element 'CareTeam.managingOrganization' cardinality must be 0..*", new[] { nameof(ManagingOrganization) });
        }
        // Validate Telecom cardinality
        var telecomCount = Telecom?.Count ?? 0;
        if (telecomCount < 0)
        {
            yield return new ValidationResult("Element 'CareTeam.telecom' cardinality must be 0..*", new[] { nameof(Telecom) });
        }
        // Validate Note cardinality
        var noteCount = Note?.Count ?? 0;
        if (noteCount < 0)
        {
            yield return new ValidationResult("Element 'CareTeam.note' cardinality must be 0..*", new[] { nameof(Note) });
        }
        ...
        if (Status != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/care-team-status|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(CareTeam) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(CareTeam) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(CareTeam) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(CareTeam) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(CareTeam) });
        // }
        // Constraint: ele-1
        ...
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("onBehalfOf.exists() implies (member.resolve() is Practitioner)"))
        // {
        //     yield return new ValidationResult("CareTeam.participant.onBehalfOf can only be populated when CareTeam.participant.member is a Practitioner", new[] { nameof(Participant) });
        // }
        // Constraint: ctm-2
        // Expression: role.exists() or member.exists()
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("role.exists() or member.exists()"))
        // {
        //     yield return new ValidationResult("CareTeam.participant.role or CareTeam.participant.member exists", new[] { nameof(Participant) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
