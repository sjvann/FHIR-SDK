...
public class Location : DomainResource
{
    ...
    /// <summary>
    /// The status property covers the general availability of the resource, not the current value which may
    /// be covered by the operationStatus, or by a schedule/slots if they are configured for the location.
    /// </summary>
    ...
    /// rooms/units/chairs/etc. such as an isolation unit/dialysis chair). This typically covers concepts
    /// such as contamination, housekeeping, and other activities like maintenance.
    /// </summary>
    [FhirElement("operationalStatus", Order = 20)]
    [Cardinality(0, 1)]
    ...
    /// <summary>
    /// The contact details of communication devices available at the location. This can include addresses,
    /// phone numbers, fax numbers, mobile numbers, email addresses and web sites.
    /// </summary>
    [FhirElement("contact", Order = 26)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("managingOrganization", Order = 30)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("characteristic", Order = 32)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("characteristic")]
    public List<CodeableConcept>? Characteristic { get; set; }

    /// <summary>
    /// What days/times during a week is this location usually open, and any exceptions where the location
    /// is not available.
    /// </summary>
    [FhirElement("hoursOfOperation", Order = 33)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// number/details).
    /// </summary>
    [FhirElement("virtualService", Order = 34)]
    [Cardinality(0, int.MaxValue)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        if (locationCount < 0)
        {
            yield return new ValidationResult("Element 'Location' cardinality must be 0..*", new[] { nameof(Location) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'Location.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Location.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Location.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'Location.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        // Validate Alias cardinality
        var aliasCount = Alias?.Count ?? 0;
        if (aliasCount < 0)
        {
            yield return new ValidationResult("Element 'Location.alias' cardinality must be 0..*", new[] { nameof(Alias) });
        }
        // Validate Type cardinality
        var typeCount = Type?.Count ?? 0;
        if (typeCount < 0)
        {
            yield return new ValidationResult("Element 'Location.type' cardinality must be 0..*", new[] { nameof(Type) });
        }
        // Validate Contact cardinality
        var contactCount = Contact?.Count ?? 0;
        if (contactCount < 0)
        {
            yield return new ValidationResult("Element 'Location.contact' cardinality must be 0..*", new[] { nameof(Contact) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Location.position.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Location.position.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Longitude == null)
        {
            yield return new ValidationResult("Element 'Location.position.longitude' cardinality must be 1..1", new[] { nameof(Longitude) });
        }
        if (Latitude == null)
        {
            yield return new ValidationResult("Element 'Location.position.latitude' cardinality must be 1..1", new[] { nameof(Latitude) });
        }
        // Validate Characteristic cardinality
        var characteristicCount = Characteristic?.Count ?? 0;
        if (characteristicCount < 0)
        {
            yield return new ValidationResult("Element 'Location.characteristic' cardinality must be 0..*", new[] { nameof(Characteristic) });
        }
        // Validate HoursOfOperation cardinality
        var hoursofoperationCount = HoursOfOperation?.Count ?? 0;
        if (hoursofoperationCount < 0)
        {
            yield return new ValidationResult("Element 'Location.hoursOfOperation' cardinality must be 0..*", new[] { nameof(HoursOfOperation) });
        }
        // Validate VirtualService cardinality
        var virtualserviceCount = VirtualService?.Count ?? 0;
        if (virtualserviceCount < 0)
        {
            yield return new ValidationResult("Element 'Location.virtualService' cardinality must be 0..*", new[] { nameof(VirtualService) });
        }
        // Validate Endpoint cardinality
        var endpointCount = Endpoint?.Count ?? 0;
        if (endpointCount < 0)
        {
            yield return new ValidationResult("Element 'Location.endpoint' cardinality must be 0..*", new[] { nameof(Endpoint) });
        }
        ...
        if (Status != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/location-status|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate Mode ValueSet binding
        if (Mode != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/location-mode|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate Type ValueSet binding
        if (Type != null)
        {
            // TODO: Implement ValueSet validation for http://terminology.hl7.org/ValueSet/v3-ServiceDeliveryLocationRoleType
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(Location) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(Location) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(Location) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(Location) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(Location) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(OperationalStatus) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Position) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Longitude) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Latitude) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Altitude) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(HoursOfOperation) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
