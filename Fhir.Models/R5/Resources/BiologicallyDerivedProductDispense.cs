...
/// </summary>
public class BiologicallyDerivedProductDispense : DomainResource
{
    ...
    /// <summary>
    /// Unique instance identifiers assigned to a biologically derived product dispense. Note: This is a
    ...
    public List<Reference<BiologicallyDerivedProductDispense>>? PartOf { get; set; }
    ...
    /// <summary>
    /// Indicates the relationship between the donor of the biologically derived product and the intended
    /// recipient.
    /// </summary>
    [FhirElement("originRelationshipType", Order = 22)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("product", Order = 23)]
    [Cardinality(1, 1)]
    [Required]
    [JsonPropertyName("product")]
    public Reference<BiologicallyDerivedProduct> Product { get; set; }
    ...
    /// </summary>
    [FhirElement("matchStatus", Order = 25)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("performer", Order = 26)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// <summary>
    /// The amount of product in the dispense. Quantity will depend on the product being dispensed. Examples
    /// are: volume; cell count; concentration.
    /// </summary>
    [FhirElement("quantity", Order = 28)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("preparedDate", Order = 29)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("whenHandedOver", Order = 30)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("destination", Order = 31)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("usageInstruction", Order = 33)]
    [Cardinality(0, 1)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate BiologicallyDerivedProductDispense cardinality
        var biologicallyderivedproductdispenseCount = BiologicallyDerivedProductDispense?.Count ?? 0;
        if (biologicallyderivedproductdispenseCount < 0)
        {
            yield return new ValidationResult("Element 'BiologicallyDerivedProductDispense' cardinality must be 0..*", new[] { nameof(BiologicallyDerivedProductDispense) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'BiologicallyDerivedProductDispense.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'BiologicallyDerivedProductDispense.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'BiologicallyDerivedProductDispense.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'BiologicallyDerivedProductDispense.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        // Validate BasedOn cardinality
        var basedonCount = BasedOn?.Count ?? 0;
        if (basedonCount < 0)
        {
            yield return new ValidationResult("Element 'BiologicallyDerivedProductDispense.basedOn' cardinality must be 0..*", new[] { nameof(BasedOn) });
        }
        // Validate PartOf cardinality
        var partofCount = PartOf?.Count ?? 0;
        if (partofCount < 0)
        {
            yield return new ValidationResult("Element 'BiologicallyDerivedProductDispense.partOf' cardinality must be 0..*", new[] { nameof(PartOf) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'BiologicallyDerivedProductDispense.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        if (Product == null)
        {
            yield return new ValidationResult("Element 'BiologicallyDerivedProductDispense.product' cardinality must be 1..1", new[] { nameof(Product) });
        }
        if (Patient == null)
        {
            yield return new ValidationResult("Element 'BiologicallyDerivedProductDispense.patient' cardinality must be 1..1", new[] { nameof(Patient) });
        }
        // Validate Performer cardinality
        var performerCount = Performer?.Count ?? 0;
        if (performerCount < 0)
        {
            yield return new ValidationResult("Element 'BiologicallyDerivedProductDispense.performer' cardinality must be 0..*", new[] { nameof(Performer) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'BiologicallyDerivedProductDispense.performer.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'BiologicallyDerivedProductDispense.performer.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Actor == null)
        {
            yield return new ValidationResult("Element 'BiologicallyDerivedProductDispense.performer.actor' cardinality must be 1..1", new[] { nameof(Actor) });
        }
        // Validate Note cardinality
        var noteCount = Note?.Count ?? 0;
        if (noteCount < 0)
        {
            yield return new ValidationResult("Element 'BiologicallyDerivedProductDispense.note' cardinality must be 0..*", new[] { nameof(Note) });
        }
        ...
        if (Status != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/biologicallyderivedproductdispense-status|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(BiologicallyDerivedProductDispense) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(BiologicallyDerivedProductDispense) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(BiologicallyDerivedProductDispense) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(BiologicallyDerivedProductDispense) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(BiologicallyDerivedProductDispense) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(OriginRelationshipType) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(MatchStatus) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(PreparedDate) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
