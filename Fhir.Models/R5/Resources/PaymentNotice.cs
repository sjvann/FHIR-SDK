...
public class PaymentNotice : DomainResource
{
    ...
    /// </summary>
    [FhirElement("request", Order = 20)]
    [Cardinality(0, 1)]
    [JsonPropertyName("request")]
    public Reference<Resource> Request { get; set; }

    /// <summary>
    /// Reference of response to resource for which payment is being made.
    /// </summary>
    [FhirElement("response", Order = 21)]
    [Cardinality(0, 1)]
    [JsonPropertyName("response")]
    public Reference<Resource> Response { get; set; }

    /// <summary>
    /// The date when this resource was created.
    /// </summary>
    [FhirElement("created", Order = 22)]
    [Cardinality(1, 1)]
    [Required]
    ...
    /// </summary>
    [FhirElement("reporter", Order = 23)]
    [Cardinality(0, 1)]
    [JsonPropertyName("reporter")]
    public Reference<Practitioner> Reporter { get; set; }

    /// <summary>
    /// A reference to the payment which is the subject of this notice.
    /// </summary>
    [FhirElement("payment", Order = 24)]
    [Cardinality(0, 1)]
    [JsonPropertyName("payment")]
    public Reference<PaymentReconciliation> Payment { get; set; }

    /// <summary>
    /// The date when the above payment action occurred.
    /// </summary>
    [FhirElement("paymentDate", Order = 25)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("payee", Order = 26)]
    [Cardinality(0, 1)]
    [JsonPropertyName("payee")]
    public Reference<Practitioner> Payee { get; set; }

    /// <summary>
    /// The party who is notified of the payment status.
    /// </summary>
    [FhirElement("recipient", Order = 27)]
    [Cardinality(1, 1)]
    [Required]
    [JsonPropertyName("recipient")]
    public Reference<Organization> Recipient { get; set; }

    /// <summary>
    /// The amount sent to the payee.
    /// </summary>
    [FhirElement("amount", Order = 28)]
    [Cardinality(1, 1)]
    [Required]
    ...
    /// </summary>
    [FhirElement("paymentStatus", Order = 29)]
    [Cardinality(0, 1)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate PaymentNotice cardinality
        var paymentnoticeCount = PaymentNotice?.Count ?? 0;
        if (paymentnoticeCount < 0)
        {
            yield return new ValidationResult("Element 'PaymentNotice' cardinality must be 0..*", new[] { nameof(PaymentNotice) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'PaymentNotice.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'PaymentNotice.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'PaymentNotice.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'PaymentNotice.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'PaymentNotice.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        if (Created == null)
        {
            yield return new ValidationResult("Element 'PaymentNotice.created' cardinality must be 1..1", new[] { nameof(Created) });
        }
        if (Recipient == null)
        {
            yield return new ValidationResult("Element 'PaymentNotice.recipient' cardinality must be 1..1", new[] { nameof(Recipient) });
        }
        if (Amount == null)
        {
            yield return new ValidationResult("Element 'PaymentNotice.amount' cardinality must be 1..1", new[] { nameof(Amount) });
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(PaymentNotice) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(PaymentNotice) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(PaymentNotice) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(PaymentNotice) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(PaymentNotice) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Reporter) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(PaymentDate) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(PaymentStatus) });
        // }
    }
