...
public class NutritionProduct : DomainResource
{
    ...
    /// <summary>
    /// The code assigned to the product, for example a USDA NDB number, a USDA FDC ID number, or a Langual
    /// code.
    /// </summary>
    ...
    /// <summary>
    /// Nutrition products can have different classifications - according to its nutritional properties,
    ...
    /// <summary>
    /// The organisation (manufacturer, representative or legal authorization holder) that is responsible
    /// for the device.
    /// </summary>
    [FhirElement("manufacturer", Order = 21)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("ingredient", Order = 23)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("knownAllergen", Order = 24)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("characteristic", Order = 25)]
    [Cardinality(0, int.MaxValue)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate NutritionProduct cardinality
        var nutritionproductCount = NutritionProduct?.Count ?? 0;
        if (nutritionproductCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct' cardinality must be 0..*", new[] { nameof(NutritionProduct) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'NutritionProduct.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        // Validate Category cardinality
        var categoryCount = Category?.Count ?? 0;
        if (categoryCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct.category' cardinality must be 0..*", new[] { nameof(Category) });
        }
        // Validate Manufacturer cardinality
        var manufacturerCount = Manufacturer?.Count ?? 0;
        if (manufacturerCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct.manufacturer' cardinality must be 0..*", new[] { nameof(Manufacturer) });
        }
        // Validate Nutrient cardinality
        var nutrientCount = Nutrient?.Count ?? 0;
        if (nutrientCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct.nutrient' cardinality must be 0..*", new[] { nameof(Nutrient) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct.nutrient.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct.nutrient.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Amount cardinality
        var amountCount = Amount?.Count ?? 0;
        if (amountCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct.nutrient.amount' cardinality must be 0..*", new[] { nameof(Amount) });
        }
        // Validate Ingredient cardinality
        var ingredientCount = Ingredient?.Count ?? 0;
        if (ingredientCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct.ingredient' cardinality must be 0..*", new[] { nameof(Ingredient) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct.ingredient.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct.ingredient.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Item == null)
        {
            yield return new ValidationResult("Element 'NutritionProduct.ingredient.item' cardinality must be 1..1", new[] { nameof(Item) });
        }
        // Validate Amount cardinality
        var amountCount = Amount?.Count ?? 0;
        if (amountCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct.ingredient.amount' cardinality must be 0..*", new[] { nameof(Amount) });
        }
        // Validate KnownAllergen cardinality
        var knownallergenCount = KnownAllergen?.Count ?? 0;
        if (knownallergenCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct.knownAllergen' cardinality must be 0..*", new[] { nameof(KnownAllergen) });
        }
        // Validate Characteristic cardinality
        var characteristicCount = Characteristic?.Count ?? 0;
        if (characteristicCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct.characteristic' cardinality must be 0..*", new[] { nameof(Characteristic) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct.characteristic.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct.characteristic.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Type == null)
        {
            yield return new ValidationResult("Element 'NutritionProduct.characteristic.type' cardinality must be 1..1", new[] { nameof(Type) });
        }
        if (Value == null)
        {
            yield return new ValidationResult("Element 'NutritionProduct.characteristic.value[x]' cardinality must be 1..1", new[] { nameof(Value) });
        }
        // Validate Instance cardinality
        var instanceCount = Instance?.Count ?? 0;
        if (instanceCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct.instance' cardinality must be 0..*", new[] { nameof(Instance) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct.instance.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct.instance.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct.instance.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        // Validate Note cardinality
        var noteCount = Note?.Count ?? 0;
        if (noteCount < 0)
        {
            yield return new ValidationResult("Element 'NutritionProduct.note' cardinality must be 0..*", new[] { nameof(Note) });
        }
        ...
        if (Status != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/nutritionproduct-status|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(NutritionProduct) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(NutritionProduct) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(NutritionProduct) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(NutritionProduct) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(NutritionProduct) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(KnownAllergen) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(UseBy) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
