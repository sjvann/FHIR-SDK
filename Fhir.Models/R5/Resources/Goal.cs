...
public class Goal : DomainResource
{
    ...
    /// </summary>
    [FhirElement("lifecycleStatus", Order = 19)]
    [Cardinality(1, 1)]
    [Required]
    ...
    /// </summary>
    [FhirElement("achievementStatus", Order = 20)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("continuous", Order = 22)]
    [Cardinality(0, 1)]
    ...
    /// &quot;control blood pressure&quot; or &quot;negotiate an obstacle course&quot; or &quot;dance with
    /// child at wedding&quot;.
    /// </summary>
    ...
    /// <summary>
    /// The date or event after which the goal should begin being pursued. (as date)
    /// </summary>
    [FhirElement("startdateUnknown", Order = 26)]
    [Cardinality(0, 1)]
    [ChoiceType("startdate", "unknown")]
    [JsonPropertyName("startdateUnknown")]
    public FhirDate? StartDate { get; set; }

    /// <summary>
    /// The date or event after which the goal should begin being pursued. (as CodeableConcept)
    /// </summary>
    [FhirElement("startCodeableConcept", Order = 27)]
    [Cardinality(0, 1)]
    [ChoiceType("start", "codeableConcept")]
    ...
    /// </summary>
    [FhirElement("statusDate", Order = 29)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("statusReason", Order = 30)]
    [Cardinality(0, 1)]
    ...
    public Reference<Patient> Source { get; set; }

    /// <summary>
    /// The identified conditions and other health record elements that are intended to be addressed by the
    /// goal.
    /// </summary>
    [FhirElement("addresses", Order = 32)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("addresses")]
    public List<Reference<Condition>>? Addresses { get; set; }
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Choice Type validation for startdate[x]
        var startdateProperties = new[] { nameof(StartDate) };
        var startdateSetCount = startdateProperties.Count(prop => 
            GetType().GetProperty(prop)?.GetValue(this) != null);

        if (startdateSetCount > 1)
        {
            yield return new ValidationResult(
                "Only one of StartDate may be specified",
                new[] { nameof(StartDate) });
        }

        // Choice Type validation for start[x]
        var startProperties = new[] { nameof(StartCodeableConcept) };
        var startSetCount = startProperties.Count(prop => 
            GetType().GetProperty(prop)?.GetValue(this) != null);

        if (startSetCount > 1)
        {
            yield return new ValidationResult(
                "Only one of StartCodeableConcept may be specified",
                new[] { nameof(StartCodeableConcept) });
        }
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        if (goalCount < 0)
        {
            yield return new ValidationResult("Element 'Goal' cardinality must be 0..*", new[] { nameof(Goal) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'Goal.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Goal.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Goal.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'Goal.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (LifecycleStatus == null)
        {
            yield return new ValidationResult("Element 'Goal.lifecycleStatus' cardinality must be 1..1", new[] { nameof(LifecycleStatus) });
        }
        // Validate Category cardinality
        var categoryCount = Category?.Count ?? 0;
        if (categoryCount < 0)
        {
            yield return new ValidationResult("Element 'Goal.category' cardinality must be 0..*", new[] { nameof(Category) });
        }
        if (Description == null)
        {
            yield return new ValidationResult("Element 'Goal.description' cardinality must be 1..1", new[] { nameof(Description) });
        }
        if (Subject == null)
        {
            yield return new ValidationResult("Element 'Goal.subject' cardinality must be 1..1", new[] { nameof(Subject) });
        }
        // Validate Target cardinality
        var targetCount = Target?.Count ?? 0;
        if (targetCount < 0)
        {
            yield return new ValidationResult("Element 'Goal.target' cardinality must be 0..*", new[] { nameof(Target) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Goal.target.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Goal.target.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Addresses cardinality
        var addressesCount = Addresses?.Count ?? 0;
        if (addressesCount < 0)
        {
            yield return new ValidationResult("Element 'Goal.addresses' cardinality must be 0..*", new[] { nameof(Addresses) });
        }
        // Validate Note cardinality
        var noteCount = Note?.Count ?? 0;
        if (noteCount < 0)
        {
            yield return new ValidationResult("Element 'Goal.note' cardinality must be 0..*", new[] { nameof(Note) });
        }
        // Validate Outcome cardinality
        var outcomeCount = Outcome?.Count ?? 0;
        if (outcomeCount < 0)
        {
            yield return new ValidationResult("Element 'Goal.outcome' cardinality must be 0..*", new[] { nameof(Outcome) });
        }
        ...
        // Validate LifecycleStatus ValueSet binding
        if (LifecycleStatus != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/goal-status|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(Goal) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(Goal) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(Goal) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(Goal) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(Goal) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(LifecycleStatus) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(AchievementStatus) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Continuous) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("(detail.exists() and measure.exists()) or detail.exists().not()"))
        // {
        //     yield return new ValidationResult("Goal.target.measure is required if Goal.target.detail is populated", new[] { nameof(Target) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
