...
public class SupplyDelivery : DomainResource
{
    ...
    public List<Reference<SupplyRequest>>? BasedOn { get; set; }
    ...
    public List<Reference<SupplyDelivery>>? PartOf { get; set; }
    ...
    /// </summary>
    [FhirElement("suppliedItem", Order = 24)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("suppliedItem")]
    public List<BackboneElement>? SuppliedItem { get; set; }

    /// <summary>
    /// The date or time(s) the activity occurred. (as dateTime)
    /// </summary>
    ...
    /// <summary>
    /// The date or time(s) the activity occurred. (as Period)
    /// </summary>
    ...
    /// <summary>
    /// The date or time(s) the activity occurred. (as Timing)
    /// </summary>
    ...
    public Reference<Practitioner> Supplier { get; set; }

    /// <summary>
    /// Identification of the facility/location where the delivery was shipped to.
    /// </summary>
    [FhirElement("destination", Order = 29)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("receiver", Order = 30)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("receiver")]
    public List<Reference<Practitioner>>? Receiver { get; set; }
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate SupplyDelivery cardinality
        var supplydeliveryCount = SupplyDelivery?.Count ?? 0;
        if (supplydeliveryCount < 0)
        {
            yield return new ValidationResult("Element 'SupplyDelivery' cardinality must be 0..*", new[] { nameof(SupplyDelivery) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'SupplyDelivery.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'SupplyDelivery.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'SupplyDelivery.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'SupplyDelivery.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        // Validate BasedOn cardinality
        var basedonCount = BasedOn?.Count ?? 0;
        if (basedonCount < 0)
        {
            yield return new ValidationResult("Element 'SupplyDelivery.basedOn' cardinality must be 0..*", new[] { nameof(BasedOn) });
        }
        // Validate PartOf cardinality
        var partofCount = PartOf?.Count ?? 0;
        if (partofCount < 0)
        {
            yield return new ValidationResult("Element 'SupplyDelivery.partOf' cardinality must be 0..*", new[] { nameof(PartOf) });
        }
        // Validate SuppliedItem cardinality
        var supplieditemCount = SuppliedItem?.Count ?? 0;
        if (supplieditemCount < 0)
        {
            yield return new ValidationResult("Element 'SupplyDelivery.suppliedItem' cardinality must be 0..*", new[] { nameof(SuppliedItem) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'SupplyDelivery.suppliedItem.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'SupplyDelivery.suppliedItem.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Receiver cardinality
        var receiverCount = Receiver?.Count ?? 0;
        if (receiverCount < 0)
        {
            yield return new ValidationResult("Element 'SupplyDelivery.receiver' cardinality must be 0..*", new[] { nameof(Receiver) });
        }
        ...
        if (Status != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/supplydelivery-status|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate Type ValueSet binding
        if (Type != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/supplydelivery-supplyitemtype|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(SupplyDelivery) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(SupplyDelivery) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(SupplyDelivery) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(SupplyDelivery) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(SupplyDelivery) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(SuppliedItem) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
