...
public class Linkage : DomainResource
{
    ...
    /// </summary>
    [FhirElement("active", Order = 18)]
    [Cardinality(0, 1)]
    [JsonPropertyName("active")]
    public FhirBoolean? Active { get; set; }

    /// <summary>
    /// Identifies the user or organization responsible for asserting the linkages as well as the user or
    /// organization who establishes the context in which the nature of each linkage is evaluated.
    /// </summary>
    [FhirElement("author", Order = 19)]
    [Cardinality(0, 1)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate Linkage cardinality
        var linkageCount = Linkage?.Count ?? 0;
        if (linkageCount < 0)
        {
            yield return new ValidationResult("Element 'Linkage' cardinality must be 0..*", new[] { nameof(Linkage) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'Linkage.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Linkage.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Linkage.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Item cardinality
        var itemCount = Item?.Count ?? 0;
        if (itemCount < 1)
        {
            yield return new ValidationResult("Element 'Linkage.item' cardinality must be 1..*", new[] { nameof(Item) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Linkage.item.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Linkage.item.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Type == null)
        {
            yield return new ValidationResult("Element 'Linkage.item.type' cardinality must be 1..1", new[] { nameof(Type) });
        }
        if (Resource == null)
        {
            yield return new ValidationResult("Element 'Linkage.item.resource' cardinality must be 1..1", new[] { nameof(Resource) });
        }
        ...
        if (Type != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/linkage-type|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(Linkage) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(Linkage) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(Linkage) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(Linkage) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(Linkage) });
        // }
        // Constraint: lnk-1
        // Expression: item.count()>1
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("item.count()>1"))
        // {
        //     yield return new ValidationResult("Must have at least two items", new[] { nameof(Linkage) });
        // }
        // Constraint: ele-1
