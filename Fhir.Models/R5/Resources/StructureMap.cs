...
public class StructureMap : DomainResource
{
    ...
    /// map is (or will be) published. This URL can be the target of a canonical reference. It SHALL remain
    ...
    /// specification, model, design or instance. This is an arbitrary value managed by the structure map
    ...
    /// <summary>
    /// A natural language name identifying the structure map. This name should be usable as an identifier
    ...
    /// <summary>
    /// The date (and optionally time) when the structure map was last significantly changed. The date must
    ...
    /// <summary>
    /// A copyright statement relating to the structure map and/or its contents. Copyright statements are
    /// generally legal restrictions on the use and publishing of the structure map.
    /// </summary>
    ...
    /// </summary>
    [FhirElement("structure", Order = 36)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// </summary>
    [FhirElement("const", Order = 38)]
    [Cardinality(0, int.MaxValue)]
    [JsonPropertyName("const")]
    public List<BackboneElement>? Const { get; set; }

    /// <summary>
    /// Organizes the mapping into managable chunks for human review/ease of maintenance.
    /// </summary>
    [FhirElement("group", Order = 39)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate StructureMap cardinality
        var structuremapCount = StructureMap?.Count ?? 0;
        if (structuremapCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap' cardinality must be 0..*", new[] { nameof(StructureMap) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Url == null)
        {
            yield return new ValidationResult("Element 'StructureMap.url' cardinality must be 1..1", new[] { nameof(Url) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Name == null)
        {
            yield return new ValidationResult("Element 'StructureMap.name' cardinality must be 1..1", new[] { nameof(Name) });
        }
        if (Status == null)
        {
            yield return new ValidationResult("Element 'StructureMap.status' cardinality must be 1..1", new[] { nameof(Status) });
        }
        // Validate Contact cardinality
        var contactCount = Contact?.Count ?? 0;
        if (contactCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.contact' cardinality must be 0..*", new[] { nameof(Contact) });
        }
        // Validate UseContext cardinality
        var usecontextCount = UseContext?.Count ?? 0;
        if (usecontextCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.useContext' cardinality must be 0..*", new[] { nameof(UseContext) });
        }
        // Validate Jurisdiction cardinality
        var jurisdictionCount = Jurisdiction?.Count ?? 0;
        if (jurisdictionCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.jurisdiction' cardinality must be 0..*", new[] { nameof(Jurisdiction) });
        }
        // Validate Structure cardinality
        var structureCount = Structure?.Count ?? 0;
        if (structureCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.structure' cardinality must be 0..*", new[] { nameof(Structure) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.structure.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.structure.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Url == null)
        {
            yield return new ValidationResult("Element 'StructureMap.structure.url' cardinality must be 1..1", new[] { nameof(Url) });
        }
        if (Mode == null)
        {
            yield return new ValidationResult("Element 'StructureMap.structure.mode' cardinality must be 1..1", new[] { nameof(Mode) });
        }
        // Validate Import cardinality
        var importCount = Import?.Count ?? 0;
        if (importCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.import' cardinality must be 0..*", new[] { nameof(Import) });
        }
        // Validate Const cardinality
        var constCount = Const?.Count ?? 0;
        if (constCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.const' cardinality must be 0..*", new[] { nameof(Const) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.const.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.const.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Group cardinality
        var groupCount = Group?.Count ?? 0;
        if (groupCount < 1)
        {
            yield return new ValidationResult("Element 'StructureMap.group' cardinality must be 1..*", new[] { nameof(Group) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.group.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.group.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Name == null)
        {
            yield return new ValidationResult("Element 'StructureMap.group.name' cardinality must be 1..1", new[] { nameof(Name) });
        }
        // Validate Input cardinality
        var inputCount = Input?.Count ?? 0;
        if (inputCount < 1)
        {
            yield return new ValidationResult("Element 'StructureMap.group.input' cardinality must be 1..*", new[] { nameof(Input) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.group.input.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.group.input.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Name == null)
        {
            yield return new ValidationResult("Element 'StructureMap.group.input.name' cardinality must be 1..1", new[] { nameof(Name) });
        }
        if (Mode == null)
        {
            yield return new ValidationResult("Element 'StructureMap.group.input.mode' cardinality must be 1..1", new[] { nameof(Mode) });
        }
        // Validate Rule cardinality
        var ruleCount = Rule?.Count ?? 0;
        if (ruleCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.group.rule' cardinality must be 0..*", new[] { nameof(Rule) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.group.rule.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.group.rule.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Source cardinality
        var sourceCount = Source?.Count ?? 0;
        if (sourceCount < 1)
        {
            yield return new ValidationResult("Element 'StructureMap.group.rule.source' cardinality must be 1..*", new[] { nameof(Source) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.group.rule.source.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.group.rule.source.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Context == null)
        {
            yield return new ValidationResult("Element 'StructureMap.group.rule.source.context' cardinality must be 1..1", new[] { nameof(Context) });
        }
        // Validate Target cardinality
        var targetCount = Target?.Count ?? 0;
        if (targetCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.group.rule.target' cardinality must be 0..*", new[] { nameof(Target) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.group.rule.target.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.group.rule.target.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate ListMode cardinality
        var listmodeCount = ListMode?.Count ?? 0;
        if (listmodeCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.group.rule.target.listMode' cardinality must be 0..*", new[] { nameof(ListMode) });
        }
        // Validate Parameter cardinality
        var parameterCount = Parameter?.Count ?? 0;
        if (parameterCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.group.rule.target.parameter' cardinality must be 0..*", new[] { nameof(Parameter) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.group.rule.target.parameter.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.group.rule.target.parameter.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Value == null)
        {
            yield return new ValidationResult("Element 'StructureMap.group.rule.target.parameter.value[x]' cardinality must be 1..1", new[] { nameof(Value) });
        }
        // Validate Rule cardinality
        var ruleCount = Rule?.Count ?? 0;
        if (ruleCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.group.rule.rule' cardinality must be 0..*", new[] { nameof(Rule) });
        }
        // Validate Dependent cardinality
        var dependentCount = Dependent?.Count ?? 0;
        if (dependentCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.group.rule.dependent' cardinality must be 0..*", new[] { nameof(Dependent) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.group.rule.dependent.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'StructureMap.group.rule.dependent.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Name == null)
        {
            yield return new ValidationResult("Element 'StructureMap.group.rule.dependent.name' cardinality must be 1..1", new[] { nameof(Name) });
        }
        // Validate Parameter cardinality
        var parameterCount = Parameter?.Count ?? 0;
        if (parameterCount < 1)
        {
            yield return new ValidationResult("Element 'StructureMap.group.rule.dependent.parameter' cardinality must be 1..*", new[] { nameof(Parameter) });
        }
        ...
        if (Mode != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/map-model-mode|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate TypeMode ValueSet binding
        if (TypeMode != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/map-group-type-mode|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate Mode ValueSet binding
        if (Mode != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/map-input-mode|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate ListMode ValueSet binding
        if (ListMode != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/map-source-list-mode|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate ListMode ValueSet binding
        if (ListMode != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/map-target-list-mode|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        // Validate Transform ValueSet binding
        if (Transform != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/map-transform|5.0.0
            // This requires a terminology service or local ValueSet cache
        }
        ...
        // {
        //     yield return new ValidationResult("Name should be usable as an identifier for the module by machine processing applications such as code generation", new[] { nameof(StructureMap) });
        // }
        // Constraint: ele-1
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Import) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Const) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Extends) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(TypeMode) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(ListMode) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Check) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(LogMessage) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // TODO: Implement FHIRPath expression evaluation
        // if (!EvaluateFHIRPath("element.exists() implies context.exists()"))
        // {
        //     yield return new ValidationResult("Can only have an element if you have a context", new[] { nameof(Target) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(ListRuleId) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
        ...
        // if (!EvaluateFHIRPath("hasValue() or (children().count() > id.count())"))
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Dependent) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
