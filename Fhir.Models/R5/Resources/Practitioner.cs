...
public class Practitioner : DomainResource
{
    ...
    /// <summary>
    /// Indicates if the practitioner is deceased or not. (as boolean)
    /// </summary>
    ...
    /// <summary>
    /// Indicates if the practitioner is deceased or not. (as dateTime)
    /// </summary>
    ...
    /// <summary>
    /// Address(es) of the practitioner that are not role specific (typically home address). Work addresses
    ...
    /// practitioner. For example, a medical license issued by a medical board of licensure authorizing the
    /// practitioner to practice medicine within a certain locality.
    /// </summary>
    [FhirElement("qualification", Order = 28)]
    [Cardinality(0, int.MaxValue)]
    ...
    /// correspondence/administrative purposes. The `PractitionerRole.communication` property should be used
    /// for publishing the languages that a practitioner is able to communicate with patients (on a per
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate Practitioner cardinality
        var practitionerCount = Practitioner?.Count ?? 0;
        if (practitionerCount < 0)
        {
            yield return new ValidationResult("Element 'Practitioner' cardinality must be 0..*", new[] { nameof(Practitioner) });
        }
        // Validate Contained cardinality
        var containedCount = Contained?.Count ?? 0;
        if (containedCount < 0)
        {
            yield return new ValidationResult("Element 'Practitioner.contained' cardinality must be 0..*", new[] { nameof(Contained) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Practitioner.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Practitioner.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'Practitioner.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        // Validate Name cardinality
        var nameCount = Name?.Count ?? 0;
        if (nameCount < 0)
        {
            yield return new ValidationResult("Element 'Practitioner.name' cardinality must be 0..*", new[] { nameof(Name) });
        }
        // Validate Telecom cardinality
        var telecomCount = Telecom?.Count ?? 0;
        if (telecomCount < 0)
        {
            yield return new ValidationResult("Element 'Practitioner.telecom' cardinality must be 0..*", new[] { nameof(Telecom) });
        }
        // Validate Address cardinality
        var addressCount = Address?.Count ?? 0;
        if (addressCount < 0)
        {
            yield return new ValidationResult("Element 'Practitioner.address' cardinality must be 0..*", new[] { nameof(Address) });
        }
        // Validate Photo cardinality
        var photoCount = Photo?.Count ?? 0;
        if (photoCount < 0)
        {
            yield return new ValidationResult("Element 'Practitioner.photo' cardinality must be 0..*", new[] { nameof(Photo) });
        }
        // Validate Qualification cardinality
        var qualificationCount = Qualification?.Count ?? 0;
        if (qualificationCount < 0)
        {
            yield return new ValidationResult("Element 'Practitioner.qualification' cardinality must be 0..*", new[] { nameof(Qualification) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Practitioner.qualification.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Practitioner.qualification.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        // Validate Identifier cardinality
        var identifierCount = Identifier?.Count ?? 0;
        if (identifierCount < 0)
        {
            yield return new ValidationResult("Element 'Practitioner.qualification.identifier' cardinality must be 0..*", new[] { nameof(Identifier) });
        }
        if (Code == null)
        {
            yield return new ValidationResult("Element 'Practitioner.qualification.code' cardinality must be 1..1", new[] { nameof(Code) });
        }
        // Validate Communication cardinality
        var communicationCount = Communication?.Count ?? 0;
        if (communicationCount < 0)
        {
            yield return new ValidationResult("Element 'Practitioner.communication' cardinality must be 0..*", new[] { nameof(Communication) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Practitioner.communication.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'Practitioner.communication.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        if (Language == null)
        {
            yield return new ValidationResult("Element 'Practitioner.communication.language' cardinality must be 1..1", new[] { nameof(Language) });
        }
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL NOT contain nested Resources", new[] { nameof(Practitioner) });
        // }
        // Constraint: dom-3
        ...
        // {
        //     yield return new ValidationResult("If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource or SHALL refer to the containing resource", new[] { nameof(Practitioner) });
        // }
        // Constraint: dom-4
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated", new[] { nameof(Practitioner) });
        // }
        // Constraint: dom-5
        ...
        // {
        //     yield return new ValidationResult("If a resource is contained in another resource, it SHALL NOT have a security label", new[] { nameof(Practitioner) });
        // }
        // Constraint: dom-6
        ...
        // {
        //     yield return new ValidationResult("A resource should have narrative for robust management", new[] { nameof(Practitioner) });
        // }
        // Constraint: ele-1
