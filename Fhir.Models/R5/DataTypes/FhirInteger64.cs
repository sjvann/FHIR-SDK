// Auto-generated for FHIR R5
using System.ComponentModel.DataAnnotations;
using System.Text.Json.Serialization;
using Fhir.Support.Base;
using Fhir.Support.Attributes;

namespace Fhir.R5.Models;

/// <summary>
/// A very large whole number
/// </summary>
public class FhirInteger64 : FhirNumericType<long>
{
    /// <summary>
    /// 建立新的 FhirInteger64 實例
    /// </summary>
    public FhirInteger64() { }

    /// <summary>
    /// 建立新的 FhirInteger64 實例
    /// </summary>
    /// <param name="value">初始值</param>
    public FhirInteger64(long? value) : base(value) { }

    /// <summary>
    /// 隱式轉換從 long?
    /// </summary>
    /// <param name="value">原生值</param>
    public static implicit operator FhirInteger64?(long? value)
    {
        return value == null ? null : new FhirInteger64(value);
    }

    /// <summary>
    /// 隱式轉換到 long?
    /// </summary>
    /// <param name="fhirValue">FHIR 值</param>
    public static implicit operator long?(FhirInteger64? fhirValue)
    {
        return fhirValue?.Value;
    }
}
