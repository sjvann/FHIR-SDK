...
public class BackboneType : DataType
{
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate BackboneType cardinality
        var backbonetypeCount = BackboneType?.Count ?? 0;
        if (backbonetypeCount < 0)
        {
            yield return new ValidationResult("Element 'BackboneType' cardinality must be 0..*", new[] { nameof(BackboneType) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'BackboneType.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        // Validate ModifierExtension cardinality
        var modifierextensionCount = ModifierExtension?.Count ?? 0;
        if (modifierextensionCount < 0)
        {
            yield return new ValidationResult("Element 'BackboneType.modifierExtension' cardinality must be 0..*", new[] { nameof(ModifierExtension) });
        }
        ...
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(BackboneType) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
