...
public class Distance : Element
{
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate Distance cardinality
        var distanceCount = Distance?.Count ?? 0;
        if (distanceCount < 0)
        {
            yield return new ValidationResult("Element 'Distance' cardinality must be 0..*", new[] { nameof(Distance) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Distance.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }

        // ValueSet binding validation
        // Validate Distance ValueSet binding
        if (Distance != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/distance-units
        ...
        // if (!EvaluateFHIRPath("(code.exists() or value.empty()) and (system.empty() or system = %ucum)"))
        // {
        //     yield return new ValidationResult("There SHALL be a code if there is a value and it SHALL be an expression of length.  If system is present, it SHALL be UCUM.", new[] { nameof(Distance) });
        // }
        // Constraint: ele-1
        ...
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(Distance) });
        // }
        // Constraint: qty-3
        ...
        // {
        //     yield return new ValidationResult("If a code for the unit is present, the system SHALL also be present", new[] { nameof(Distance) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
