...
public class CodeableReference : DataType
{
    ...
    /// <summary>
    /// A reference to a concept - e.g. the information is identified by its general class to the degree of
    /// precision found in the terminology.
    /// </summary>
    [FhirElement("concept", Order = 12)]
    [Cardinality(0, 1)]
    ...
    /// </summary>
    [FhirElement("reference", Order = 13)]
    [Cardinality(0, 1)]
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate CodeableReference cardinality
        var codeablereferenceCount = CodeableReference?.Count ?? 0;
        if (codeablereferenceCount < 0)
        {
            yield return new ValidationResult("Element 'CodeableReference' cardinality must be 0..*", new[] { nameof(CodeableReference) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'CodeableReference.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        ...
        // {
        //     yield return new ValidationResult("All FHIR elements must have a @value or children", new[] { nameof(CodeableReference) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
