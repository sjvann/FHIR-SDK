...
public class Count : Element
{
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate Count cardinality
        var countCount = Count?.Count ?? 0;
        if (countCount < 0)
        {
            yield return new ValidationResult("Element 'Count' cardinality must be 0..*", new[] { nameof(Count) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Count.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }
        ...
        // if (!EvaluateFHIRPath("(code.exists() or value.empty()) and (system.empty() or system = %ucum) and (code.empty() or code = '1') and (value.empty() or value.hasValue().not() or value.toString().contains('.').not())"))
        // {
        //     yield return new ValidationResult("There SHALL be a code with a value of "1" if there is a value. If system is present, it SHALL be UCUM.  If present, the value SHALL be a whole number.", new[] { nameof(Count) });
        // }
        // Constraint: ele-1
        ...
        // {
        //     yield return new ValidationResult("If a code for the unit is present, the system SHALL also be present", new[] { nameof(Count) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
