...
public class Age : Element
{
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // Validate Age cardinality
        var ageCount = Age?.Count ?? 0;
        if (ageCount < 0)
        {
            yield return new ValidationResult("Element 'Age' cardinality must be 0..*", new[] { nameof(Age) });
        }
        // Validate Extension cardinality
        var extensionCount = Extension?.Count ?? 0;
        if (extensionCount < 0)
        {
            yield return new ValidationResult("Element 'Age.extension' cardinality must be 0..*", new[] { nameof(Extension) });
        }

        // ValueSet binding validation
        // Validate Age ValueSet binding
        if (Age != null)
        {
            // TODO: Implement ValueSet validation for http://hl7.org/fhir/ValueSet/age-units
        ...
        // if (!EvaluateFHIRPath("(code.exists() or value.empty()) and (system.empty() or system = %ucum) and (value.empty() or value.hasValue().not() or value > 0)"))
        // {
        //     yield return new ValidationResult("There SHALL be a code if there is a value and it SHALL be an expression of time.  If system is present, it SHALL be UCUM.  If value is present, it SHALL be positive.", new[] { nameof(Age) });
        // }
        // Constraint: ele-1
        ...
        // {
        //     yield return new ValidationResult("If a code for the unit is present, the system SHALL also be present", new[] { nameof(Age) });
        // }
        // Constraint: ele-1
        // Expression: hasValue() or (children().count() > id.count())
        // TODO: Implement FHIRPath expression evaluation
